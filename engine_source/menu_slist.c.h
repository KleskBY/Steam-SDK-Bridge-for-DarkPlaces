...
RELATED_ (last_nav_cname Net_SlistQW_f Commit_To_Cname NetConn_ClientParsePacket)
...
RELATED_ (serverlist_list, serverlist_list_count)
...
RELATED_ (Net_SlistQW_f, Net_Slist_f ServerList_QueryList)

// Baker: serverlist_viewlist is a subset of serverlist_cache (basically)
RELATED_ (serverlist_viewlist serverlist_viewlist_count serverlist_cache)
RELATED_ (NetConn_ClientParsePacket_ServerList_UpdateCache which line 1 is written)
...
#if !defined(INVADERS) && !defined(DARKNESS) // Server list DP + QW
		if (elapsed > 300) {
...
static void M_ServerList_Draw (void)
{
	...
	M_DrawPic((640 - Draw_GetPicWidth(p0)) / 2, 4, "gfx/p_multi", HOTSPOTS_NONE_0, NA0, NA0);

	RELATED_ (NetConn_ClientParsePacket_ServerList_UpdateCache)
	...
	case K_MOUSE1:
...

		{
			...
			if (is_new_cursor == false && click_delta_time && click_delta_time < DOUBLE_CLICK_0_5) {
				// fall thru to K_ENTER and connect
			} else {
...
#if !defined(INVADERS) && !defined(DARKNESS) // Server list
		Net_Slist_Both_f (cmd);
#else
		if (lanConfig_cursor == 2)
			Net_SlistQW_f(cmd);
