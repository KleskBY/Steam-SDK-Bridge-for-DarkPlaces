...
{
	KeyUp(cmd, &in_mlook);
	if ( !(in_mlook.state&1) && lookspring.value /*d:0*/)
...
static void IN_BestWeapon_f(cmd_state_t *cmd)
{
	...
	const char *t;
	if (Cmd_Argc(cmd) < 2) {
		Con_PrintLinef ("bestweapon requires 1 or more parameters");
		return;
	}
...
void CL_Input (void)
{
	...
	if (!key_consoleactive && key_dest == key_game && !cl.csqc_wantsmousemove && cl_prydoncursor.integer <= 0) { // MENUMOUSE
...
CALLERS_ (CL_ClientMovement_UpdateStatus)
...
static qbool CL_SendClientCommand (int is_reliable, int clc_code, char *msg)
{
	...
	if (is_reliable) {
		...
		RELATED_ (NetConn_SendUnreliableMessage)
...
RELATED_ (SV_ReadClientMessage SV_ReadClientMessage_ReadClientMove)
RELATED_ () // Baker: Where is read from server?
...
#if 0 // PHYSICAL
	RELATED_ (ZIRCON_PEXT ZIRCON_EXT_CHUNKED_2)
			...
			for (mj = 0, mseq_cmd = &cl.movecmd[maxusercmds - 1]; mj < maxusercmds; mj++, mseq_cmd --) {
				...
				if (cls.protocol != PROTOCOL_DARKPLACES6) {
					...
					if (is_zircon_move) {
						...
						RELATED_ (CL_ClientMovement_Replay)
