...
typedef struct texture_s
{
	...
	int currentmaterialflags; // Baker: MATERIALFLAG_BLENDED etc.
	...
	rtexture_t *rreflectmasktexture; // mask for fake reflections
	rtexture_t *rreflectcubetexture; // fake reflections cubemap
...
typedef struct msurface_s
{
	...
	struct q3deffect_s *q3_effect_msurf; // q3bsp // Baker: I believe these are SLICK, SKY, LADDER, NOIMPACT, NOMARKS, FLESH, NODRAW, HINT, DUST, METALSTEPS
...
extern struct cvar_s external_vis;
...
qbool Mod_LoadTextureFromQ3Shader(mempool_t *mempool, const char *modelname, texture_t *texture, 
					const char *name, qbool warnmissing, qbool fallback, int shall_do_external, 
					int defaulttexflags, int defaultmaterialflags);
texture_shaderpass_t *Mod_CreateShaderPass(mempool_t *mempool, struct skinframe_s *skinframe);
...
void Mod_LoadCustomMaterial(mempool_t *mempool, texture_t *texture, const char *name, int supercontents, int materialflags, struct skinframe_s *skinframe);
