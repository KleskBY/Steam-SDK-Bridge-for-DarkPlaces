...
static qbool Mod_Alias_CalculateBoundingBox(void)
{
	...
	if (loadmodel->AnimateVertices)
	{
		...
		{
			RELATED_ (Mod_MDL_AnimateVertices) // Mod_Alias_CalculateBoundingBox
...
static void Mod_MDLMD2MD3_TraceBox(model_t *model, const frameblend_t *frameblend, const skeleton_t *skeleton, trace_t *trace, const vec3_t start, const vec3_t boxmins, const vec3_t boxmaxs, const vec3_t end, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask)
{
	...
	for (i = 0, surface = model->data_surfaces;i < model->num_surfaces;i++, surface++)
		Collision_TraceBrushTriangleMeshFloat(trace, &thisbrush_start.brush, &thisbrush_end.brush, surface->num_triangles, model->surfmesh.data_element3i + 3 * surface->num_firsttriangle, vertex3f, 0, NULL, SUPERCONTENTS_SOLID | (surface->texture->basematerialflags & MATERIALFLAGMASK_TRANSLUCENT ? 0 : SUPERCONTENTS_OPAQUE), Q3SURFACEFLAG_NONE_0, surface->texture, segmentmins, segmentmaxs);
...
void Mod_BuildAliasSkinsFromSkinFiles(texture_t *skin, skinfile_t *skinfile, const char *meshname, const char *shadername)
{
	...
	if (developer_extra.integer)
		Con_DPrintLinef ("Looking up texture for %s (default: %s)", meshname, shadername);
		...
		for (i = 0;skinfile;skinfile = skinfile->next, i++, skin += loadmodel->num_surfaces)
		{
			...
			for (skinfileitem = skinfile->items; skinfileitem; skinfileitem = skinfileitem->next)
			{
				...
				if (String_Match(skinfileitem->skname, meshname))
				{
					...
					if (developer_extra.integer)
						Con_DPrintLinef ("--> got %s from skin file", stripbuf);
					Mod_LoadTextureFromQ3Shader(loadmodel->mempool, loadmodel->model_name, skin, stripbuf, q_tx_warn_missing_true, q_tx_fallback_notexture_true, q_tx_do_external_true, 
						(r_mipskins.integer ? TEXF_MIPMAP : 0) | TEXF_ALPHA | TEXF_PICMIP | TEXF_COMPRESS
						, MATERIALFLAG_WALL
					);
					break;
				}
	...
	else
	{
		...
		Mod_LoadTextureFromQ3Shader(loadmodel->mempool, loadmodel->model_name, skin, stripbuf, q_tx_warn_missing_true, q_tx_fallback_notexture_true, q_tx_do_external_true, (r_mipskins.integer ? TEXF_MIPMAP : 0) | TEXF_ALPHA | TEXF_PICMIP | TEXF_COMPRESS, MATERIALFLAG_WALL);
	}
}
extern cvar_t r_nolerp_list;
extern cvar_t r_lavaball_hack;
...
{
	int my_aliasflags, j, version, totalskins, skinwidth, skinheight, groupframes, groupskins, numverts;
	...
	if (version != ALIAS_VERSION)
		Host_Error_Line ("%s has wrong version number (%d should be %d)",
	...
	loadmodel->synctype = (int)/*(synctype_t)*/LittleLong (pinmodel->synctype);
	BOUNDI((int)loadmodel->synctype,0,2);
	
	my_aliasflags = LittleLong (pinmodel->aliasflags);
//#pragma message ("Baker: This is not quite the right way disable PRYDON CURSOR with model load")

	if (developer_loading.integer) {
		Con_PrintLinef ("MDL %s has flags %d", loadmodel->model_name, my_aliasflags);
	}

#if 1 // Baker: Evil Q15 hack
	if (Have_Flag (my_aliasflags, MF_FENCE)) {
		if (String_Ends_With (loadmodel->model_name, "g_shot.mdl") 
			|| String_Ends_With (loadmodel->model_name, "v_shot2.mdl")) {
			Flag_Remove_From (my_aliasflags, MF_FENCE);
		}
		// HACKAGE 
	}
	// Baker: Another evil hack ...
	qbool add_ef_flame = false;
	if (r_lavaball_hack.integer && Have_Flag (my_aliasflags, MF_ROCKET_1)) {
		if (String_Ends_With (loadmodel->model_name, "lavaball.mdl")) {
			add_ef_flame = true;
			Flag_Remove_From (my_aliasflags, MF_ROCKET_1);
			//Flag_Add_To (my_aliasflags, EF_STARDUST);
			//EF_SELECTABLE
		}
		// HACKAGE 
	}
	...
	if (gamemode != GAME_PRYDON) {
		if (Have_Flag (my_aliasflags /*pinmodel->flags*/, MF_FENCE) ) {
			is_fence = true;
			Flag_Remove_From (my_aliasflags, MF_FENCE);
	...
	loadmodel->mf_effects = ((my_aliasflags & 255) << 24) | (my_aliasflags & 0x00FFFF00);
	// EF_ROCKET  16m
	// MF_ROCKET_1

	if (add_ef_flame)
		Flag_Add_To (loadmodel->mf_effects, EF_FLAME);
	...
	totalskins = 0;
	for (int i = 0; i < loadmodel->numskins; i ++) {
...
// load the frames
	...
	while (s_lines_za) {
		...
		const msurface_t *surface2 = loadmodel->data_surfaces;

		// Parse the tags
		int isok = true;
		for (int tagnum = 0; tagnum < numtags_here; tagnum ++) {
			char *sname_za = String_Instance_Base1_ZAlloc (s_tags_za, ',', tagnum*2 +1 + 0, NULL);
			char *snumb_za = String_Instance_Base1_ZAlloc (s_tags_za, ',', tagnum*2 +1 + 1, NULL);
			int d = atoi(snumb_za);
			//	// mdl tag file indicating triangles to tag
			//	tagbackpack,184,taghead,100,taghand,184,tagbackpack2,273
			//triangleindex < surface->num_triangles
			if (in_range(0, d, surface2->num_triangles - 1) == false) {
				Con_PrintLinef ("Tag triangle index %d is out of range 0 to surface2->num_triangles -1 which is %d",
					d, surface2->num_triangles - 1);
				isok = false;
			}

			c_strlcpy (mdl_tagnames[tagnum], sname_za);
			mdl_trianglenum[tagnum] = d;
			Mem_FreeNull_ (sname_za);
			Mem_FreeNull_ (snumb_za);
		}
		...
		for (int posenum = 0; posenum < loadmodel->num_tagframes /** loadmodel->num_tags*/; posenum ++) {
			...
			for (int tagnum = 0; tagnum < loadmodel->num_tags; tagnum ++) {
				...
				int triangleindex;
				const int *etri;
				triangleindex = 0, etri = loadmodel->surfmesh.data_element3i + surface2->num_firsttriangle * 3;
				...
				//	RELATED_ (Matrix4x4_CreateFromQuakeEntity)
				//} 
				//
				//else {
...
int Mod_IDP2_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	*hash, **md2verthash, *md2verthashdata;
	...
	if (LittleLong(pinmodel->num_tris) < 1 || LittleLong(pinmodel->num_tris) > 65536)
		Host_Error_Line ("%s has invalid number of triangles: %d", loadmodel->model_name, LittleLong(pinmodel->num_tris));
	if (LittleLong(pinmodel->num_xyz) < 1 || LittleLong(pinmodel->num_xyz) > 65536)
		Host_Error_Line ("%s has invalid number of vertices: %d", loadmodel->model_name, LittleLong(pinmodel->num_xyz));
	if (LittleLong(pinmodel->num_frames) < 1 || LittleLong(pinmodel->num_frames) > 65536)
		Host_Error_Line ("%s has invalid number of frames: %d", loadmodel->model_name, LittleLong(pinmodel->num_frames));
	if (LittleLong(pinmodel->num_skins) < 0 || LittleLong(pinmodel->num_skins) > 256)
		Host_Error_Line ("%s has invalid number of skins: %d", loadmodel->model_name, LittleLong(pinmodel->num_skins));
	...
	if (LittleLong(pinmodel->num_skins) >= 1 && (LittleLong(pinmodel->ofs_skins) <= 0 || LittleLong(pinmodel->ofs_skins) >= end))
		Host_Error_Line ("%s is not a valid model", loadmodel->model_name);
	...
	else if (loadmodel->numskins)
	{
		...
		for (i = 0;i < loadmodel->numskins;i++, inskin += MD2_SKINNAME)
			Mod_LoadTextureFromQ3Shader(loadmodel->mempool, loadmodel->model_name, loadmodel->data_textures + i * loadmodel->num_surfaces, inskin, q_tx_warn_missing_true, q_tx_fallback_notexture_true, q_tx_do_external_true, (r_mipskins.integer ? TEXF_MIPMAP : 0) | TEXF_ALPHA | TEXF_PICMIP | TEXF_COMPRESS, MATERIALFLAG_WALL);
	}
	else
	{
		...
		loadmodel->num_texturesperskin = loadmodel->num_surfaces;
		loadmodel->data_textures = (texture_t *)Mem_Alloc(loadmodel->mempool, loadmodel->num_surfaces * loadmodel->numskins * sizeof(texture_t));
		Mod_LoadCustomMaterial(loadmodel->mempool, loadmodel->data_textures, loadmodel->model_name, SUPERCONTENTS_SOLID, MATERIALFLAG_WALL, R_SkinFrame_LoadMissing());
	}
...
int Mod_IDP3_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (memcmp(pinmodel->identifier, "IDP3", 4))
		Host_Error_Line ("%s is not a MD3 (IDP3) file", loadmodel->model_name);
	version = LittleLong (pinmodel->version);
	...
	if (developer_loading.integer) {
		Con_PrintLinef ("MD3 %s has flags %d", loadmodel->model_name, i);
	}

	loadmodel->mf_effects = ((i & 255) << 24) | (i & 0x00FFFF00);
	...
	for (i = 0, pinmesh = (md3mesh_t *)((unsigned char *)pinmodel + LittleLong(pinmodel->lump_meshes));i < loadmodel->num_surfaces;i++, pinmesh = (md3mesh_t *)((unsigned char *)pinmesh + LittleLong(pinmesh->lump_end)))
	{
		if (memcmp(pinmesh->identifier, "IDP3", 4))
			Host_Error_Line ("Mod_IDP3_Load: invalid mesh identifier (not IDP3)");
		if (LittleLong(pinmesh->num_frames) != loadmodel->numframes)
			Host_Error_Line ("Mod_IDP3_Load: mesh numframes differs from header");
	...
	for (i = 0, pinmesh = (md3mesh_t *)((unsigned char *)pinmodel + LittleLong(pinmodel->lump_meshes));i < loadmodel->num_surfaces;i++, pinmesh = (md3mesh_t *)((unsigned char *)pinmesh + LittleLong(pinmesh->lump_end)))
	{
		...
		if (developer_loading.integer) {
			Con_PrintLinef (CON_BRONZE "%4d: %s MD3 Mesh: %s", j, loadmodel->model_name, pinmesh->name);
		}
...
int Mod_ZYMOTICMODEL_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (memcmp(pinmodel->id, "ZYMOTICMODEL", 12))
		Host_Error_Line ("Mod_ZYMOTICMODEL_Load: %s is not a zymotic model", loadmodel->model_name);
	if (BigLong(pinmodel->type) != 1)
		Host_Error_Line ("Mod_ZYMOTICMODEL_Load: only type 1 (skeletal pose) models are currently supported (name = %s)", loadmodel->model_name);
	...
	for (i = 0;i < pheader->numscenes;i++)
	{
		...
		if ((unsigned int) loadmodel->animscenes[i].firstframe >= (unsigned int) numposes)
			Host_Error_Line ("%s scene->firstframe (%d) >= numposes (%d)", loadmodel->model_name, loadmodel->animscenes[i].firstframe, numposes);
		if ((unsigned int) loadmodel->animscenes[i].firstframe + (unsigned int) loadmodel->animscenes[i].framecount > (unsigned int) numposes)
			Host_Error_Line ("%s scene->firstframe (%d) + framecount (%d) >= numposes (%d)", loadmodel->model_name, loadmodel->animscenes[i].firstframe, loadmodel->animscenes[i].framecount, numposes);
		if (loadmodel->animscenes[i].framerate < 0)
			Host_Error_Line ("%s scene->framerate (%f) < 0", loadmodel->model_name, loadmodel->animscenes[i].framerate);
		scene++;
	}
	...
	for (i = 0;i < pheader->numbones;i++)
	{
		...
		if (loadmodel->data_bones[i].parent >= i)
			Host_Error_Line ("%s bone[%d].parent >= %d", loadmodel->model_name, i, i);
	}
	...
	for (i = 0;i < pheader->numverts;i++)
	{
		...
		if (vertbonecounts[i] != 1)
			Host_Error_Line ("%s bonecount[%d] != 1 (vertex weight support is impossible in this format)", loadmodel->model_name, i);
	}
	...
	for (i = 0;i < numposes;i++)
	{
		...
		for (j = 0;j < loadmodel->num_bones;j++)
		{
			...
			//	Con_Printf ("%s: bone %d = %f %f %f %f : %f %f %f %f : %f %f %f %f : scale = %f\n", loadmodel->name, j, pose[0], pose[1], pose[2], pose[3], pose[4], pose[5], pose[6], pose[7], pose[8], pose[9], pose[10], pose[11], VectorLength(pose));
	...
	if (pheader->lump_render.length != count)
		Host_Error_Line ("%s renderlist is wrong size (%d bytes, should be %d bytes)", loadmodel->model_name, pheader->lump_render.length, count);
	...
	for (i = 0;i < loadmodel->num_surfaces;i++)
	{
		...
		if (renderlist >= renderlistend)
			Host_Error_Line ("%s corrupt renderlist (wrong size)", loadmodel->model_name);
		...
		for (j = 0;j < surface->num_triangles * 3;j++)
		{
			if ((unsigned int)outelements[j] >= (unsigned int)meshvertices)
				Host_Error_Line ("%s corrupt renderlist (out of bounds index)", loadmodel->model_name);
...
int Mod_DARKPLACESMODEL_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (memcmp(pheader->id, "DARKPLACESMODEL\0", 16))
		Host_Error_Line ("Mod_DARKPLACESMODEL_Load: %s is not a darkplaces model", loadmodel->model_name);
	if (BigLong(pheader->type) != 2)
		Host_Error_Line ("Mod_DARKPLACESMODEL_Load: only type 2 (hierarchical skeletal pose) models are currently supported (name = %s)", loadmodel->model_name);
...
int Mod_PSKMODEL_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (strcmp(pchunk->id, "ACTRHEAD"))
		Host_Error_Line ("Mod_PSKMODEL_Load: %s is not an Unreal Engine ActorX (.psk + .psa) model", loadmodel->model_name);
	...
	while (buffer < bufferend)
	{
		...
		else if (String_Match(pchunk->id, "PNTS0000"))
		{
			...
			if (recordsize != sizeof(*p))
				Host_Error_Line ("%s: %s has unsupported recordsize", loadmodel->model_name, pchunk->id);
	...
	while (animbuffer < animbufferend)
	{
		...
		else if (String_Match(pchunk->id, "BONENAMES"))
		{
			pskboneinfo_t *p;
			if (recordsize != sizeof(*p))
				Host_Error_Line ("%s: %s has unsupported recordsize", animname, pchunk->id);
			// byteswap in place and keep the pointer
			...
			if (numanimbones != numbones)
				Host_Error_Line ("%s: this loader only supports animations with the same bones as the mesh", loadmodel->model_name);
			for (index = 0, p = (pskboneinfo_t *)animbuffer;index < numrecords;index++, p++) {
				...
				{
					Con_Printf ("%s: bone->parent %d >= numanimbones %d\n", animname, p->parent, numanimbones);
					p->parent = 0;
				}
				// check that bones are the same as in the base
				if (strcmp(p->name, bones[index].name) || p->parent != bones[index].parent)
					Host_Error_Line ("%s: this loader only supports animations with the same bones as the mesh", animname);
			}
			animbuffer = p;
		}
	...
	if (!numpnts || !pnts || !numvtxw || !vtxw || !numfaces || !faces || !nummatts || !matts || !numbones || !bones || !numrawweights || !rawweights)
		Host_Error_Line ("%s: missing required chunks", loadmodel->model_name);
	...
	if (numanims)
	{
		...
		if (numanimkeys != numbones * loadmodel->numframes)
			Host_Error_Line ("%s: %s has incorrect number of animation keys", animname, pchunk->id);
	}
	else
	...
	for (index = 0;index < numbones;index++)
	{
		...
		if (loadmodel->data_bones[index].parent >= index)
			Host_Error_Line ("%s bone[%d].parent >= %d", loadmodel->model_name, index, index);
	}
...
int Mod_INTERQUAKEMODEL_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (pbase + sizeof(iqmheader_t) > pend)
		Host_Error_Line ("Mod_INTERQUAKEMODEL_Load: %s is not an Inter-Quake Model %d", loadmodel->model_name, (int)(pend - pbase));
	...
	if (memcmp(header.id, "INTERQUAKEMODEL", 16))
		Host_Error_Line ("Mod_INTERQUAKEMODEL_Load: %s is not an Inter-Quake Model", loadmodel->model_name);
	if (LittleLong(header.version) != 1 && LittleLong(header.version) != 2)
		Host_Error_Line ("Mod_INTERQUAKEMODEL_Load: only version 1 and 2 models are currently supported (name = %s)", loadmodel->model_name);
