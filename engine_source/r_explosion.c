	...
	for (i = 0, e = explosion;i < MAX_EXPLOSIONS_64;i++, e++)
	{
		if (!e->alpha)
		{
			...
			for (j = 0;j < EXPLOSIONVERTS;j++)
			{
				...
				if (e->clipping)
				{
					trace = CL_TraceLine(e->origin, e->vert[j], MOVE_NOMONSTERS_1, NULL, SUPERCONTENTS_SOLID, 0, 0, collision_extendmovelength.value, true, false, NULL, false, false);
...
static void R_MoveExplosion(explosion_t *e)
{
	...
	for (i = 0;i < EXPLOSIONVERTS;i++)
	{
		if (e->vertvel[i][0] || e->vertvel[i][1] || e->vertvel[i][2])
		{
			...
			if (e->clipping)
			{
				trace = CL_TraceLine(e->vert[i], end, MOVE_NOMONSTERS_1, NULL, SUPERCONTENTS_SOLID, 0, 0, collision_extendmovelength.value, true, false, NULL, false, false);
				if (trace.fraction < 1)
				{
