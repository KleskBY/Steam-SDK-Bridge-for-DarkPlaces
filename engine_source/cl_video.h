...
extern struct cvar_s cl_video_frame_modulo;
...
typedef struct clvideo_s
{
	...
	RELATED_ (open LinkVideoTexture R_MakeTextureDynamic)
	RELATED_ (free UnlinkVideoTexture /*this does not destroy the cachepic_t*/)
	RELATED_ (video->decodeframe -> dpvsimpledecode_video /*this does not destroy the cachepic_t*/)
	RELATED_ (R_MarkDirtyTexture clvideo_t)
	RELATED_ (R_UploadFullTexture R_UploadPartialTexture )
	RELATED_ (skinframe_t -> base r_texture )
	RELATED_ (DrawQ_SuperPic_Video Mod_Mesh_GetTexture)
...
#define VIDEO_NOT_PLAYING_0				0
#define VIDEO_PLAYING_PLAYVIDEO_1		1
#define VIDEO_PLAYING_MENU_2			2
