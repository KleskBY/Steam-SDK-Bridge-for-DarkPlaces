...
// LadyHavoc: changed this to NOT use a return statement, so that it can be used in
// functions that must return a value
void VM_Warningf (prvm_prog_t *prog, const char *fmt, ...)
{
	va_list argptr;
	...
	if (prvm_backtraceforwarnings.integer && recursive != host.realtime) // NOTE: this compares to the time, just in case if PRVM_PrintState causes a Host_Error_Line and keeps recursive set
	{
...
// Baker: Adds a newline.  Does not print to a fixed buffer, varadic expansion.
// Baker: Notice it does a Con_PrintLinef instead of a Con_Printf .. that's how.
void VM_WarningLinef (prvm_prog_t *prog, const char *fmt, ...)
{
	VA_EXPAND_ALLOC (text, text_slen, bufsiz, fmt);
	static double recursive = -1;

	Con_PrintLinef (CON_WARN "%s VM warning: %s", prog->name, text);
...
//void VM_CheckNullString(prvm_prog_t *prog, const char *s) // Baker: Jan
//{
//	if (!s[0])
//		prog->error_cmd("%s: Null string", prog->name);
//}
...
void VM_GenerateFrameGroupBlend(prvm_prog_t *prog, framegroupblend_t *framegroupblend, const prvm_edict_t *ed)
{
	...
	framegroupblend[0].fb_frame = (int) PRVM_gameedictfloat(ed, frame     );
	framegroupblend[1].fb_frame = (int) PRVM_gameedictfloat(ed, frame2    );
	framegroupblend[2].fb_frame = (int) PRVM_gameedictfloat(ed, frame3    );
	framegroupblend[3].fb_frame = (int) PRVM_gameedictfloat(ed, frame4    );
...
//            this way
void VM_FrameBlendFromFrameGroupBlend(frameblend_t *frameblend, const framegroupblend_t *framegroupblend,
const model_t *model, double curtime)
{
	...
	memset(blend, 0, MAX_FRAMEBLENDS_8 * sizeof(*blend));
	...
	for (k = 0, g = framegroupblend;k < MAX_FRAMEGROUPBLENDS_4;k++, g++)
	{
		f = g->fb_frame;
		if ((unsigned int)f >= (unsigned int)numframes)
		{
			if (developer_extra.integer)
				Con_DPrintLinef ("VM_FrameBlendFromFrameGroupBlend: no such frame %d in model %s", f, model->model_name);
			f = 0;
		}
		d = lerp = g->lerp;
		if (lerp <= 0)
		...
		if (model->animscenes)
		{
			...
			if (scene->framecount > 1)
			{
				...
				if (d > 0)
				{
					...
					{
						if (blend[i].rlerp <= 0 || blend[i].subframe == sub2)
						{
		...
		if (d > 0)
		{
			for (i = 0;i < MAX_FRAMEBLENDS_8;i++)
			{
				if (blend[i].rlerp <= 0 || blend[i].subframe == f)
				{
					blend[i].subframe = f;
					blend[i].rlerp += d;
					break;
...
static qbool checkextension(prvm_prog_t *prog, const char *name)
{
	...
	{
		if (String_Match_Caseless(*e, name))
		{
#ifdef USEODE
			// special sheck for ODE
			if (String_Starts_With_Caseless_PRE(name, "DP_PHYSICS_ODE"))
			{
			...
			if (String_Match_Caseless("DP_CRYPTO", name))
				return Crypto_Available();
			if (String_Match_Caseless("DP_QC_DIGEST_SHA256", name))
				return Crypto_Available();

			// special shreck for libcurl
			if (String_Match_Caseless("DP_QC_URI_GET", name) || String_Match_Caseless("DP_QC_URI_POST", name))
...
void VM_bprint(prvm_prog_t *prog)
{
	...
	if (!sv.active)
	{
		VM_WarningLinef (prog, "VM_bprint: game is not server(%s) !", prog->name);
		return;
	}
...
void VM_sprint(prvm_prog_t *prog)
{
	...
	{
		VM_WarningLinef (prog, "VM_sprint: %s: invalid client or server is not active !", prog->name);
		return;
	}
...
void VM_localsound(prvm_prog_t *prog)
{
	...
	if (prog->argc == 3)
	{
		...
		if (!S_LocalSoundEx(s, chan, vol))
		{
			...
			VM_WarningLinef (prog, "VM_localsound: Failed to play %s for %s !", s, prog->name);
			return;
		}
	}
...
void VM_etos(prvm_prog_t *prog)
{
	...
	dpsnprintf (s, sizeof(s), "entity %d", PRVM_G_EDICTNUM(OFS_PARM0));
	PRVM_G_INT(OFS_RETURN) = PRVM_SetTempString(prog, s);
}
...
========================
*/
// Baker: float 	etof(entity ent) = #79;
// Baker: float   validstring(string str) #81 - Not called by Xonotic or Nexuiz
// FTEQW: Returns true if str isn't null. In case 'if [not](str)' was configured to test for empty instead of null.
...
void VM_strftime(prvm_prog_t *prog)
{
	...
	struct tm *tm;
#endif
	char fmt[VM_STRINGTEMP_LENGTH_16384];
	char result[VM_STRINGTEMP_LENGTH_16384];
...
void VM_remove(prvm_prog_t *prog)
{
	...
	if ( PRVM_NUM_FOR_EDICT(ed) <= prog->reserved_edicts )
	{
		if (developer.integer > 0)
			VM_WarningLinef (prog, "VM_remove: tried to remove the null entity or a reserved entity!" );
	}
	else if ( ed->free )
	{
		if (developer.integer > 0)
			VM_WarningLinef (prog, "VM_remove: tried to remove an already freed entity!" );
	}
	else
		PRVM_ED_Free (prog, ed);
}
...
void VM_precache_sound(prvm_prog_t *prog)
{
	...
	{
		VM_WarningLinef (prog, "VM_precache_sound: Failed to load %s for %s", s, prog->name);
		return;
	}
}
...
changelevel(string map, optional string startspot)
==============
*/
RELATED_ (SV_Changelevel_f SV_Changelevel2_f  VM_strcat SV_Restart_f VM_changelevel SV_SpawnServer SV_SaveSpawnparms) // Q2X
void VM_changelevel (prvm_prog_t *prog) // Q2X
{
	char vabuf[1024];
	// Baker: arg
	//VM_SAFEPARMCOUNT(1, VM_changelevel);
	//VM_SAFEPARMCOUNT(2, VM_changelevel);
	VM_SAFEPARMCOUNTRANGE(1, 2, VM_changelevel);

	if (!sv.active) {
		VM_WarningLinef (prog, "VM_changelevel: game is not server (%s)", prog->name);
		return;
	}
...
// make sure we don't issue two changelevels
	...
	// enable_intermap 2 - force changelevel2 always
	int have_intermap_2 = false;
	...
	if (v_enable_intermap_offset >= 0) {
		...
		if (val >= 2)
			have_intermap_2 = true;
	} // if

	if (prog->argc == 2) {
		// Intermap - eventually we will need session id
		// Baker: changelevel2 will do the .siv file
		RELATED_ (SV_Savegame_f SV_Savegame_to SV_Changelevel2_f )
		char string[VM_STRINGTEMP_LENGTH_16384];
		VM_VarString (prog, 1, string, sizeof(string));
		const char *s = va(vabuf, sizeof(vabuf), "changelevel2 %s %s", PRVM_G_STRING(OFS_PARM0), PRVM_G_STRING(OFS_PARM1) ); // Q2X area
		Cbuf_AddTextLine (cmd_local, s);
		return;
	}

	if (have_intermap_2) {
		const char *s = va(vabuf, sizeof(vabuf), "changelevel2 %s NULL", PRVM_G_STRING(OFS_PARM0));
		Cbuf_AddTextLine (cmd_local, s);
		return;
	}

	RELATED_ (SV_Changelevel_f)
	const char *s = va(vabuf, sizeof(vabuf), "changelevel %s", PRVM_G_STRING(OFS_PARM0) ); // Q2X area
	Cbuf_AddTextLine (cmd_local, s);
}
...
=========
*/
// Baker: What are flags? CF_SERVER and CF_ARCHIVE and such.
// Baker: We could add description as #4.  DarkPlaces will reject a 4 args command though ...
void VM_registercvar(prvm_prog_t *prog)
{
	...
	if (flags > CF_MAXFLAGSVAL) {
		VM_WarningLinef (prog, "VM_registercvar rejected: %s has flags of %d, exceeds MAXFLAGS %d", name, flags, (int)CF_MAXFLAGSVAL);
		return;
	}
...
// check for overlap with a command
	if (Cmd_Exists(cmd_local, name)) {
		VM_WarningLinef (prog, "VM_registercvar rejected: %s is a command", name);
		return;
	}

	// Baker: This is "register"
...
{
	int i;
	for (i = 0;i < PRVM_MAX_OPENFILES_256;i++)
...
{
	if (index < 0 || index >= PRVM_MAX_OPENFILES_256)
	{
		Con_Printf ("VM_GetFileHandle: invalid file handle %d used in %s\n", index, prog->name);
		return NULL;
	}
	if (prog->openfiles[index] == NULL)
	{
		Con_Printf ("VM_GetFileHandle: no such file handle %d (or file has been closed) in %s\n", index, prog->name);
		return NULL;
	}
...
void VM_fopen(prvm_prog_t *prog)
{
	...
	for (filenum = 0;filenum < PRVM_MAX_OPENFILES_256;filenum++)
		if (prog->openfiles[filenum] == NULL)
			break;
	if (filenum >= PRVM_MAX_OPENFILES_256)
	{
		PRVM_G_FLOAT(OFS_RETURN) = -2;
		VM_WarningLinef (prog, "VM_fopen: %s ran out of file handles (%d)", prog->name, PRVM_MAX_OPENFILES_256);
		return;
	}
	...
	case 0: // FILE_READ
		modestring = "rb";
		prog->openfiles[filenum] = FS_OpenVirtualFile(va(vabuf, sizeof(vabuf), "data/%s", filename), fs_quiet_FALSE);
		if (prog->openfiles[filenum] == NULL)
			prog->openfiles[filenum] = FS_OpenVirtualFile(va(vabuf, sizeof(vabuf), "%s", filename), fs_quiet_FALSE);
		break;
	case 1: // FILE_APPEND
		modestring = "a";
		prog->openfiles[filenum] = FS_OpenRealFile(va(vabuf, sizeof(vabuf), "data/%s", filename), modestring, fs_quiet_FALSE);
		break;
	case 2: // FILE_WRITE
		modestring = "w";
		prog->openfiles[filenum] = FS_OpenRealFile(va(vabuf, sizeof(vabuf), "data/%s", filename), modestring, fs_quiet_FALSE); // Baker: Sub function, ignore
		break;
	default:
		PRVM_G_FLOAT(OFS_RETURN) = -3;
		VM_WarningLinef (prog, "VM_fopen: %s: no such mode %d (valid: 0 = read, 1 = append, 2 = write)", prog->name, mode);
		return;
	}
	...
	if (prog->openfiles[filenum] == NULL)
	{
		PRVM_G_FLOAT(OFS_RETURN) = -1;
		if (developer_extra.integer)
			VM_WarningLinef (prog, "VM_fopen: %s: %s mode %s failed", prog->name, filename, modestring);
	}
	else
	{
		PRVM_G_FLOAT(OFS_RETURN) = filenum;
		if (developer_extra.integer)
			Con_DPrintLinef ("VM_fopen: %s: %s mode %s opened as #%d", prog->name, filename, modestring, filenum);
...
void VM_fclose(prvm_prog_t *prog)
{
	...
	if (filenum < 0 || filenum >= PRVM_MAX_OPENFILES_256)
	{
		VM_WarningLinef (prog, "VM_fclose: invalid file handle %d used in %s", filenum, prog->name);
		return;
	}
	if (prog->openfiles[filenum] == NULL)
	{
		VM_WarningLinef (prog, "VM_fclose: no such file handle %d (or file has been closed) in %s", filenum, prog->name);
		return;
	}
	...
	if (developer_extra.integer)
		Con_DPrintf ("VM_fclose: %s: #%d closed\n", prog->name, filenum);
}
...
void VM_fgets(prvm_prog_t *prog)
{
	...
	if (filenum < 0 || filenum >= PRVM_MAX_OPENFILES_256)
	{
		VM_WarningLinef (prog, "VM_fgets: invalid file handle %d used in %s", filenum, prog->name);
		return;
	}
	if (prog->openfiles[filenum] == NULL)
	{
		VM_WarningLinef (prog, "VM_fgets: no such file handle %d (or file has been closed) in %s", filenum, prog->name);
		return;
	}
	end = 0;
	for (;;)
	{
		...
		if (c == '\r' || c == '\n' || c < 0)
			break;
		if (end < VM_STRINGTEMP_LENGTH_16384 - 1)
			string[end++] = c;
	}
	string[end] = 0;
	// remove \n following \r
...
void VM_fputs(prvm_prog_t *prog)
{
	...
	if (filenum < 0 || filenum >= PRVM_MAX_OPENFILES_256)
	{
		VM_WarningLinef (prog, "VM_fputs: invalid file handle %d used in %s", filenum, prog->name);
		return;
	}
	if (prog->openfiles[filenum] == NULL)
	{
		VM_WarningLinef (prog, "VM_fputs: no such file handle %d (or file has been closed) in %s", filenum, prog->name);
		return;
	}
	VM_VarString(prog, 1, string, sizeof(string));
...
void VM_writetofile(prvm_prog_t *prog)
{
	...
	if ( !file )
	{
		VM_WarningLinef (prog, "VM_writetofile: invalid or closed file handle");
		return;
	}
	...
	if (ent->free)
	{
		VM_WarningLinef (prog, "VM_writetofile: %s: entity %d is free !", prog->name, PRVM_NUM_FOR_EDICT(ent));
		return;
	}
...
void VM_entityfieldname(prvm_prog_t *prog)
{
	...
	{
		VM_WarningLinef (prog, "VM_entityfieldname: %s: field index out of bounds", prog->name);
...
void VM_entityfieldtype(prvm_prog_t *prog)
{
	...
	if (i < 0 || i >= prog->numfielddefs)
	{
		VM_WarningLinef (prog, "VM_entityfieldtype: %s: field index out of bounds", prog->name);
...
void VM_getentityfieldstring(prvm_prog_t *prog)
{
	...
	if (i < 0 || i >= prog->numfielddefs)
	{
        VM_WarningLinef (prog, "VM_entityfielddata: %s: field index out of bounds", prog->name);
		PRVM_G_INT(OFS_RETURN) = PRVM_SetTempString(prog, "");
		return;
	}
	...
	{
		PRVM_G_INT(OFS_RETURN) = PRVM_SetTempString(prog, "");
		VM_WarningLinef (prog, "VM_entityfielddata: %s: entity %d is free !", prog->name, PRVM_NUM_FOR_EDICT(ent));
		return;
	}
...
{
	char szNewString[VM_STRINGTEMP_LENGTH_16384];
	const char *szString;
...
{
	char s[VM_STRINGTEMP_LENGTH_16384];
...
void VM_stov(prvm_prog_t *prog)
{
	...
	VM_VarString(prog, 0, string, sizeof(string));
	Math_atov_vm(string, PRVM_G_VECTOR(OFS_RETURN));
}
...
static int tokens[VM_STRINGTEMP_LENGTH_16384 / 2];
static int tokens_startpos[VM_STRINGTEMP_LENGTH_16384 / 2];
static int tokens_endpos[VM_STRINGTEMP_LENGTH_16384 / 2];
static char tokenize_string[VM_STRINGTEMP_LENGTH_16384];
...
void VM_tokenizebyseparator (prvm_prog_t *prog)
{
	...
	while (num_tokens < (int)(sizeof(tokens)/sizeof(tokens[0])))
	{
		...
		if (j >= (int)sizeof(tokentext))
			break;
#if 1 // Baker: August 10 2025 - bones fix PRVM: fix incorrect tempstring length in VM_tokenizebyseparator()
		//OLD:
		tokentext[j++] = 0;
		tokens[num_tokens++] = PRVM_SetTempString(prog, token);
#else
		// Baker: They must have changed PRVM_SetTempString to take extra arg .. do this later.
		tokentext[j] = '\0';
	...
	else
	{
		...
		switch(timer_index)
		{
			...
			case 4: // GETTIME_CDTRACK
				...
				break;
			default:
				VM_WarningLinef (prog, "VM_gettime: %s: unsupported timer specified, returning realtime", prog->name);
				PRVM_G_FLOAT(OFS_RETURN) = host.realtime;
				break;
		}
	}
...
Baker: FTE ... Returns the current playback time of the sample on the given entity's channel. Beware CHAN_AUTO (in csqc, channels are not limited by network protocol).
...
void VM_getsoundtime (prvm_prog_t *prog)
{
	...
	{
		VM_WarningLinef (prog, "VM_getsoundtime: %s: not supported on this progs", prog->name);
		PRVM_G_FLOAT(OFS_RETURN) = -1;
		return;
	}
	...
	if (!IS_CHAN(entchannel))
		VM_WarningLinef (prog, "VM_getsoundtime: %s: bad channel %d", prog->name, entchannel);
...
========================
*/
// Baker: #613 void parseentitydata(entity ent, string data)
// Baker: I don't know what this does and it is not used in Xonotic or Nexuiz in QuakeC source ... ?
// Baker: I think it takes a string and force feeds the entity fields from the string.
void VM_parseentitydata(prvm_prog_t *prog)
{
	...
	if (ent->free)
		prog->error_cmd("VM_parseentitydata: %s: Can only set already spawned entities (entity %d is free)!", prog->name, PRVM_NUM_FOR_EDICT(ent));
	...
	PRVM_ED_ParseEdict (prog, data, ent, q_is_saveload_false); // ITK #2
}
...
void VM_loadfromfile(prvm_prog_t *prog)
{
	...
	{
		PRVM_G_FLOAT(OFS_RETURN) = -4;
		VM_WarningLinef (prog, "VM_loadfromfile: %s dangerous or non-portable filename " QUOTED_S " not allowed. (contains : or \\ or begins with .. or /)", prog->name, filename);
		return;
	}
...
void VM_modulo(prvm_prog_t *prog)
{
	...
	{
		VM_WarningLinef (prog, "Attempted modulo of %f by zero" NEWLINE, val);
		PRVM_G_FLOAT(OFS_RETURN) = 0;
	}
}
...
{
	int i;
	for (i = 0;i < PRVM_MAX_OPENSEARCHES_128;i++)
...
static void VM_Zircon_API_Shutdown_Reset(prvm_prog_t *prog)
{
	// clear inventory

	for (int j = 0; j < PRVM_MAX_OPEN_DOT_MAPS_8; j ++) {
		stringlistfreecontents (&prog->invent[j]);
	}
	// reset open .maps list
	for (int j = 0; j < PRVM_MAX_OPEN_DOT_MAPS_8; j ++) {
		if (prog->open_dot_maps[j]) {
			entitylistfreecontents (prog->open_dot_maps[j]);
			Mem_FreeNull_ (prog->open_dot_maps[j]);
		}
	} // for
#ifdef CONFIG_MENU
	VM_Reset_CSQC_Form_Clear_Reference ();
	...
	for (int j = 0; j < PRVM_MAX_OPEN_FORMS_8; j ++) {
		...
		if (prog->forms[j]) {
			...
			/*void *form1 =*/ CSQC_Form_Destroy_VoidP(prog->forms[j]);
			Con_PrintLinef ("Destroyed form %d", j);
			if (prog->forms[j])
				prog->forms[j] = NULL;
		}
		//if (prog->form_callbacks[j]) {
		//	prog->form_callbacks[j] = 0;
		//}
	} // for
#endif
}
...
void VM_search_begin(prvm_prog_t *prog)
{
	...
	for(handle = 0; handle < PRVM_MAX_OPENSEARCHES_128; handle++)
		if (!prog->opensearches[handle])
			break;

	if (handle >= PRVM_MAX_OPENSEARCHES_128) {
		PRVM_G_FLOAT(OFS_RETURN) = -2;
		VM_WarningLinef (prog, "VM_search_begin: %s ran out of search handles (%d)", prog->name,
			PRVM_MAX_OPENSEARCHES_128);
		return;
	}
...
void VM_search_end(prvm_prog_t *prog)
{
	...
	if (handle < 0 || handle >= PRVM_MAX_OPENSEARCHES_128)
	{
		VM_WarningLinef (prog, "VM_search_end: invalid handle %d used in %s", handle, prog->name);
		return;
	}
	if (prog->opensearches[handle] == NULL)
	{
		VM_WarningLinef (prog, "VM_search_end: no such handle %d in %s", handle, prog->name);
		return;
	}
...
void VM_search_getsize(prvm_prog_t *prog)
{
	...
	if (handle < 0 || handle >= PRVM_MAX_OPENSEARCHES_128)
	{
		VM_WarningLinef (prog, "VM_search_getsize: invalid handle %d used in %s", handle, prog->name);
		return;
	}
	if (prog->opensearches[handle] == NULL)
	{
		VM_WarningLinef (prog, "VM_search_getsize: no such handle %d in %s", handle, prog->name);
		return;
	}
...
void VM_search_getfilename(prvm_prog_t *prog)
{
	...
	if (handle < 0 || handle >= PRVM_MAX_OPENSEARCHES_128)
	{
		VM_WarningLinef (prog, "VM_search_getfilename: invalid handle %d used in %s", handle, prog->name);
		return;
	}
	if (prog->opensearches[handle] == NULL)
	{
		VM_WarningLinef (prog, "VM_search_getfilename: no such handle %d in %s", handle, prog->name);
		return;
	}
	if (filenum < 0 || filenum >= prog->opensearches[handle]->numfilenames)
	{
		VM_WarningLinef (prog, "VM_search_getfilename: invalid filenum %d in %s", filenum, prog->name);
		return;
	}
...
{
	const char *cmd;
	char ret[VM_STRINGTEMP_LENGTH_16384];
	...
	if (prog->argc == 2)
		bindmap = bound(-1, PRVM_G_FLOAT(OFS_PARM1), MAX_BINDMAPS_8-1);
	else
	...
	for(i = 0; i < FKFC_NUMKEYS; i++)
		strlcat(ret, va(vabuf, sizeof(vabuf), " \'%d\'", keys[i]), sizeof(ret));
...
void VM_setkeybind (prvm_prog_t *prog)
{
	...
	if (prog->argc == 3)
		bindmap = bound(-1, PRVM_G_FLOAT(OFS_PARM2), MAX_BINDMAPS_8-1);
	else
		bindmap = 0; // consistent to "bind"
...
{
	const char *instr, *in;
	char outstr[VM_STRINGTEMP_LENGTH_16384];
	size_t outpos;
...
void VM_buf_del (prvm_prog_t *prog)
{
	...
	{
		VM_WarningLinef (prog, "VM_buf_del: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
}
...
void VM_buf_getsize (prvm_prog_t *prog)
{
	...
	{
		PRVM_G_FLOAT(OFS_RETURN) = -1;
		VM_WarningLinef (prog, "VM_buf_getsize: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	else
...
void VM_buf_copy (prvm_prog_t *prog)
{
	...
	if (!srcstringbuffer)
	{
		VM_WarningLinef (prog, "VM_buf_copy: invalid source buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	i = (int)PRVM_G_FLOAT(OFS_PARM1);
	if (i == (int)PRVM_G_FLOAT(OFS_PARM0))
	{
		VM_WarningLinef (prog, "VM_buf_copy: source == destination (%d) in %s", i, prog->name);
		return;
	}
	dststringbuffer = (prvm_stringbuffer_t *)Mem_ExpandableArray_RecordAtIndex(&prog->stringbuffersarray, (int)PRVM_G_FLOAT(OFS_PARM0));
	if (!dststringbuffer)
	{
		VM_WarningLinef (prog, "VM_buf_copy: invalid destination buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM1), prog->name);
		return;
	}
...
void VM_buf_sort (prvm_prog_t *prog)
{
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_buf_sort: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	if (stringbuffer->num_strings <= 0)
	{
		// Baker: This is not an error.
#if 0
		VM_WarningLinef(prog, "VM_buf_sort: tried to sort empty buffer %d in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
#endif
		return;
	}
...
{
	prvm_stringbuffer_t *stringbuffer;
	char			k[VM_STRINGTEMP_LENGTH_16384];
	const char		*sep;
	int				i;
	size_t			l;
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_buf_implode: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
...
void VM_bufstr_get (prvm_prog_t *prog)
{
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_bufstr_get: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	strindex = (int)PRVM_G_FLOAT(OFS_PARM1);
	if (strindex < 0)
	{
		// VM_Warninglinef (prog, "VM_bufstr_get: invalid string index %d used in %s", strindex, prog->name);
		return;
	}
...
void VM_bufstr_set (prvm_prog_t *prog)
{
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_bufstr_set: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	strindex = (int)PRVM_G_FLOAT(OFS_PARM1);
	if (strindex < 0 || strindex >= 1000000) // huge number of strings
	{
		VM_WarningLinef (prog, "VM_bufstr_set: invalid string index %d used in %s", strindex, prog->name);
		return;
	}
...
void VM_bufstr_add (prvm_prog_t *prog)
{
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_bufstr_add: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	if (!PRVM_G_INT(OFS_PARM1)) // NULL string
	{
		VM_WarningLinef (prog, "VM_bufstr_add: can not add an empty string to buffer %d in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
...
void VM_bufstr_free (prvm_prog_t *prog)
{
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_bufstr_free: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	i = (int)PRVM_G_FLOAT(OFS_PARM1);
	if (i < 0)
	{
		VM_WarningLinef (prog, "VM_bufstr_free: invalid string index %d used in %s", i, prog->name);
		return;
	}
...
void VM_buf_loadfile(prvm_prog_t *prog)
{
	...
	// get file
	filename = PRVM_G_STRING(OFS_PARM0);
	file = FS_OpenVirtualFile(va(vabuf, sizeof(vabuf), "data/%s", filename), fs_quiet_FALSE);
	if (file == NULL)
		file = FS_OpenVirtualFile(va(vabuf, sizeof(vabuf), "%s", filename), fs_quiet_FALSE);
	if (file == NULL)
	{
		if (developer_extra.integer)
			VM_WarningLinef (prog, "VM_buf_loadfile: failed to open file %s in %s", filename, prog->name);
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_buf_loadfile: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM1), prog->name);
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
...
void VM_buf_writefile(prvm_prog_t *prog)
{
	...
	filenum = (int)PRVM_G_FLOAT(OFS_PARM0);
	if (filenum < 0 || filenum >= PRVM_MAX_OPENFILES_256) {
		VM_WarningLinef (prog, "VM_buf_writefile: invalid file handle %d used in %s", filenum, prog->name);
		return;
	}

	if (prog->openfiles[filenum] == NULL) {
		VM_WarningLinef (prog, "VM_buf_writefile: no such file handle %d (or file has been closed) in %s", filenum, prog->name);
		return;
	}
	...
	stringbuffer = (prvm_stringbuffer_t *)Mem_ExpandableArray_RecordAtIndex(&prog->stringbuffersarray, (int)PRVM_G_FLOAT(OFS_PARM1));
	if (!stringbuffer) {
		VM_WarningLinef (prog, "VM_buf_writefile: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM1), prog->name);
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
	...
	if (strindex < 0 || strindex >= stringbuffer->num_strings) {
		VM_WarningLinef (prog, "VM_buf_writefile: wrong start string index %d used in %s", strindex, prog->name);
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
	if (strnum < 0) {
		VM_WarningLinef (prog, "VM_buf_writefile: wrong strings count %d used in %s", strnum, prog->name);
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
...
void VM_bufstr_find(prvm_prog_t *prog)
{
	...
	if (!stringbuffer)
	{
		VM_WarningLinef (prog, "VM_bufstr_find: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
		return;
	}
	...
	{
		VM_WarningLinef (prog, "VM_bufstr_find: invalid match rule %d in %s", matchrule, prog->name);
		return;
	}
	if (matchrule)
	...
	{
		if (stringbuffer->strings[i] && match_rule(stringbuffer->strings[i], VM_STRINGTEMP_LENGTH_16384, match, matchlen, matchrule))
		{
			PRVM_G_FLOAT(OFS_RETURN) = i;
			break;
		}
...
void VM_matchpattern(prvm_prog_t *prog)
{
	...
	if (matchrule < 0 || matchrule > 5)
	{
		VM_WarningLinef (prog, "VM_matchpattern: invalid match rule %d in %s", matchrule, prog->name);
		return;
	}
	if (matchrule)
		match = PRVM_G_STRING(OFS_PARM1);
	...
	// match
	PRVM_G_FLOAT(OFS_RETURN) = match_rule(s, VM_STRINGTEMP_LENGTH_16384, match, l, matchrule);
}
...
void VM_changeyaw (prvm_prog_t *prog)
{
	...
	if (ent == prog->edicts)
	{
		VM_WarningLinef (prog, "changeyaw: can not modify world entity");
		return;
	}
	if (ent->free)
	{
		VM_WarningLinef (prog, "changeyaw: can not modify free entity");
		return;
	}
...
void VM_changepitch (prvm_prog_t *prog)
{
	...
	if (ent == prog->edicts)
	{
		VM_WarningLinef (prog, "changepitch: can not modify world entity");
		return;
	}
	if (ent->free)
	{
		VM_WarningLinef (prog, "changepitch: can not modify free entity");
		return;
	}
...
void VM_strconv (prvm_prog_t *prog)
{
	...
	unsigned char resbuf[VM_STRINGTEMP_LENGTH_16384];
...
{
	char src[VM_STRINGTEMP_LENGTH_16384];
	char destbuf[VM_STRINGTEMP_LENGTH_16384];
	int pad;
...
{
	const char *info, *key;
	char value[VM_STRINGTEMP_LENGTH_16384];
	char temp[VM_STRINGTEMP_LENGTH_16384];
	...
	strlcpy(temp, info, VM_STRINGTEMP_LENGTH_16384);

	InfoString_SetValue(temp, VM_STRINGTEMP_LENGTH_16384, key, value);
...
void VM_infoget (prvm_prog_t *prog)
{
	...
	InfoString_GetValue(info, key, value, VM_STRINGTEMP_LENGTH_16384);
...
void VM_digest_hex(prvm_prog_t *prog)
{
	...
	if (String_Match(digest, "MD4"))
	{
		outlen = 16;
		mdfour((unsigned char *) out, (unsigned char *) s, len);
	}
	else if (String_Match(digest, "SHA256") && Crypto_Available())
	{
		outlen = 32;
...
{
	CL_PurgeOwner	(VID_OWNER_MENU_1);

	VM_Search_Reset(prog);
	VM_Zircon_API_Shutdown_Reset(prog);
...
{
	char src[VM_STRINGTEMP_LENGTH_16384];
	char dest[VM_STRINGTEMP_LENGTH_16384];
	char *p, *q;
...
// returns 1 on success, and then calls the callback with the ID, 0 or the HTTP status code,
// and the received data in a string

// Baker: also called crypto_uri_postbuf
...
void VM_uri_get (prvm_prog_t *prog)
{
	...
	if (postseparator && posttype && *posttype)
	{
		...
		if (poststringbuffer >= 0)
		{
			...
			if (!stringbuffer)
			{
				VM_WarningLinef (prog, "uri_get: invalid buffer %d used in %s", (int)PRVM_G_FLOAT(OFS_PARM0), prog->name);
				return;
			}
			ltotal = 0;
			for(i = 0;i < stringbuffer->num_strings;i++)
...
void VM_CL_getextresponse (prvm_prog_t *prog)
{
	...
	else
	{
		...
		first = (cl_net_extresponse_last + NET_EXTRESPONSE_MAX_16 - cl_net_extresponse_count) % NET_EXTRESPONSE_MAX_16;
...
void VM_SV_getextresponse (prvm_prog_t *prog)
{
	...
	else
	{
		...
		first = (sv_net_extresponse_last + NET_EXTRESPONSE_MAX_16 - sv_net_extresponse_count) % NET_EXTRESPONSE_MAX_16;
...
void VM_nudgeoutofsolid(prvm_prog_t *prog)
{
	...
	if (ent == prog->edicts)
	{
		VM_WarningLinef (prog, "nudgeoutofsolid: can not modify world entity");
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
	if (ent->free)
	{
		VM_WarningLinef (prog, "nudgeoutofsolid: can not modify free entity");
		PRVM_G_FLOAT(OFS_RETURN) = 0;
		return;
	}
	...
	if (PRVM_G_FLOAT(OFS_RETURN) > 0)
	{
		if (prog == SVVM_prog)
			...
			Sys_Error ("PHYS_NudgeOutOfSolid: cannot be called from %s VM", prog->name);
	}
}
#endif
...
void VM_callfunction(prvm_prog_t *prog)
{
	...
	else if (func->first_statement < 0)
	{
		...
		if (builtinnumber < prog->numbuiltins && prog->builtins[builtinnumber])
			...
			prog->error_cmd("No such builtin #%d in %s; most likely cause: outdated engine build. Try updating!", builtinnumber, prog->name);
	}
		...
		switch(*s)
		{
			...
			case '%':
				...
				if (*s >= '0' && *s <= '9')
				{
					...
					if (!err)
					{
						VM_WarningLinef (prog, "VM_sprintf: invalid directive in %s: %s", prog->name, s0);
						goto finished;
					}
					if (*err == '$')
					{
...
void VM_getsurfacenumpoints(prvm_prog_t *prog)
{
	...
	int surface_index = (int)PRVM_G_FLOAT(OFS_PARM1);
	prvm_edict_t *surface_ed = PRVM_G_EDICT(OFS_PARM0);

	model_t		*model		= getmodel (prog, surface_ed);
	msurface_t	*surface	= getsurface (model, surface_index);
	...
	RELATED_ (SUPERCONTENTS_BODY)
	PRVM_gameglobalfloat(trace_dphitcontents) = surface->texture->supercontents;
	PRVM_gameglobalfloat(trace_dphitq3surfaceflags) = surface->texture->surfaceflags;
...
void VM_getsurfacepointattribute(prvm_prog_t *prog)
{
	...
	switch( attributetype ) {
		// float SPA_POSITION = 0;
#define SPA_POSITION_0				0
#define SPA_S_AXIS_1				1
#define SPA_T_AXIS_2				2
#define SPA_R_AXIS_3				3
#define SPA_TEXCOORDS0_4			4
#define SPA_LIGHTMAP0_TEXCOORDS_5	5
#define SPA_LIGHTMAP_COLOR_6		6
#define SPA_Q3SURFFLAGS_ZIRCON_7	7
...
//PF_getsurfacenumtriangles, // #??? float(entity e, float s) getsurfacenumtriangles = #628;
...
//PF_getsurfacetriangle,     // vector(entity e, float s, float n) getsurfacetriangle = #629;
...
void VM_physics_enable(prvm_prog_t *prog)
{
	...
	{
		if (developer.integer > 0)
			VM_WarningLinef (prog, "VM_physics_enable: null entity!");
		return;
	}
	// entity should have MOVETYPE_PHYSICS_32 already set, this can damage memory (making leaked allocation) so warn about this even if non-developer
	if (PRVM_serveredictfloat(ed, movetype) != MOVETYPE_PHYSICS_32)
	{
		VM_WarningLinef (prog, "VM_physics_enable: entity is not MOVETYPE_PHYSICS_32!");
		return;
	}
...
void VM_physics_addforce(prvm_prog_t *prog)
{
	...
	{
		if (developer.integer > 0)
			VM_WarningLinef (prog, "VM_physics_addforce: null entity!");
		return;
	}
	// entity should have MOVETYPE_PHYSICS_32 already set, this can damage memory (making leaked allocation) so warn about this even if non-developer
	if (PRVM_serveredictfloat(ed, movetype) != MOVETYPE_PHYSICS_32)
	{
		VM_WarningLinef (prog, "VM_physics_addforce: entity is not MOVETYPE_PHYSICS_32!");
		return;
	}
...
void VM_physics_addtorque(prvm_prog_t *prog)
{
	...
	{
		if (developer.integer > 0)
			VM_WarningLinef (prog, "VM_physics_addtorque: null entity!");
		return;
	}
	// entity should have MOVETYPE_PHYSICS_32 already set, this can damage memory (making leaked allocation) so warn about this even if non-developer
	if (PRVM_serveredictfloat(ed, movetype) != MOVETYPE_PHYSICS_32)
	{
		VM_WarningLinef (prog, "VM_physics_addtorque: entity is not MOVETYPE_PHYSICS_32!");
		return;
	}
...
// Baker: float res = crypto_uri_postbuf (gc_arg1, /*id*/ download_idx);

RELATED_ (VM_uri_get VM_bufstr_get)

// gridsize ... ?
// pieces how
// simplex ... first ... tunnels style
// Room .. Walk bits so north south is 5
// Room number ... what is a in room, is it stairs?
// Fountain?
// Mini-map?
// 1 N
// 2 E
...
// .. We are going to need a flood fill function or path find function.
// .. We must be able to visualize it on-screen so how will that work?
// ..
RELATED_ (FS_Shell_NonBlocking_f M_Menu_Grid_f)
...
#include "prvm_dungeon.c.h"

#define HMAP_HANDLE_INVALID_NEG_4 -4

#define HMAP_HANDLE_BOILER \
...
// Baker: This one let's default NULL return stay instead of jamming -4 into return
#define HMAP_HANDLE_STRING_RETURN_BOILER \
	int _hmap = (int)PRVM_G_FLOAT(OFS_PARM0); \
	int handle = UNPLUS1(_hmap); \
	\
	if (in_range_beyond (0, handle, PRVM_MAX_OPEN_DOT_MAPS_8) == false) { \
...
// Baker: This is float returning
#define HFORM_HANDLE_BOILER \
...
#define HFORM_HANDLE_STRING_RETURN_BOILER \
	int _hform = (int)PRVM_G_FLOAT(OFS_PARM0); \
	int handle = UNPLUS1(_hform); \
	\
	if (in_range_beyond (0, handle, PRVM_MAX_OPEN_FORMS_8) == false) { \
...
#include "prvm_dungeon_cmds.h"

#include "prvm_form_cmds.h"
#include "prvm_inventory.h"

#include "prvm_steam.h"
