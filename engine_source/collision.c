...
{
	Con_DPrintLinef ("3 %s brush->numpoints = %d", name, brush->numpoints);
	for (int j = 0; j < brush->numpoints; j ++)
		Con_DPrintLinef ("%3d: %f %f %f", j, brush->points[j].v[0], brush->points[j].v[1], brush->points[j].v[2]);
	// FIXME: optimize!
	Con_DPrintLinef ("4 brush->numplanes = %d", brush->numplanes);
	for (int j = 0; j < brush->numplanes; j ++)
		Con_DPrintLinef ("%3d: %f %f %f %f", j, brush->planes[j].normal[0], brush->planes[j].normal[1], brush->planes[j].normal[2], brush->planes[j].dist);
}
...
static void Collision_ValidateBrush(colbrushf_t *brush)
{
	...
	if (!brush->numpoints)
	{
		Con_DPrintLinef ("Collision_ValidateBrush: brush with no points!");
		printbrush = true;
	}
#if 0
	...
	if (brush->numplanes)
	{
		...
		for (k = 0;k < brush->numplanes;k++)
			if (DotProduct(brush->planes[k].normal, brush->planes[k].normal) < 0.0001f)
				Con_PrintLinef ("Collision_ValidateBrush: plane #%d (%f %f %f %f) is degenerate", k, brush->planes[k].normal[0], brush->planes[k].normal[1], brush->planes[k].normal[2], brush->planes[k].dist);
		for (j = 0;j < brush->numpoints;j++)
		{
			...
			for (k = 0;k < brush->numplanes;k++)
			{
				...
				{
					Con_DPrintLinef ("Collision_ValidateBrush: point #%d (%f %f %f) infront of plane #%d (%f %f %f %f)", j, brush->points[j].v[0], brush->points[j].v[1], brush->points[j].v[2], k, brush->planes[k].normal[0], brush->planes[k].normal[1], brush->planes[k].normal[2], brush->planes[k].dist);
					printbrush = true;
				}
		...
		{
			Con_DPrintLinef ("Collision_ValidateBrush: some points have insufficient planes, every point must be on at least 3 planes to form a corner.");
			printbrush = true;
		}
		if (pointsoffplanes == 0) // all points are on all planes
		{
			Con_DPrintLinef ("Collision_ValidateBrush: all points lie on all planes (degenerate, no brush volume!)");
			printbrush = true;
		}
	}
	if (printbrush)
		Collision_PrintBrushAsQHull (brush, "unnamed brush");
}
...
colbrushf_t *Collision_NewBrushFromPlanes(mempool_t *mempool, int numoriginalplanes, const colplanef_t *originalplanes, int supercontents, 
										  int q3surfaceflags, const texture_t *texture, int hasaabbplanes)
{
	...
	int pmaxpoints_64 = 64; // Baker: This is never changed, effective a constant
	int pnumpoints;
	double p[2][3*64];
#if 0
	...
	for (j = 0;j < numoriginalplanes;j++)
	{
		...
		planesbuf[numplanesbuf].q3surfaceflags_plane = originalplanes[j].q3surfaceflags_plane;
		...
		for (k = 0;k < numoriginalplanes && pnumpoints >= 3 && pnumpoints <= pmaxpoints_64;k++)
		{
			...
			if (k != j)
			{
				...
				// the cutting plane
				PolygonD_Divide(pnumpoints, p[w], -originalplanes[k].normal[0], -originalplanes[k].normal[1], -originalplanes[k].normal[2], -originalplanes[k].dist, COLLISION_PLANE_DIST_EPSILON, pmaxpoints_64, p[!w], &pnumpoints, 0, NULL, NULL, NULL);
				w = !w;
			}
		}
		...
		for (k = 0, n = pnumpoints-1;k < pnumpoints;n = k, k++)
		{
			...
			if (m == numedgedirsbuf)
			{
				...
				if (numedgedirsbuf >= maxedgedirsbuf) {
					Con_PrintLinef ("Collision_NewBrushFromPlanes: failed to build collision brush: too many edgedirs for buffer");
					return NULL;
				}
				// add the new one
	...
	if (numplanesbuf < 4) {
		Con_PrintLinef ("Collision_NewBrushFromPlanes: failed to build collision brush: %d triangles, %d planes (input was %d planes), %d vertices", numelementsbuf / 3, numplanesbuf, numoriginalplanes, numpointsbuf);
		return NULL;
	}
	...
	for (j = 0;j < numplanesbuf;j++)
	{
		...
		if (fabs(planesbuf[j].dist - d) > COLLISION_PLANE_DIST_EPSILON)
			Con_DPrintLinef ("plane %f %f %f %f mismatches dist %f", planesbuf[j].normal[0], planesbuf[j].normal[1], planesbuf[j].normal[2], planesbuf[j].dist, d);
	}
	...
	for (j = 0;j < brush->numplanes;j++)
	{
		...
		brush->planes[j].q3surfaceflags_plane = planesbuf[j].q3surfaceflags_plane;
...
void Collision_CalcPlanesForTriangleBrushFloat(colbrushf_t *brush)
{
	...
	if (collision_triangle_bevelsides.integer /*d: 0*/)
	{
	...
	else if (collision_triangle_axialsides.integer /*d:1*/)
	{
			...
			if (startdist >= 0)
			{
				...
				if (enterfrac < f)
				{
					...
					if (nplane < numplanes1)
					{
						...
						nplane2 = nplane;
						hitq3surfaceflags = other_start->planes[nplane2].q3surfaceflags_plane;
					...
					else if (nplane < numplanes2)
					{
						...
						nplane2 = nplane - numplanes1;
						hitq3surfaceflags = trace_start->planes[nplane2].q3surfaceflags_plane;
		...
		if (startdist > enddist)
		{
			...
			if (startdist > 0)
			{
				...
				if (enterfrac < f)
				{
					...
					newimpactplane[2] = startplane[2] * ie + endplane[2] * enterfrac;
					newimpactplane[3] = startplane[3] * ie + endplane[3] * enterfrac;
					hitq3surfaceflags = other_start->planes[nplane].q3surfaceflags_plane;
...
void Collision_TraceBrushTriangleMeshFloat(trace_t *trace, const colbrushf_t *thisbrush_start, const colbrushf_t *thisbrush_end, int numtriangles, const int *element3i, const float *vertex3f, int stride, float *bbox6f, int supercontents, int q3surfaceflags, const texture_t *texture, const vec3_t segmentmins, const vec3_t segmentmaxs)
{
	...
	for (i = 0;i < brush.numplanes;i++)
	{
		brush.planes[i].q3surfaceflags_plane = q3surfaceflags;
...
void Collision_BrushForBox(colboxbrushf_t *boxbrush, const vec3_t mins, const vec3_t maxs, int supercontents, int q3surfaceflags, const texture_t *texture)
{
	...
	else
	{
		...
		for (i = 0;i < 6;i++)
		{
			boxbrush->brush.planes[i].q3surfaceflags_plane = q3surfaceflags;
...
void Collision_Cache_ClipLineToGenericEntitySurfaces(trace_t *trace, model_t *model, matrix4x4_t *matrix, matrix4x4_t *inversematrix, const vec3_t start, const vec3_t end, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask)
{
	...
	Collision_ClipLineToGenericEntity(trace, model, q_frameblend_NULL, q_skeleton_NULL, vec3_origin, vec3_origin, 0, matrix, inversematrix, start, end, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask, collision_extendmovelength.value, true);
...
void Collision_ClipToWorld(trace_t *trace, model_t *model, const vec3_t tstart, const vec3_t mins, const vec3_t maxs, const vec3_t tend, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask, float extend)
{
	...
	if (model && model->TraceBox)
		model->TraceBox(model, q_frameblend_NULL, q_skeleton_NULL, trace, extendtraceinfo.extendstart, mins, maxs, extendtraceinfo.extendend, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask);
	Collision_ClipExtendFinish(&extendtraceinfo);
}
...
void Collision_ClipLineToWorld(trace_t *trace, model_t *model, const vec3_t tstart, const vec3_t tend, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask, float extend, qbool hitsurfaces)
{
	...
	if (model && model->TraceLineAgainstSurfaces && hitsurfaces)
		RELATED_ (example: Mod_Q1BSP_TraceLineAgainstSurfaces Mod_Q1BSP_TraceLine)
		model->TraceLineAgainstSurfaces(model, q_frameblend_NULL, q_skeleton_NULL, trace, extendtraceinfo.extendstart, extendtraceinfo.extendend, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask);
	else if (model && model->TraceLine)
		model->TraceLine(model, q_frameblend_NULL, q_skeleton_NULL, trace, extendtraceinfo.extendstart, extendtraceinfo.extendend, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask);
										...
										const vec3_t bodymins, const vec3_t bodymaxs, int bodysupercontents, matrix4x4_t *matrix, matrix4x4_t *inversematrix, const vec3_t start, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask)
{
	...
	if (model && model->TracePoint)
		model->TracePoint(model, q_frameblend_NULL, q_skeleton_NULL, trace, starttransformed, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask);
	else
...
void Collision_ClipPointToWorld(trace_t *trace, model_t *model, const vec3_t start, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask)
{
	...
	trace->fraction = 1;
	if (model && model->TracePoint)
		model->TracePoint(model, q_frameblend_NULL, q_skeleton_NULL, trace, start, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask);
	VectorCopy(start, trace->endpos);
}
