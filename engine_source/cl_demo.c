...
void CL_NextDemo (void)
{
	...
	dpsnprintf (str, sizeof(str), "playdemo %s" NEWLINE, cls.demos[cls.demonum]);
...
// Baker: Does startdemos do this? YES
// Baker: CL_NextDemo puts this in command buffer?
void CL_PlayDemo(const char *demo)
{
	...
	if (!f) {
		...
		// Baker: So ... demo play error.  I guess if a map were running we do nothing?
		memset (&cls.demos, 0, sizeof(cls.demos)); cls.demonum = STARTDEMO_STOP_LOOP_NEG1;	// "playdemo" error ... just stop the demo loop?
		return;
	}
	...
	CL_Disconnect(); // "playdemo" probably keep?
...
// all the loading time doesn't get counted
	...
	cls.demonum = STARTDEMO_STOP_LOOP_NEG1;		// stop demo loop
}
...
static void CL_Startdemos_f(cmd_state_t *cmd)
{
	...
	// Baker: If a map is running, startdemos shall not occur (pre-existing behavior)

	if (cls.state == ca_dedicated || Sys_CheckParm("-listen") || 
		Sys_CheckParm("-benchmark") || Sys_CheckParm("-demo") || Sys_CheckParm("-capturedemo"))
		return;

    // Baker: April 19 2025 - QW fix.
    // This is basically in standard Quake
    // and somehow not in DarkPlaces.
	if (cls.state == ca_connected) {
		memset (&cls.demos, 0, sizeof(cls.demos)); cls.demonum = STARTDEMO_STOP_LOOP_NEG1; // Baker: This has to do with connecting to a Quakeworld server.
		// size_t dz =sizeof(cls.demos); // sizeof is 128
...
//		Con_CloseConsole_If_Client ();
	...
	cls.demonum = 0; // Baker: December 22 2024 - Make them play if they had been stopped.


#if 0 // Baker: For sure not right now ... 	
	Con_CloseConsole_If_Client (); // Baker: December 22 2024
	key_dest = key_game; // Baker: This is #if 0'd Feb 13 2025
#endif

	if (!sv.active && cls.demonum != STARTDEMO_STOP_LOOP_NEG1 && !cls.demoplayback) {
		cls.demonum = 0;
		CL_NextDemo ();
	}
	else {
		// Baker: ?  Apparently server is active?  May 26 2025
