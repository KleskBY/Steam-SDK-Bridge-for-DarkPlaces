	...
	else
	{
		...
		if (chase_active.value)
		{
			...
			if (chase_overhead.integer)
			{
				...
				trace = CL_TraceLine(vieworg, chase_dest, MOVE_NOMONSTERS_1, NULL, SUPERCONTENTS_SOLID | SUPERCONTENTS_SKY, 0, MATERIALFLAGMASK_TRANSLUCENT, collision_extendmovelength.value, true, false, NULL, false, true);
			...
			if (!cldead)
			{
				...
				if (cl_bob.value && cl_bobcycle.value) {
					...
					if (cl_bob_limit_heightcheck.integer)
					{
						...
						trace = CL_TraceLine(vieworg, bob_height_check_dest, MOVE_NOMONSTERS_1, NULL, SUPERCONTENTS_SOLID | SUPERCONTENTS_SKY | (cl_bob_limit_heightcheck_dontcrosswatersurface.integer ? SUPERCONTENTS_LIQUIDSMASK : 0), 0, MATERIALFLAGMASK_TRANSLUCENT, collision_extendmovelength.value, true, false, NULL, false, true);
...
void V_CalcRefdef (void)
{
	...
	if (cls.state == ca_connected && cls.signon == SIGNONS_4 && 
		...
		cldead = cl.stats_sv[STAT_HEALTH] <= 0 && cl.scores[cl.playerentity-1].qw_spectator == false;

		// Baker: Special NEXIUZ observer and intermission values for health -666, -2342
		if (cldead && IS_OLDNEXUIZ_DERIVED (gamemode) && 
			(cl.stats_sv[STAT_HEALTH] == NEXUIZ_OBS_NEG_666 || cl.stats_sv[STAT_HEALTH] == -2342))
			cldead = false;
		...
		V_CalcRefdefUsing (&ent->render.matrix, cl.viewangles, !ent->persistent.trail_allowed, 
			cl.onground, cl.mcmd.clx_jump, cl.stats_sv[STAT_VIEWHEIGHT], 
...
void V_CalcViewBlend(void)
{
	...
	if (cls.state == ca_connected && cls.signon == SIGNONS_4)
	{
		...
		int is_cldead = cl.stats_sv[STAT_HEALTH] <= 0 && cl.scores[cl.playerentity-1].qw_spectator == false;
...
{
	Cmd_AddCommand(CF_CLIENT | CF_CLIENT_FROM_SERVER, "v_cshift", V_cshift_f, "sets tint color of view (4 parms RGBA 0-255)");
