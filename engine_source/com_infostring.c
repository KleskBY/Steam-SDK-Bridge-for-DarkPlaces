...
char *InfoString_GetValue(const char *buffer, const char *key, char *value, size_t valuelength)
{
	...
	if (valuelength < 1 || !value) {
		Con_PrintLinef ("InfoString_GetValue: no room in value");
		return NULL;
	}
	value[0] = 0;
	if (strchr(key, '\\')) { // Baker: Windows backslash "\" 
		Con_PrintLinef ("InfoString_GetValue: key name " QUOTED_S " contains \\ which is not possible in an infostring", key);
		return NULL;
	}
	if (strchr(key, '\"')) {
		Con_PrintLinef ("InfoString_SetValue: key name " QUOTED_S " contains \" which is not allowed in an infostring", key);
		return NULL;
	}
	if (!key[0]) {
		Con_PrintLinef ("InfoString_GetValue: can not look up a key with no name");
		return NULL;
	}
	while (buffer[pos] == '\\') {
...
void InfoString_SetValue(char *buffer, size_t bufferlength, const char *key, const char *value)
{
	...
	if (strchr(key, '\\') || strchr(value, '\\')) {
		Con_PrintLinef ("InfoString_SetValue: " QUOTED_S " " QUOTED_S " contains \\ which is not possible to store in an infostring", key, value);
		return;
	}
	if (strchr(key, '\"') || strchr(value, '\"')) {
		Con_PrintLinef ("InfoString_SetValue: " QUOTED_S " " QUOTED_S " contains \" which is not allowed in an infostring", key, value);
		return;
	}
	if (!key[0]) {
		Con_PrintLinef ("InfoString_SetValue: can not set a key with no name");
		return;
	}
	while (buffer[pos] == '\\') {
		if (!memcmp(buffer + pos+1, key, keylength) &&
	...
	{
		Con_PrintLinef ("InfoString_SetValue: no room for " QUOTED_S " " QUOTED_S " in infostring", key, value);
		return;
	}
	if (value[0]) {
...
void InfoString_Print(char *buffer)
{
	...
	while (*buffer) {
		if (*buffer != '\\') {
			Con_PrintLinef ("InfoString_Print: corrupt string");
			return;
		}
		...
		Con_PrintLinef ("%20s %s", key, value[0] ? value : "NO VALUE");
	}
}
