...
RELATED_ (conbuffer_t Key_Event)
...
RELATED_ (KeyDest_Set Con_ToggleConsole Key_Event consel_t CON_MASK_DEVELOPER)
...
RELATED_ (Con_DrawConsoleLine_Num_Rows_Drawn)
...
void Consel_MouseMove_Check (void)
{
	...
	if (vid_mouse_show_coords.integer) {
		if (vid_mouse_show_coords.integer > 1)
			Vid_SetWindowTitlef ("%6.1f, %6.1f", in_windowmouse_x * vid_conwidth.integer / vid.width, in_windowmouse_y * vid_conheight.integer / vid.height);
		else
