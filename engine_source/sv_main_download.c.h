...
RELATED_ (CL_ParseServerMessage -> svc_downloaddata    -> CL_ParseDownload_DP)
RELATED_ (SV_ReadClientMessage  -> clc_ackdownloaddata -> SV_ReadClientMessage)

RELATED_ (downloadx_sv_frame: "sv_startdownload" starts one)
RELATED_ (downloadx_sv_start_download)
RELATED_ (download maps/alcyone.bsp -> "cl_downloadbegin" SV_Download_f)
...
RELATED_ (CL: 	#0		"download maps/alcyone.bsp") // CL sends that to server
RELATED_ (SV:	downloadx_sv_start_download				SV_Download_f
		 "cl_downloadbegin 3232323 maps/alcyone.bsp"	)
RELATED_ (CL:	downloadx_cl_start_download_1			CL_DownloadBegin_DP_f -> "sv_startdownload")
RELATED_ (SV:	downloadx_sv_start_download_2			SV_StartDownload_f host_client->download_started = true)
RELATED_ (SV:	downloadx_sv_start_download_during_1	SV_SendClientDatagram)
RELATED_ (SV:	downloadx_cl_start_download_during_2	svc_downloaddata -> replies clc_ackdownloaddata)
RELATED_ (SV:	downloadx_sv_start_download_during_3	clc_ackdownloaddata -> might do cl_downloadfinished)
RELATED_ (CL:	downloadx_sv_finish_0					"cl_downloadfinished size crc models/filename.bsp")
RELATED_ (CL:	downloadx_cl_finish_1					CL_DownloadFinished_DP_f)
...
RELATED_ (CL:	downloadx_cl_finish_2_write_file		CL_StopDownload_DP )
...
RELATED_ (DP more data -> SV_SendClientMessages -> for cl -> SV_SendClientDatagram ->)

RELATED_ (QW_CL_CheckOrDownloadFile)
...
RELATED_ (CL_BeginDownloads_DP)
RELATED_ (cl.loadmodel_total .. set to nummodels cl.loadsound_total)
RELATED_ (svc_downloaddata -> CL_ParseDownload_DP CL_DownloadFinished_DP_f)
RELATED_ (CL_Input.c sends clc_ackdownloaddata -> )
RELATED_ (CL_StopDownload_DP also writes the download to file plus rescans file system)

RELATED_ (If server ack size >= filesize, "cl_downloadfinished %d %d %s")
...
RELATED_ (clc_ackdownloaddata svc_downloaddata)


RELATED_ (CL_BeginDownloads_Prespawn_Zircon_Extensions_Send)		//
RELATED_ (CL_BeginDownloads_DP)										// "cl_begindownloads" SIGNON_1 will go to 2
RELATED_ (CL_BeginDownloads_Prespawn_Zircon_Extensions_Send)

// Baker: There is no cl_download, right?

RELATED_ (host_client->netconnection->mysocket)
...
RELATED_ ("download maps/mymap.bsp chunked")
static void SV_Download_f(cmd_state_t *cmd)
{
	const char *whichpack, *extension; // Dec 22 2024 unused ... *whichpack2,
	...
	if (FS_CheckNastyPath(s_download_file, fs_isgamedir_false)) {
...
downloadx_sv_start_download:
		...
		RELATED_ (CL_DownloadBegin_DP_f)
