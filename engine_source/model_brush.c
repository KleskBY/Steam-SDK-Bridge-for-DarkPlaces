...
cvar_t r_cull_darkplaces_classic = {CF_CLIENT, "r_cull_darkplaces_classic", "1", "Culls using DarkPlaces Classic logic instead of DarkPlaces Beta logic.  1 = Addresses a Malice issue."};
...
cvar_t mod_q3bsp_lightgrid_texture = {CF_CLIENT, "mod_q3bsp_lightgrid_texture", "0", "directly apply the lightgrid as a global texture rather than only reading it at the entity origin [Zircon default]"}; // Baker: Changed default to 0 August 24 2025
...
void Collision_ClipTrace_Box(trace_t *trace, const vec3_t cmins, const vec3_t cmaxs, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask, int boxsupercontents, int boxq3surfaceflags, const texture_t *boxtexture)
{
	...
	cbox_planes[0].q3surfaceflags_plane = boxq3surfaceflags;cbox_planes[0].texture = boxtexture;
	cbox_planes[1].q3surfaceflags_plane = boxq3surfaceflags;cbox_planes[1].texture = boxtexture;
	cbox_planes[2].q3surfaceflags_plane = boxq3surfaceflags;cbox_planes[2].texture = boxtexture;
	cbox_planes[3].q3surfaceflags_plane = boxq3surfaceflags;cbox_planes[3].texture = boxtexture;
	cbox_planes[4].q3surfaceflags_plane = boxq3surfaceflags;cbox_planes[4].texture = boxtexture;
	cbox_planes[5].q3surfaceflags_plane = boxq3surfaceflags;cbox_planes[5].texture = boxtexture;
	memset(trace, 0, sizeof(trace_t));
...
static void Mod_BSP_DecompressVis(const unsigned char *in, const unsigned char *inend, unsigned char *out, unsigned char *outend)
{
	...
	{
		if (in == inend) {
			Con_PrintLinef ("Mod_BSP_DecompressVis: input underrun on model " QUOTED_S " (decompressed %d of %d output bytes)", loadmodel->model_name, (int)(out - outstart), (int)(outend - outstart));
			return;
		}
		c = *in++;
		if (c)
		...
		{
			if (in == inend) {
				Con_PrintLinef ("Mod_BSP_DecompressVis: input underrun (during zero-run) on model " QUOTED_S " (decompressed %d of %d output bytes)", loadmodel->model_name, (int)(out - outstart), (int)(outend - outstart));
				return;
			}
			for (c = *in++;c > 0;c--) {
				if (out == outend) {
					Con_PrintLinef ("Mod_BSP_DecompressVis: output overrun on model " QUOTED_S " (decompressed %d of %d output bytes)", loadmodel->model_name, (int)(out - outstart), (int)(outend - outstart));
					return;
				}
				*out++ = 0;
			}
...
static void Mod_Q1BSP_LoadSplitSky (unsigned char *src, int width, int height, int bytesperpixel)
{
	...
	loadmodel->brush.solidskyskinframe = R_SkinFrame_LoadInternalBGRA("sky_solidtexture", 0         , (unsigned char *) solidpixels, w, h, 0, 0, 0, vid.sRGB3D, q_is_sky_load_true); // Baker r9011 fix q1bsp sky never being reloaded after first map
	loadmodel->brush.alphaskyskinframe = R_SkinFrame_LoadInternalBGRA("sky_alphatexture", TEXF_ALPHA, (unsigned char *) alphapixels, w, h, 0, 0, 0, vid.sRGB3D, q_is_sky_load_true); // Baker r9011 fix q1bsp sky never being reloaded after first map
...
static void Mod_Q1BSP_LoadTextures(sizebuf_t *sb)
{
	...
	for (i = 0; i < nummiptex; i++) {
		...
		// HEREON - We could AI resample here.  Or regular resample.
...
regularo:
				...
				|| String_Starts_With(tx->name, "sky") /*!strncmp(tx->name, "sky", 3)*/)
			{
				...
				if (!skinframe && loadmodel->brush.ishlbsp) {
					...
					if (pixels == NULL)
						pixels = freepixels = W_GetTextureBGRA_HalfLife(tx->name);
					if (pixels != NULL)
					{
						tx->width = image_width;
						tx->height = image_height;
						tx->materialshaderpass->skinframes[0] = R_SkinFrame_LoadInternalBGRA(tx->name, TEXF_ALPHA | TEXF_MIPMAP | TEXF_ISWORLD | TEXF_PICMIP, pixels, image_width, image_height, image_width, image_height, CRC_Block(pixels, image_width * image_height * 4), q_tx_convertsrgb_true, q_is_sky_load_false);
...
static void Mod_Q1BSP_LoadFaces(sizebuf_t *sb)
{
	...
	for (surfacenum = 0, surface = loadmodel->data_surfaces;surfacenum < count;surfacenum++, surface++)
	{
		...
		for (i = 0;i < surface->num_vertices;i++)
		{
			...
			s = DotProductDouble(((loadmodel->surfmesh.data_vertex3f + 3 * surface->num_firstvertex) + i * 3), surface->lightmapinfo->texinfo->vecs[0]) + surface->lightmapinfo->texinfo->vecs[0][3];
			t = DotProductDouble(((loadmodel->surfmesh.data_vertex3f + 3 * surface->num_firstvertex) + i * 3), surface->lightmapinfo->texinfo->vecs[1]) + surface->lightmapinfo->texinfo->vecs[1][3];
		...
		texmins[0] = texmaxs[0] = DotProductDouble((loadmodel->surfmesh.data_vertex3f + 3 * surface->num_firstvertex), surface->lightmapinfo->texinfo->vecs[0]) + surface->lightmapinfo->texinfo->vecs[0][3];
		texmins[1] = texmaxs[1] = DotProductDouble((loadmodel->surfmesh.data_vertex3f + 3 * surface->num_firstvertex), surface->lightmapinfo->texinfo->vecs[1]) + surface->lightmapinfo->texinfo->vecs[1][3];
		...
		if (!(surface->lightmapinfo->texinfo->q1flags & TEX_SPECIAL) || surface->lightmapinfo->samples)
		{
			...
			loadmodel->lit = loadmodel->brushq1.lightdata ? true : false; // Baker r1002: Proper Quake behavior for Q1BSP maps with no light data -- all entities in map render fullbright.
		}
	}
	...
	if (cls.state != ca_dedicated)
	{
		...
		for (surfacenum = 0, surface = loadmodel->data_surfaces;surfacenum < count;surfacenum++, surface++) {
			...
			for (i = 0;i < surface->num_vertices;i++)
			{
				u = ((DotProductDouble(((loadmodel->surfmesh.data_vertex3f + 3 * surface->num_firstvertex) + i * 3), surface->lightmapinfo->texinfo->vecs[0]) + surface->lightmapinfo->texinfo->vecs[0][3]) + 8 - surface->lightmapinfo->texturemins[0]) * (1.0 / 16.0);
				v = ((DotProductDouble(((loadmodel->surfmesh.data_vertex3f + 3 * surface->num_firstvertex) + i * 3), surface->lightmapinfo->texinfo->vecs[1]) + surface->lightmapinfo->texinfo->vecs[1][3]) + 8 - surface->lightmapinfo->texturemins[1]) * (1.0 / 16.0);
...
static void Mod_Q1BSP_LoadLeafs(sizebuf_t *sb)
{
	...
	loadmodel->brush.data_pvsclusters = (byte *)Mem_Alloc(loadmodel->mempool, loadmodel->brush.num_pvsclusters * loadmodel->brush.num_pvsclusterbytes);
	...
	for ( i=0 ; i<count ; i++, out++)
	{
		...
		if (p >= 0 && out->clusterindex >= 0)
		{
			if (p >= loadmodel->brushq1.num_compressedpvs)
				Con_PrintLinef ("Mod_Q1BSP_LoadLeafs: invalid visofs");
			else
		...
		for (;;)
		{
			...
			if (com_token[0] == '{')
			{
				...
				for (;;)
				{
					...
					planedist = DotProductDouble(point[0], planenormal);
...
static void Mod_BSP_RecursiveNodePortals(mnode_t *node)
{
	...
	{
		Con_PrintLinef (CON_WARN "Mod_BSP_RecursiveNodePortals: WARNING: new portal was clipped away");
		nodeportal->numpoints = 0;
	}
	else if (nodeportal->numpoints >= MAX_PORTALPOINTS)
	{
		Con_PrintLinef (CON_WARN "Mod_BSP_RecursiveNodePortals: WARNING: new portal has too many points");
		nodeportal->numpoints = 0;
	}
...
#define VISPATCH_MAPNAME_LENGTH_32	32

typedef struct vispatch_s
{
	char	mapname[VISPATCH_MAPNAME_LENGTH_32];	// Baker: DO NOT CHANGE THIS to MAX_QPATH_64, must be 32
...
{
	int i, j, k;
	sizebuf_t lumpsb[HEADER_LUMPS_15];
	mmodel_t *bm;
...
// load into heap
	...
	for (i = 0;i < HEADER_LUMPS_15;i++)
	{
		...
		if (i == LUMP_VISIBILITY_4 || i == LUMP_LEAFS_10 || i == LUMP_NODES)
			continue;
	...
	qbool did_load_external_vis = false;
	do {
		byte *data_ta = NULL;
		while (external_vis.integer > 2 || (external_vis.integer && cls.protocol != PROTOCOL_QUAKEWORLD)) {
			va_super (visfilename, MAX_QPATH_128, "%s", loadmodel->model_name);
			FS_StripExtension (visfilename, visfilename, sizeof (visfilename));
			c_strlcat (visfilename, ".vis");

			fs_offset_t data_za_size;
			byte *data_ta = (byte *) FS_LoadFile(visfilename, tempmempool, fs_quiet_FALSE, &data_za_size);
			if (!data_ta)
				break;

			ccs *bsp_location_ta = FS_FileWhich_Ta (loadmodel->model_name, fs_which_reply_type_NULL);
			ccs *vis_location_ta = FS_FileWhich_Ta (visfilename, fs_which_reply_type_NULL);

			qbool are_same_location = String_Match (bsp_location_ta, vis_location_ta);

			Con_DPrintLinef ("Vis Check .BSP is in folder %s", bsp_location_ta);
			Con_DPrintLinef ("Vis Check .VIS is in folder %s", vis_location_ta);

			if (!are_same_location) {
				Con_PrintLinef ("External .vis is not in same folder (%s) as .bsp (%s), ignoring ...", vis_location_ta, bsp_location_ta);
			}
			Mem_FreeNull_ (bsp_location_ta);
			Mem_FreeNull_ (vis_location_ta);

			if (!are_same_location) {
				break;
			}

			// Need realpath of the map
			// Need realpath of the .vis
			// Strip filename for both.  The must match

			sizebuf_t sb_file = {0};
			MSG_InitReadBuffer(&sb_file, data_ta, data_za_size);

			// READ 36.
			vispatch_t myvis = {0};
			MSG_ReadBytes (&sb_file, VISPATCH_MAPNAME_LENGTH_32, (byte *)&myvis.mapname[0]);// (&sb_vis)
			myvis.filelen = MSG_ReadLittleLong (&sb_file); // 34980 - DM3

			// MAKE SURE MAP NAME MATCHES ?

			// 16744 - DM3
			int vis_file_length = MSG_ReadLittleLong (&sb_file); // -1 or sb->badread is failure.

			Con_DPrintLinef ("...%d bytes visibility data", vis_file_length);

			sizebuf_t sb_vis = {0};
			MSG_InitReadBuffer(&sb_vis, (byte *)&sb_file.data[sb_file.readcount], vis_file_length);

			if (vis_file_length == -1 || !sb_file.cursize) {
				break;
			}

			sb_file.readcount += sb_vis.cursize;

			fs_offset_t vispatch_leafssize = MSG_ReadLittleLong (&sb_file);
			
			sizebuf_t sb_leafs = {0}; // 16788 - DM3 .... 18228 lump filesize
			fs_offset_t expected_leaflumpsize = lumpsb[LUMP_LEAFS_10].cursize;
			if (vispatch_leafssize != expected_leaflumpsize) {
				Con_PrintLinef ("External .vis leafsize (" INT64_F ") is not same as .bsp leaflumpsize (" INT64_F "), ignoring ...", 
					vispatch_leafssize, expected_leaflumpsize);
				break;
			}

			// Baker: Now that FTEQW lumpsize validation passed ...
			MSG_InitReadBuffer(&sb_leafs, (byte *)&sb_file.data[sb_file.readcount], vispatch_leafssize);

			Mod_Q1BSP_LoadVisibility (&sb_vis);
			Mod_BSP_LoadSubmodels(&lumpsb[LUMP_MODELS_14], &hullinfo);
			Mod_Q1BSP_LoadLeafs(&sb_leafs);

			loadmodel->brush.is_vised = did_load_external_vis = true; // SUCCESS PART 1
			Con_PrintLinef ("Loaded %s", visfilename);
			break;
		} // while (1)

		Mem_FreeNull_ (data_ta);

		if (did_load_external_vis)
			break; // EXTERNAL VIS DONE

		Mod_Q1BSP_LoadVisibility(&lumpsb[LUMP_VISIBILITY_4]);
		// load submodels before leafs because they contain the number of vis leafs
		Mod_BSP_LoadSubmodels(&lumpsb[LUMP_MODELS_14], &hullinfo);
		Mod_Q1BSP_LoadLeafs(&lumpsb[LUMP_LEAFS_10]);
	} while (0);
	...
	for (i = 0; i < HEADER_LUMPS_15; i++) {
		if (did_load_external_vis && isin2 (i, LUMP_VISIBILITY_4, LUMP_LEAFS_10))
			continue;
	...
	if (mod_bsp_portalize.integer /*d:1*/) {
		...
		// Baker: May 24 2025 - Added mod->radius to block health/ammo boxes
		if (cl.loadmodel_current == 1 && mod->radius >= 128) // Baker: October 13 2024 .. don't do this for healthboxes and such
#endif
		{
			if (mod_bsp_portalize.integer >= 2 || mod->brush.is_vised == false || r_novis.integer) {
				// Baker: This is still hitting for healthboxes/ammo models when connecting to a server.
				if (mod->brush.is_vised == false)
					Con_PrintLinef ("No vis data ... making portals for %s...", mod->model_name);
				Mod_BSP_MakePortals ();
			} else {
	...
	for (i = 0;i < mod->brush.numsubmodels;i++)
	{
		...
		if (mod_q1bsp_polygoncollisions.integer /*d:0*/)
		{
	...
	Con_DPrintLinef ("Stats for q1bsp model " QUOTED_S ": %d faces, %d nodes, %d leafs, %d visleafs, %d visleafportals, mesh: %d vertices, %d triangles, %d surfaces", loadmodel->model_name, loadmodel->num_surfaces, loadmodel->brush.num_nodes, loadmodel->brush.num_leafs, mod->brush.num_pvsclusters, loadmodel->brush.num_portals, loadmodel->surfmesh.num_vertices, loadmodel->surfmesh.num_triangles, loadmodel->num_surfaces);
	return true;
}
...
static void Mod_Q2BSP_LoadBrushes(sizebuf_t *sb)
{
	...
	for (i = 0; i < count; i++, out++)
	{
		...
		for (j = 0;j < out->numbrushsides;j++)
		{
			...
			planes[j].q3surfaceflags_plane = out->firstbrushside[j].texture->surfaceflags;
			planes[j].texture = out->firstbrushside[j].texture;
			q3surfaceflags |= planes[j].q3surfaceflags_plane;
		}
...
// load into heap
	...
	Con_DPrintLinef ("Stats for q2bsp model " QUOTED_S ": %d faces, %d nodes, %d leafs, %d clusters, %d clusterportals, mesh: %d vertices, %d triangles, %d surfaces", loadmodel->model_name, loadmodel->num_surfaces, loadmodel->brush.num_nodes, loadmodel->brush.num_leafs, mod->brush.num_pvsclusters, loadmodel->brush.num_portals, loadmodel->surfmesh.num_vertices, loadmodel->surfmesh.num_triangles, loadmodel->num_surfaces);
}
...
#define sscanf sscanf_s
				...
				if (sscanf(value, "%f %f %f", &v[0], &v[1], &v[2]) != 3)
					Con_PrintLinef ("Mod_Q3BSP_LoadEntities: funny gridsize " QUOTED_S " in %s, interpreting as \"%f %f %f\" to match q3map2's parsing", value, loadmodel->model_name, v[0], v[1], v[2]);
...
static void Mod_Q3BSP_LoadEffects(lump_t *l)
{
	...
	for (i = 0;i < count;i++, in++, out++)
	{
		...
		out->brushindex = n;
		out->unknown_q3_field = LittleLong(in->unknown_q3_field);
	}
}
...
#if 1 // Baker: Dec 5 2024 DarkPlaces classic does not have this
		// Baker: Dec 5 2024 shell dungeon unlit reports as lit without this ...
		// This helps or not?  No help.
		// Baker: This causes shell shell_dungeon to be considered unlit.
			if (in->color4ub[0] != 0 || in->color4ub[1] != 0 || in->color4ub[2] != 0)
#endif
				loadmodel->lit = true;
	}
}
...
static void Mod_Q3BSP_LoadTriangles(lump_t *l)
{
	...
	if (!loadmodel->brushq3.num_vertices) {
		if (count)
			Con_PrintLinef ("Mod_Q3BSP_LoadTriangles: %s has triangles but no vertexes, broken compiler, ignoring problem", loadmodel->model_name);
...
static void Mod_Q3BSP_LoadLightmaps(lump_t *l, lump_t *faceslump)
{
	...
	else if (l->filelen)
	{
		...
		if (developer_loading.integer)
			Con_PrintLinef ("Using internal lightmaps");
	...
	loadmodel->brushq3.data_lightmaps = (rtexture_t **)Mem_Alloc(loadmodel->mempool,
		loadmodel->brushq3.num_mergedlightmaps * sizeof(rtexture_t *));
	if (loadmodel->brushq3.deluxemapping)
	...
	for (i = 0; i < count; i ++) {
		...
		if (((realindex + 1) & (mergedrowsxcolumns - 1)) == 0 || (realindex + 1) == realcount)
		{
			...
			RELATED_ (R_LoadTexture2D, R_UploadFullTexture, R_UploadPartialTexture, R_UpdateTexture, )
...
static void Mod_Q3BSP_LoadFaces(lump_t *l)
{
	...
	for (;i < count;i++, in++, out++)
	{
		...
		if (n < -1 || n >= loadmodel->brushq3.num_effects)
		{
			if (developer_extra.integer)
				Con_DPrintf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): invalid effectindex %d (%d effects)\n", i, out->texture->name, n, loadmodel->brushq3.num_effects);
			n = -1;
		}
		if (n == -1)
			out->q3_effect_msurf = NULL;
		else
			out->q3_effect_msurf = loadmodel->brushq3.data_effects + n;
		...
		if (cls.state != ca_dedicated)
		{
			...
			else if (n >= loadmodel->brushq3.num_originallightmaps)
			{
				if (loadmodel->brushq3.num_originallightmaps != 0)
					Con_Printf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): invalid lightmapindex %d (%d lightmaps)\n", i, out->texture->name, n, loadmodel->brushq3.num_originallightmaps);
				n = -1;
			}
			else
			{
				...
				// Baker: We are having trouble detecting unlit maps in Q3 and even DarkPlaces Classic
				// doesn't do it right.
				// However, if we report a map as .lit and it isn't then r_fullbright can fix.
				// Which is better than a poorly lit map reporting as fullbright :(
#if 1 // Baker: December 5 2024 - Enabling this despite it not being perfect.
				// Baker: Dec 5 2024 - DarkPlaces Classic does this.  If we object to this
				// for some reason, provide the reason why and example.
				// Baker: Dec 5 2024 - shell_dungeon_unlit reports as .lit does this fix?
				// A: No.  And doesn't hurt lit version.

				// DarkPlaces Classic FALSELY reports shell_dungeon_unlit as lit
				loadmodel->lit = true;
		...
		{
			Con_Printf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): numelements %d is not a multiple of 3\n", i, out->texture->name, LittleLong(in->numelements));
			continue;
		}
		if (firstvertex < 0 || firstvertex + numvertices > loadmodel->brushq3.num_vertices)
		{
			Con_Printf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): invalid vertex range %d : %d (%d vertices)\n", i, out->texture->name, firstvertex, firstvertex + numvertices, loadmodel->brushq3.num_vertices);
			continue;
		}
		if (firstelement < 0 || firstelement + numtriangles * 3 > loadmodel->brushq3.num_triangles * 3)
		{
			Con_Printf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): invalid element range %d : %d (%d elements)\n", i, out->texture->name, firstelement, firstelement + numtriangles * 3, loadmodel->brushq3.num_triangles * 3);
			continue;
		}
		switch(type)
		{
		...
		case Q3FACETYPE_PATCH:
			...
			{
				Con_Printf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): invalid patchsize %dx%d\n", i, out->texture->name, patchsize[0], patchsize[1]);
				continue;
			}
			...
			// store it for the LOD grouping step
	 		...
	 		patchtess[patchtesscount].info.lods[PATCH_LOD_VISUAL_1].xtess = xtess;
	 		patchtess[patchtesscount].info.lods[PATCH_LOD_VISUAL_1].ytess = ytess;
	 		patchtess[patchtesscount].info.lods[PATCH_LOD_COLLISION_0].xtess = cxtess;
	 		patchtess[patchtesscount].info.lods[PATCH_LOD_COLLISION_0].ytess = cytess;
		...
		case Q3FACETYPE_FLARE:
			if (developer_extra.integer)
				Con_DPrintf ("Mod_Q3BSP_LoadFaces: face #%d (texture " QUOTED_S "): Q3FACETYPE_FLARE not supported (yet)\n", i, out->texture->name);
			// don't render it
			continue;
		}
	...
	for(i = 0; i < patchtesscount; ++i)
	{
		finalwidth = Q3PatchDimForTess(patchtess[i].info.xsize, patchtess[i].info.lods[PATCH_LOD_VISUAL_1].xtess);
		finalheight = Q3PatchDimForTess(patchtess[i].info.ysize,patchtess[i].info.lods[PATCH_LOD_VISUAL_1].ytess);
		...
		finalwidth = Q3PatchDimForTess(patchtess[i].info.xsize, patchtess[i].info.lods[PATCH_LOD_COLLISION_0].xtess);
		finalheight = Q3PatchDimForTess(patchtess[i].info.ysize,patchtess[i].info.lods[PATCH_LOD_COLLISION_0].ytess);
		numvertices = finalwidth * finalheight;
		numtriangles = (finalwidth - 1) * (finalheight - 1) * 2;
	...
	for (;i < count && meshvertices + out->num_vertices <= loadmodel->surfmesh.num_vertices;i++, in++, out++)
	{
		...
		case Q3FACETYPE_PATCH:
			...
			for(j = 0; j < patchtesscount; ++j)
				...
				{
					xtess = patchtess[j].info.lods[PATCH_LOD_VISUAL_1].xtess;
					ytess = patchtess[j].info.lods[PATCH_LOD_VISUAL_1].ytess;
					cxtess = patchtess[j].info.lods[PATCH_LOD_COLLISION_0].xtess;
					cytess = patchtess[j].info.lods[PATCH_LOD_COLLISION_0].ytess;
					break;
				}
			if (xtess == -1)
			{
		...
		if (invalidelements)
		{
			Con_Printf (CON_WARN "Mod_Q3BSP_LoadFaces: Warning: face #%d has %d invalid elements, type = %d, texture->name = " QUOTED_S ", texture->surfaceflags = %d, firstvertex = %d, numvertices = %d, firstelement = %d, numelements = %d, elements list:\n", i, invalidelements, type, out->texture->name, out->texture->surfaceflags, firstvertex, out->num_vertices, firstelement, out->num_triangles * 3);
			for (j = 0;j < out->num_triangles * 3;j++)
			{
...
static void Mod_Q3BSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	*/
	...
	Con_DPrintLinef ("Stats for q3bsp model " QUOTED_S ": %d faces, %d nodes, %d leafs, %d clusters, %d clusterportals, mesh: %d vertices, %d triangles, %d surfaces", loadmodel->model_name, loadmodel->num_surfaces, loadmodel->brush.num_nodes, loadmodel->brush.num_leafs, mod->brush.num_pvsclusters, loadmodel->brush.num_portals, loadmodel->surfmesh.num_vertices, loadmodel->surfmesh.num_triangles, loadmodel->num_surfaces);
}
...
int Mod_OBJ_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	dpsnprintf(materialname, sizeof(materialname), "%s", loadmodel->model_name);
	...
	Con_DPrintLinef ("Stats for obj model " QUOTED_S ": %d faces, %d nodes, %d leafs, %d clusters, %d clusterportals, mesh: %d vertices, %d triangles, %d surfaces", loadmodel->model_name, loadmodel->num_surfaces, loadmodel->brush.num_nodes, loadmodel->brush.num_leafs, mod->brush.num_pvsclusters, loadmodel->brush.num_portals, loadmodel->surfmesh.num_vertices, loadmodel->surfmesh.num_triangles, loadmodel->num_surfaces);
	return true;
}
...
//	Cvar_RegisterVariable(&r_subdivide_size);
	...
	Cvar_RegisterVariable(&r_cull_darkplaces_classic);
