...
qbool CSQC_AddRenderEdict(prvm_edict_t *ed, int edictnum)
{
	...
	entrender->ef_effects |= entrender->model->mf_effects;
	...
	if (PRVM_clientedictfloat(ed, effects))	
		entrender->ef_effects |= (int)PRVM_clientedictfloat(ed, effects);
	if (!entrender->ralpha)
...
#endif
	// Baker: Then clmain calls
	// CL_UpdateRenderEntity which does a matrix invert and then calls VM_FrameBlendFromFrameGroupBlend
	// e is a entity_t *e entity_render_t
	// entrender entrender = cl.csqcrenderentities + edictnum;
	// e->render -- accesses entity_render_t
	// Baker: in theory if we set entity lerpfrac, frame, frame1time 
	RELATED_ (CL_UpdateRenderEntity VM_FrameBlendFromFrameGroupBlend out: frameblend_t *frameblend)
	...
	if (renderflags)
	{
		if (renderflags & RF_VIEWMODEL)			
			entrender->crflags |= RENDER_VIEWMODEL | RENDER_NODEPTHTEST;
		if (renderflags & RF_EXTERNALMODEL)		
			entrender->crflags |= RENDER_EXTERIORMODEL;
		if (renderflags & RF_WORLDOBJECT)		
			entrender->crflags |= RENDER_WORLDOBJECT;
		if (renderflags & RF_DEPTHHACK)			
			entrender->crflags |= RENDER_NODEPTHTEST;
		if (renderflags & RF_ADDITIVE)			
	...
	if (!r_fullbright.integer && r_refdef.scene.worldmodel && r_refdef.scene.worldmodel->lit) { // Baker r1002: Proper Quake behavior for Q1BSP maps with no light data -- all entities in map render fullbright.
		if (!(entrender->ef_effects & EF_FULLBRIGHT) && !(renderflags & RF_FULLBRIGHT))
			entrender->crflags |= RENDER_LIGHT;
	}
	// hide player shadow during intermission or nehahra movie
	if (!(entrender->ef_effects & (EF_NOSHADOW | EF_ADDITIVE_32 | EF_NODEPTHTEST))
	 &&  (entrender->ralpha >= 1)
	...
	if (entrender->ef_effects & EF_NOSELFSHADOW)
		entrender->crflags |= RENDER_NOSELFSHADOW;
	if (entrender->ef_effects & EF_NODEPTHTEST)
		entrender->crflags |= RENDER_NODEPTHTEST;
	if (entrender->ef_effects & EF_ADDITIVE_32)
		entrender->crflags |= RENDER_ADDITIVE;
	if (entrender->ef_effects & EF_DOUBLESIDED)
		entrender->crflags |= RENDER_DOUBLESIDED;
	if (entrender->ef_effects & EF_DYNAMICMODELLIGHT)
		entrender->crflags |= RENDER_DYNAMICMODELLIGHT;
...
//			CL_VM_FormEvent (UNPLUS1(f->ui.csqc_handle_plus1), 
//				k->cm.name_a, "OnChange", k->selectedindex);

//qbool CL_VM_FormEvent (int hform, ccs *name, *ccs fevent, int ival, ccs *sval)
RELATED_ (VM_map_compile_with_callback VM_callfunction)
// Setting a property does NOT fire an event.  Ever.
CALLERS_ (Object_Event_OnChange_Recursive)
qbool CL_VM_FormEvent (int hform, func_t form_event_callback, ccs *s_object_name, ccs *s_event_name, float fval, ccs *s_value_string)
{
	// Baker: ival is like "ListIndex"
	qbool r;
	prvm_prog_t *prog = CLVM_prog;

	if (!cl.csqc_loaded && !cl.csqc_is_loading)
		return false;

CSQC_BEGIN
	PRVM_clientglobalfloat(time) = cl.time; // Baker: Dubious
	...
	if (s_value_string) {
		PRVM_G_FLOAT(OFS_PARM0) = (float)PLUS1(hform); // 0-7 ===> 1-8 so we never send ZERO
		PRVM_G_INT(OFS_PARM1) = PRVM_SetTempString(prog, s_object_name);
		PRVM_G_INT(OFS_PARM2) = PRVM_SetTempString(prog, s_event_name);
		PRVM_G_INT(OFS_PARM3) = PRVM_SetTempString(prog, s_value_string);
		prog->ExecuteProgram(prog, form_event_callback, "");
		r = true; // Baker? //r = CSQC_RETURNVAL != 0;
	} else {
		// Catch-all takes all strings
		va_super (s_value_num, 256, FLOAT_LOSSLESS_FORMAT, fval);
		//void Form1_Event (string controlname, string eventname, string text);
		PRVM_G_FLOAT(OFS_PARM0) = (float)PLUS1(hform); // 0-7 ===> 1-8 so we never send ZERO
		PRVM_G_INT(OFS_PARM1) = PRVM_SetTempString(prog, s_object_name);
		PRVM_G_INT(OFS_PARM2) = PRVM_SetTempString(prog, s_event_name);
		PRVM_G_INT(OFS_PARM3) = PRVM_SetTempString(prog, s_value_num);
		prog->ExecuteProgram(prog, form_event_callback, "");
		r = true; // Baker? //r = CSQC_RETURNVAL != 0;
	}
...
qbool CL_VM_String_Callback (int hmodalbossplus1, func_t close_callback, ccs *sMessage)
{
	// Baker: ival is like "ListIndex"
	qbool r;
	prvm_prog_t *prog = CLVM_prog;
...
CSQC_BEGIN
	...
	// after executing this function, delete all tempstrings it created
	int restorevm_tempstringsbuf_cursize = prog->tempstringsbuf.cursize;	
	PRVM_G_FLOAT(OFS_PARM0) = hmodalbossplus1;
	PRVM_G_INT(OFS_PARM1) = PRVM_SetTempString(prog, sMessage);
	prog->ExecuteProgram(prog, close_callback, "");
	r = true; // Baker? //r = CSQC_RETURNVAL != 0;
...
RELATED_ (svc_stufftext qw_svc_stufftext)
...
#endif // 0 - CL_VM_Event - Baker: Some [515] code not active
...
void CL_VM_Init (void)
{
	...
	//PRVM_clientedictfloat(progs->edicts, modelindex)
	PRVM_clientedictfloat(prog->edicts, modelindex) = 1; // Baker: World is always 1
	...
	// call the prog init
	cl.csqc_is_loading = true; // Baker: Jan 17 2025
