...
typedef struct rtlight_s
{
	...
	vec_t specularscale;
	/// LIGHTFLAG_* flags
	int rtl_flags; // LIGHTFLAG_REALTIMEMODE vs. LIGHTFLAG_NORMALMODE
...
RELATED_ (Non-QW: CL_SignonReply SIGNON_1 -> CL_BeginDownloads_DP)
...
typedef struct client_static_s { // cls.
	...
	mempool_t *levelmempool;		// Baker: particles, pvs, lightmaps ...
	mempool_t *permanentmempool;	// Baker: video resolutions, tempentities, qwdownload,  hlwads, gfx.wad, cl_video.c
...
// connection information
	...
	RELATED_ (ZIRCON_EXT_CHUNKED_2, ZIRCON_EXT_FREEMOVE_4, ZIRCON_EXT_NONSOLID_FLAG_8)
	...
	// Baker: Jan 29 2025 - This relates to forcing "map" to load something by injecting player starts.
	// I forgot about this and not sure what to do here ...
	// I guess action plan is document this well.
	// Something like ...
	// map e1m1 info_player_team info_player_start (??)
	// To be honest, I would prefer forcing an info_player_start at 0,0,0 instead.
	char *s_find_za;
	char *s_replace_za;
	char *s_effective_za;
}
...
extern int32list_s cl_list_tosses;
...
typedef struct client_state_s
{
	...
	int islocalgame; // Baker: cls.state == ca_connected && sv.active && cl.maxclients == 1
...
// information for local display
	// health, etc
	int stats_sv[MAX_CL_STATS_256];
	float *statsf_sv; // points to stats[] array
...
#define LATESTFRAMENUMS_32 32
	...
	int *brushmodel_entities;  // Baker: These are star '*' inline map models (no healthboxes)
	particle_t *particles;
	showlmp_t *showlmps;

	int num_entities;
	int num_static_entities;
	int num_brushmodel_entities; // Baker: These are star '*' inline map models (no healthboxes)
	int num_effects;
	int num_beams;
	int num_dlights;
	int num_particles;
	...
	RELATED_ (CL_Frame CL_UpdateScreen CL_UpdateScreen_SCR_DrawScreen VM_CL_R_RenderScene "renderscene" R_RenderView)
	RELATED_ (CL_Frame CL_UpdateScreen CL_UpdateScreen_SCR_DrawScreen R_RenderView)
	RELATED_ (host.superframecount)

	// cls.world_start_realtime

	char		levelshotsname[MAX_QPATH_128]; // "devinfo" likes to know.  Exclusive user.
	float		cl_levelshot_time;
	...
	qbool	tool_marker_on;
	vec3_t	tool_marker_origin;
	qbool csqc_is_loading;

	mleaf_t *viewleaf;
	qbool	viewleaf_changed;
}
client_state_t;
...
#define STARTDEMO_STOP_LOOP_NEG1 -1
