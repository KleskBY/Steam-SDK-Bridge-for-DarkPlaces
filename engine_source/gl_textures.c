...
#endif
	default:
		Host_Error_Line ("R_GetTexTypeInfo: unknown texture format %d with flags %x", (int)textype, flags);
		break;
	}
	return NULL;
}
...
CALLERS_ (R_RealGetTexture R_GetCurrentTexture)

RELATED_ (the callback is VideoUpdateCallback )
...
void R_FreeTexture(rtexture_t *rt)
{
	...
	glt = (gltexture_t *)rt;
	if (glt == NULL)
		Host_Error_Line ("R_FreeTexture: texture == NULL");
...
//void 
RELATED_ (CL_Color_c SV_ProtocolName_c Host_Framerate_c);
...
void R_TextureStats_Print(qbool printeach, qbool printpool, qbool printtotal)
{
	...
	for (pool = gltexturepoolchain;pool;pool = pool->next)
	{
		...
		if (printpool)
			Con_Printf ("texturepool %10p total: %d (%.3fMB, %.3fMB original), uploaded %d (%.3fMB, %.3fMB original), upload on demand %d (%.3fMB, %.3fMB original)\n", (void *)pool, pooltotal, pooltotalt / 1048576.0, pooltotalp / 1048576.0, poolloaded, poolloadedt / 1048576.0, poolloadedp / 1048576.0, pooltotal - poolloaded, (pooltotalt - poolloadedt) / 1048576.0, (pooltotalp - poolloadedp) / 1048576.0);
		sumtotal += pooltotal;
	...
	if (printtotal)
		Con_Printf ("textures total: %d (%.3fMB, %.3fMB original), uploaded %d (%.3fMB, %.3fMB original), upload on demand %d (%.3fMB, %.3fMB original)\n", sumtotal, sumtotalt / 1048576.0, sumtotalp / 1048576.0, sumloaded, sumloadedt / 1048576.0, sumloadedp / 1048576.0, sumtotal - sumloaded, (sumtotalt - sumloadedt) / 1048576.0, (sumtotalp - sumloadedp) / 1048576.0);
}
...
static void R_MakeResizeBufferBigger(int size)
{
	if (resizebuffersize < size)
	{
		...
		if (!resizebuffer || !colorconvertbuffer)
			Host_Error_Line ("R_Upload: out of memory");
	}
}
...
CALLERS_		(R_UpdateTexture R_ClearTexture R_SetupTexture)
WARP_X_DOWNSTREAM_	(qglTexImage2D)
static void R_UploadFullTexture(gltexture_t *glt, const unsigned char *data)
{
	...
	if (glt->texturetype != GLTEXTURETYPE_2D && (glt->tilewidth != glt->inputwidth || glt->tileheight != glt->inputheight || glt->tiledepth != glt->inputdepth))
		Sys_Error ("R_UploadFullTexture " QUOTED_S ": stretch uploads allowed only on 2D textures", glt->identifier);
...
//		prevbuffer = resizebuffer;
	...
	case RENDERPATH_GLES2:
		if (glt->texnum) // not renderbuffers
		{
			...
			{
			case GLTEXTURETYPE_2D:
				/*2D*/ qglTexImage2D(GL_TEXTURE_2D, mip++, glt->glinternalformat, width, height, 0, glt->glformat, glt->gltype, prevbuffer);CHECKGLERROR
				if (glt->flags & TEXF_MIPMAP)
				{
					while (width > 1 || height > 1 || depth > 1)
					{
						...
						prevbuffer = resizebuffer;
						/*2D mip*/ qglTexImage2D(GL_TEXTURE_2D, mip++, glt->glinternalformat, width, height, 0, glt->glformat, glt->gltype, prevbuffer);CHECKGLERROR
					}
				}
				break;
			case GLTEXTURETYPE_3D:
			...
			case GLTEXTURETYPE_CUBEMAP:
				...
				for (i = 0;i < 6;i++)
				{
					...
					mip = 0;
					/*cube*/ qglTexImage2D(cubemapside[i], mip++, glt->glinternalformat, width, height, 0, glt->glformat, glt->gltype, prevbuffer);CHECKGLERROR
					if (glt->flags & TEXF_MIPMAP)
					{
						while (width > 1 || height > 1 || depth > 1)
						{
...
CALLERS_		(R_LoadTexture2D)
WARP_X_DOWNSTREAM_	(R_UploadFullTexture to qglTexImage2D)
	...
	textype_t textype, int texturetype, const unsigned char *data, const unsigned int *palette)
{
	...
	if (size < 1) {
		Con_PrintLinef ("R_LoadTexture: bogus texture size (%dx%dx%dx%dbppx%dsides = %d bytes)", width, height, depth, texinfo->inputbytesperpixel * 8, sides, size);
		return NULL;
	}
	...
	R_UploadFullTexture(glt, data); // Baker: R_SetUpTexture --> UPLOAD HERE qglTexImage2D - Baker: LM_OID
...
rtexture_t *R_LoadTextureDDSFile(rtexturepool_t *rtexturepool, const char *filename, qbool srgb, int flags, qbool *hasalphaflag, float *avgcolor, int miplevel, qbool optionaltexture) // DDS textures are opaque, so miplevel isn't a pointer but just seen as a hint
{
	...
	for (mip = 0;mip <= dds_miplevels;mip++) // <= to include the not-counted "largest" miplevel
	{
		...
		case RENDERPATH_GLES2:
			...
			{
				/*DDS*/ qglTexImage2D(GL_TEXTURE_2D, mip, glt->glinternalformat, upload_mipwidth, upload_mipheight, 0, glt->glformat, glt->gltype, upload_mippixels);CHECKGLERROR
			}
			break;
		} // switch
...
void R_UpdateTexture(rtexture_t *rt, const unsigned char *data, int x, int y, int z, int width, int height, int depth, int combine)
{
	...
	if (!glt->texnum)
	{
		Con_DPrintLinef ("R_UpdateTexture: texture %p " QUOTED_S " in pool %p has not been uploaded yet", (void *)glt, glt->identifier, (void *)glt->pool);
		return;
	}
	...
	if (glt->bufferpixels)
	{
		...
		if (glt->inputdepth != 1 || glt->sides != 1)
			Host_Error_Line ("R_UpdateTexture on buffered texture that is not 2D");
		if (x < 0 || y < 0 || z < 0 || glt->tilewidth < x + width || glt->tileheight < y + height || glt->tiledepth < z + depth)
			Host_Error_Line ("R_UpdateTexture on buffered texture with out of bounds coordinates (%d %d %d to %d %d %d is not within 0 0 0 to %d %d %d)", x, y, z, x + width, y + height, z + depth, glt->tilewidth, glt->tileheight, glt->tiledepth);
...
RELATED_ (VideoUpdateCallback  ->updatecallback R_UpdateDynamicTexture)
