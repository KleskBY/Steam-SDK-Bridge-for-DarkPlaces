...
static char command_when_done[256] = ""; // CURLBAKER .. yay globals
static char command_when_error[256] = ""; // CURLBAKER .. yay globals
...
void Curl_Register_predownload(void) // CURLBAKER
{
	if (curl_mutex) Thread_LockMutex(curl_mutex);
...
static unsigned char *decode_image(downloadinfo *di, const char *content_type)
{
	...
	unsigned char *data = FS_LoadFile(di->curlfilename, tempmempool, fs_quiet_true, &filesize);
	if (data)
	{
		int mip = 0;
...
static void Curl_EndDownload(downloadinfo *di, CurlStatus status, CURLcode error, const char *content_type_)
{
	...
	switch(status)
	{
		...
		case CURL_DOWNLOAD_SERVERERROR:
			...
			{
				FS_Close(di->stream);
				di->stream = FS_OpenRealFile(di->curlfilename, "wb", false); // WRITE-EON curl end download
			}
	...
	if (di->stream)
		FS_Close(di->stream); // BAKER_CURL

#define CLEAR_AND_RETRY() \
	do \
	{ \
		di->stream = FS_OpenRealFile(di->curlfilename, "wb", false); \
		FS_Close(di->stream); \
		if (di->startpos && !di->callback) \
		{ \
			Curl_Begin(di->url, di->extraheaders, di->maxspeed, di->curlfilename, di->loadtype, di->forthismap, di->post_content_type, di->postbuf, di->postbufsize, NULL, 0, NULL, NULL); \
			di->forthismap = false; \
		} \
	} \
	while(0)
	...
	{
		ok = FS_AddPack(di->curlfilename, NULL, true, true);
		if (!ok)
			CLEAR_AND_RETRY();
	}
	else if (ok && di->loadtype == LOADTYPE_CACHEPIC)
	{
		...
		p = di->curlfilename;
#ifdef WE_ARE_EVIL // BAKER: Seemingly not defined.
	...
	else if (ok && di->loadtype == LOADTYPE_SKINFRAME)
	{
		...
		pixels = decode_image(di, content_type);
		if (pixels)
			R_SkinFrame_LoadInternalBGRA(p, TEXF_FORCE_RELOAD | TEXF_MIPMAP | TEXF_ALPHA, pixels, image_width, image_height, 0, 0, 0, q_tx_convertsrgb_false, q_is_sky_load_false); // TODO what sRGB argument to put here?
		else
			CLEAR_AND_RETRY();
	}
	...
	if (numdownloads < curl_maxdownloads.integer)
	{
		...
		List_For_Each_Entry(di, &downloads, downloadinfo, list)
		{
			if (!di->started)
			{
				if (!di->buffer)
				{
					Con_Printf ("Downloading %s -> %s", CleanURL(di->url, urlbuf, sizeof(urlbuf)), di->curlfilename);

					di->stream = FS_OpenRealFile(di->curlfilename, "ab", false);
					if (!di->stream)
					{
...
====================
*/
RELATED_ (Curl_ClearRequirements)
...
static downloadinfo *Curl_Find(const char *filename)
{
	...
	if (!curl_dll)
		return NULL;
	List_For_Each_Entry(di, &downloads, downloadinfo, list)
		if (String_Match_Caseless(di->curlfilename, filename))
			return di;
	return NULL;
}
...
RELATED_ (VM_uri_get Curl_EndDownload Curl_Frame)
static qbool Curl_Begin(const char *URL, const char *extraheaders, double maxspeed, const char *name, int loadtype, qbool forthismap, const char *post_content_type, const unsigned char *postbuf, size_t postbufsize, unsigned char *buf, size_t bufsize, curl_callback_t callback, void *cbdata)
{
	if (buf)
		if (loadtype != LOADTYPE_NONE)
			Host_Error_Line ("Curl_Begin: loadtype and buffer are both set");
	...
	else
	{
		...
		strlcpy(di->curlfilename, name, sizeof(di->curlfilename));
...
====================
*/
static double Curl_GetDownloadAmount(downloadinfo *di) // CURLBAKER
{
	if (!curl_dll)
		return -2;
	if (di->curle)
...
static void Curl_Info_f(cmd_state_t *cmd)
{
	...
	if (Curl_Running())
	{
		...
		List_For_Each_Entry(di, &downloads, downloadinfo, list)
		{
			...
			Con_Printf ("  %s -> %s ",  CleanURL(di->url, urlbuf, sizeof(urlbuf)), di->curlfilename);
			percent = 100.0 * Curl_GetDownloadAmount(di);
			speed = Curl_GetDownloadSpeed(di);
			if (percent >= 0)
				Con_Printf ("(%.1f%% @ %.1f KiB/s)\n", percent, speed / 1024.0); // CURLBAKER
			else
				Con_Print("(queued)\n");
		}
		if (curl_mutex) Thread_UnlockMutex(curl_mutex);
...
static void Curl_Curl_f(cmd_state_t *cmd)
{
	...
	for(i = 1; i != end; ++i) {
		...
		else if (String_Match(s_arg, "--finish_autodownload")) { // CURLBAKER
			if (numdownloads_added) {
				char donecommand[256];
				if (cls.netcon) {
					if (cl.loadbegun) { // curling won't inhibit loading the map any more 
						// when at this stage, so bail out and force a reconnect
						...
						Curl_Register_predownload(); // CURLBAKER
				}
			}
			return;
		}
...
Curl_downloadinfo_t *Curl_GetDownloadInfo(int *nDownloads, const char **additional_info, char *addinfo, size_t addinfolength)
{
	...
	List_For_Each_Entry(di, &downloads, downloadinfo, list)
	{
		...
		if (developer.integer <= 0)
			if (di->buffer)
				continue;
		strlcpy(downinfo[i].curfilename, di->curlfilename, sizeof(downinfo[i].curfilename));
		if (di->curle)
		{
...
====================
*/
RELATED_ (SV_SpawnServer  Curl_RequireFile(modelname) )
...
====================
*/
CALLERS_ (SV_SpawnServer, Curl_Shutdown) 
...
CALLERS_ (SV_SendServerinfo exclusive sv_curl_serverpackages <-- cvar ) // Baker: SV_SendServerinfo calls this, host_client = client;
...
RELATED_ (Curl_EndDownload ) // Stops download
