...
RELATED_ (SV_Download_f clc_ackdownloaddata svc_downloaddata -> CL_ParseDownload_DP)

RELATED_ (SV_Zircon_Extensions_Send)
static void CL_BeginDownloads_Prespawn_Zircon_Extensions_Send (void) // @ SIGNON_1 will go to 2
{
	//char vabuf[1024];
	// Do we know protocol yet?
	RELATED_ (SV_PreSpawn_f)
...
RELATED_ (CL_SignonReply SIGNON_1 sends us here)
...
CALLERS_ (QW_CL_ParseDownload yesh)
...
//KILLME Con_PrintLinef ("DPCHUNKS - cl_downloadbegin");
	...
	ccs *sfile = Cmd_Argv(cmd, 2);
	if (size < 0 || size > 1<<30 || FS_CheckNastyPath(sfile, fs_isgamedir_false)) {
