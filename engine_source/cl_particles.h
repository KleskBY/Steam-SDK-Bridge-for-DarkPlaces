...
extern struct cvar_s developer_pointparticles;
...
typedef enum
{
	PORIENT_BILLBOARD_0 = 0,
	PORIENT_SPARK = 1,
	PORIENT_ORIENTED_DOUBLESIDED = 2,
	PORIENT_VBEAM = 3,
	PORIENT_HBEAM = 4,
	PORIENT_UPRIGHT = 5,	// Baker: Facing viewer upright always no rotation? spin?  SPRITELY1
	PORIENT_INVALID = -1
}
porientation_t;
...
// New effect info field .. "string"
#define PAIRED_PARTICLE_FLAG_128		128 
#define PAIRED_PARTICLE_REMOVE_MASK_127	127
...
typedef struct particle_s
{
	...
	// Baker: By "groups by effect" .. effect means blendmode mostly.
...
//	float           delayedcollisions; // time that p->bounce becomes active
	...
	short			pangle; // base rotation of particle
	short			pspin; // geometry rotation speed around the particle center normal
	...
	RELATED_ (ptype_e)
	unsigned char   ptypeindex;			// Baker: This is one of the 12 particle types like pt_smoke
	unsigned char   pblendmode;
	unsigned char   porientation;
	unsigned char   ptexnum;			// Baker: We are locked to a max of 256 textures
...
int GetEffectList_Count (const char *s_prefix); // Particle effects
...
ccs *CL_Particles_EffectIndex_To_Name (int index); // Baker: March 23 2025 - returns "*UNKNOWN*" on failure.
