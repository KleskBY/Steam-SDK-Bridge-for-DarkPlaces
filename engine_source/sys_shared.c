...
{
	// Baker: DarkPlaces Beta has rewritten this to check for packets while sleeping.
	double t = 0;
	if (sys_usenoclockbutbenchmark.integer /*d:0*/)
	{
		if (microseconds)
		{
			double old_benchmark_time = benchmark_time;
	...
	if (sys_debugsleep.integer /*d:0*/)
	{
		t = Sys_DirtyTime();
	}
	if (sys_supportsdlgetticks && sys_usesdldelay.integer /*d:0*/)
	{
		Sys_SDL_Delay(microseconds / 1000);
	}
#if HAVE_SELECT // Baker: defined as 1 ONLY for Non-Windows
	else
	{
		struct timeval tv;
		tv.tv_sec = microseconds / 1000000;
		tv.tv_usec = microseconds % 1000000;
		select(0, NULL, NULL, NULL, &tv);  // Baker: Linux NORMAL
	}
#elif HAVE_USLEEP // Baker: defined as 1 ONLY for Non-Windows
	else
	{
		usleep(microseconds); // Baker: NOTHING SHOULD USE THIS
	}
#elif HAVE_Sleep // Baker: This IS defined for Windows
	else
	{
		Sleep(microseconds / 1000); // Windows normal
	}
#else
	else
	{
...
CALLERS_ (Exclusive: main plus also SDL_main)
