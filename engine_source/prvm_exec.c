...
void PRVM_PrintFunctionStatements (prvm_prog_t *prog, const char *name)
{
	...
	if (!func)
	{
		Con_PrintLinef ("%s progs: no function named %s", prog->name, name);
		return;
	}
...
void PRVM_Crash(prvm_prog_t *prog)
{
	...
	if ( prog->depth > 0 ) {
		Con_PrintLinef ("QuakeC crash report for %s:", prog->name);
		PRVM_PrintState(prog, 0);
	}

	if (prvm_errordump.integer) {
		// make a savegame
		SV_Savegame_to(prog, q_siv_savestring_NULL, va(vabuf, sizeof(vabuf), "crash-%s.dmp", prog->name), q_is_siv_write_false, /*totaltimeatlastexit*/ 0);
	}
