...
cvar_t vid_fullscreen_height = {CF_CLIENT | CF_ARCHIVE, "_vid_fullscreen_height", "768", "most recent user set fullscreen height for ALT-ENTER [Zircon]"};
cvar_t vid_window_width = {CF_CLIENT | CF_ARCHIVE, "_vid_window_width", "1200", "most recent user set windowed width for ALT-ENTER [Zircon]"};
cvar_t vid_window_height = {CF_CLIENT | CF_ARCHIVE, "_vid_window_height", "640", "most recent user set windowed height for ALT-ENTER [Zircon]"};
...
cvar_t vid_magnification_factor = {CF_CLIENT | CF_READONLY, "vid_magnification_factor", "0", "Current 360P magnification factor, always a whole number [Zircon]"};
...
skip_spam2:
	...
	if (missingrequiredfuncs)
		Sys_Error ("OpenGL driver/hardware lacks required features:" NEWLINE "%s", missingfuncs);
#endif

	if (vid.restart_count == 0) // Baker r1481: Reduce ALT-ENTER video restart spam
		Con_DPrintLinef ("Checking OpenGL extensions...");
...
void VID_ApplyJoyState(vid_joystate_t *joystate)
{
	...
	if (joystate->is360)
	{
		...
		while (qXInputGetKeystroke && qXInputGetKeystroke(XUSER_INDEX_ANY, 0, &keystroke) == S_OK)
			Con_PrintLinef ("XInput KeyStroke: VirtualKey %d, Unicode %d, Flags %x, UserIndex %d, HidCode %d", keystroke.VirtualKey, keystroke.Unicode, keystroke.Flags, keystroke.UserIndex, keystroke.HidCode);
#endif
...
int VID_Shared_SetJoystick(int index)
{
	...
	if (vid_xinputindex != xinputindex)
	{
		...
		if (xinputindex >= 0)
			Con_PrintLinef ("Joystick %d opened (XInput Device %d)", index, xinputindex);
	}
	return xinputcount;
#else
	return 0;
...
static int VID_Mode(int fullscreen, int width, int height, int bpp, float refreshrate, int stereobuffer)
{
	...
	if (VID_InitMode(&mode))
	{
		...
		if (vid_fullscreen.integer) {
			Cvar_SetValueQuick(&vid_fullscreen_width, width);
			Cvar_SetValueQuick(&vid_fullscreen_height, height);
		} else {
			Cvar_SetValueQuick(&vid_window_width, width);
		...
		if (vid_touchscreen.integer) {
			// Baker: Don't use the cvars
			// although they get synced here
			// and it makes no difference
			// in this instance, it's still wrong
			in_windowmouse_x = vid.mode.width / 2.f;
			in_windowmouse_y = vid.mode.height / 2.f;
		}
...
// COMMANDLINEOPTION: Video: -ydpi <dpi> performs +vid_touchscreen_ydpi <dpi> (example -ydpi 160 or -ydpi 320)
	...
	if (!success) {
		Con_PrintLinef ("Desired video mode fail, trying fallbacks...");
