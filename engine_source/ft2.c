...
void font_start(void)
{
	...
	{
		Con_PrintLinef (CON_ERROR "ERROR: Failed to initialize the FreeType2 library!");
		Font_CloseLibrary();
		return;
	}

	font_mempool = Mem_AllocPool("FontFreeType", 0, NULL);
	if (!font_mempool)
	{
...
static qbool Font_LoadFile (const char *name, int _face, ft2_settings_t *settings, ft2_font_t *font, const byte *ft_data_in, fs_offset_t ft_datasize)
{
	...
	if (false == Font_OpenLibrary()) {
		if (!r_font_disable_freetype.integer /*d: 0*/) {
...
qbool Font_GetKerningForMap(ft2_font_t *font, int map_index, float w, float h, Uchar left, Uchar right, float *outx, float *outy)
{
	...
	else
	{
		...
		{
			// this deserves an error message
			Con_PrintLinef (CON_ERROR "Failed to get kerning for %s\n", font->name);
			return false;
		}
...
void Font_UnloadFont(ft2_font_t *font)
{
...
#ifndef DP_FREETYPE_STATIC // Baker: Unclear whether or not this is defined.  Sept 4 2025 - DEFINED!
	// Baker: Sept 4 2025 - this is the norm in Visual Studio 2008.  Android/iPhone this NOT normal.
	if (ft2_dll)
#else
	// Normal for Android/iPhone
	if (!r_font_disable_freetype.integer)
#endif
	{
		if (font->face) {
	...
	if (font->font_data) {
	    fontfilecache_Free(font->font_data);
	    font->font_data = NULL;
	}
}
...
static float Font_SearchSize(ft2_font_t *font, FT_Face fontface, float size)
{
	...
	while (1)
	{
		...
		{
			Con_PrintLinef (CON_ERROR "ERROR: can't set size for font %s: %f ((%f))", font->name, size, intSize);
			return -1;
		}
		...
		if (intSize < 2)
		{
			Con_PrintLinef (CON_ERROR "ERROR: no appropriate size found for font %s: %f", font->name, size);
			return -1;
		}
		--intSize;
	}
...
static qbool Font_LoadMap(ft2_font_t *font, ft2_font_map_t *mapstart, Uchar _ch, ft2_font_map_t **outmap, short *pascender, short *pdescender, short *pheight)
{
	...
	{
		Con_PrintLinef (CON_ERROR "ERROR: can't set sizes for font %s: %f", font->name, mapstart->size);
		return false;
	}
	...
	if (!map)
	{
		Con_PrintLinef (CON_ERROR "ERROR: Out of memory when loading fontmap for %s", font->name);
		return false;
	}
	     ...
	     ++ch)
	{
		...
		if (w > (map->glyphSize - gpad_l - gpad_r) || h > (map->glyphSize - gpad_t - gpad_b)) {
			Con_PrintLinef (CON_WARN "WARNING: Glyph %lu is too big in font %s, size %g: %d x %d", ch, font->name, map->size, w, h);
			if (w > map->glyphSize)
...

	{
		...
		map->pic = Draw_NewPic(map_identifier, w, h, data, r_font_use_alpha_textures.integer /*d:0*/ ? TEXTYPE_ALPHA : TEXTYPE_RGBA, TEXF_ALPHA | TEXF_CLAMP | (r_font_compress.integer > 0 ? TEXF_COMPRESS : 0));
	...
	if (data) // Baker: CHARSETDATA HERE
		Mem_Free(data);
	...
	if (!Draw_IsPicLoaded(map->pic))
	{
		...
		Con_PrintLinef (CON_ERROR "ERROR: Failed to generate texture for font %s size %f map %lu",
