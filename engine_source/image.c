...
void Image_StripImageExtension (const char *in, char *out, size_t size_out)
{
	...
	if (ext && String_Isin7 (ext, "tga", "pcx", "lmp", "png", "jpg", "jpeg", "wal"))
		//(String_Match(ext, "tga") || String_Match(ext, "pcx") ||
		//String_Match(ext, "lmp") || String_Match(ext, "png") || String_Match(ext, "jpg") || String_Match(ext, "wal")))
...
imageformat_t imageformats_nopath[] =
{
	...
	{"textures/%s.jpeg", JPEG_LoadImage_BGRA}, // JPEGSTER imageformats_nopath
	{"textures/%s.jpg", JPEG_LoadImage_BGRA},
	{"%s.tga", LoadTGA_BGRA},
	{"%s.png", PNG_LoadImage_BGRA},
	{"%s.jpeg", JPEG_LoadImage_BGRA},
	{"%s.jpg", JPEG_LoadImage_BGRA},
	{"%s.pcx", LoadPCX_BGRA},
	{NULL, NULL}
};
...
imageformat_t imageformats_textures[] =
{
	{"%s.tga", LoadTGA_BGRA},
	{"%s.png", PNG_LoadImage_BGRA},
	{"%s.jpeg", JPEG_LoadImage_BGRA}, // JPEGSTER imageformats_textures
	{"%s.jpg", JPEG_LoadImage_BGRA},
	{"%s.pcx", LoadPCX_BGRA},
	{"%s.wal", LoadWAL_BGRA},
	{NULL, NULL}
...
imageformat_t imageformats_textures_nopng[] =
{
	{"%s.tga", LoadTGA_BGRA},
//	{"%s.png", PNG_LoadImage_BGRA},
	{"%s.jpeg", JPEG_LoadImage_BGRA}, // JPEGSTER imageformats_textures
...
imageformat_t imageformats_gfx[] =
{
	{"%s.tga", LoadTGA_BGRA},
	{"%s.png", PNG_LoadImage_BGRA},
	{"%s.jpeg", JPEG_LoadImage_BGRA},  // JPEGSTER imageformats_gfx
	{"%s.jpg", JPEG_LoadImage_BGRA},
	{"%s.pcx", LoadPCX_BGRA},
	{"%s.lmp", LoadLMP_BGRA},
	{NULL, NULL}
...
imageformat_t imageformats_other[] =
{
	{"%s.tga", LoadTGA_BGRA},
	{"%s.png", PNG_LoadImage_BGRA},
	{"%s.jpeg", JPEG_LoadImage_BGRA},  // JPEGSTER imageformats_other
	{"%s.jpg", JPEG_LoadImage_BGRA},
	{"%s.pcx", LoadPCX_BGRA},
	{"%s.lmp", LoadLMP_BGRA},
	{NULL, NULL}
...
// Baker: This zallocs if it doesn't return NULL
unsigned char *loadimagepixelsbgra (ccs *filename, qbool complain, qbool allowFixtrans, qbool convertsRGB, int *miplevel) // LoadTGA_BGRA
{
	...
	if (developer_texturelogging.integer)
		Log_Printf("textures.log", "%s" NEWLINE, filename);
	...
	// Baker: The difference between lists is .LMP is in the textures one.
	// Let's be honest ... Vortex added the one for DeluxeQuake to hide textures from
	// his map editor and DeluxeQuake was never released.
	if (gamemode == GAME_TENEBRAE)
		firstformat = imageformats_tenebrae; // Baker: Never
#if 0
	else if (gamemode == GAME_DELUXEQUAKE)
		firstformat = imageformats_dq; // Baker: DeluxeQuake seems to be unreleased 2008 TastySpeen QExpo by Vortex
#endif
	else if (String_Match_Caseless(path, "textures"))
		if (png_texture_suppress.integer)
			firstformat = imageformats_textures_nopng;
		else
		...
		firstformat = imageformats_nopath; // Baker: Can this happen?
	else
		if (png_texture_suppress.integer)
			firstformat = imageformats_textures_nopng;
		else
			firstformat = imageformats_other; // Baker: This is norm for models.
				...
				if (format->loadfunc == JPEG_LoadImage_BGRA) // jpeg can't do alpha, so let's simulate it by loading another jpeg
				{
					...
					if (f)
					{
						...
						if (data2 && mymiplevel == mymiplevel2 && image_width == image_width_save && image_height == image_height_save)
							...
							Con_PrintLinef ("loadimagepixelsrgba: corrupt or invalid alpha image %s_alpha", basename);
					...
					else if (String_Ends_With (format->formatstring, "jpg")) {
#if 1
						// #1 No alpha for .JPG (notice not JPEG)
						char JPEG_formatstring[MAX_QPATH_128];
						c_strlcpy (JPEG_formatstring, format->formatstring);
						String_Edit_Replace_Memory_Constant (JPEG_formatstring, sizeof(JPEG_formatstring), ".jpg", ".jpeg");

						dpsnprintf (name2, sizeof(name2), JPEG_formatstring, va(vabuf, sizeof(vabuf), "%s_alpha", basename));
						f = FS_LoadFile(name2, tempmempool, fs_quiet_true, &filesize);
						if (f)
						{
							int mymiplevel2 = miplevel ? *miplevel : 0;
					...
					// #2 - No _alpha for .jpeg (Notice the "e")
					else if (String_Ends_With (format->formatstring, "jpeg")) {
#if 1
						// Baker: If .jpg does not find _alpha texture we must try .JPG also.
						char JPG_formatstring[MAX_QPATH_128];
						c_strlcpy (JPG_formatstring, format->formatstring);
						String_Edit_Replace_Memory_Constant (JPG_formatstring, sizeof(JPG_formatstring), ".jpeg", ".jpg");

						dpsnprintf (name2, sizeof(name2), JPG_formatstring, va(vabuf, sizeof(vabuf), "%s_alpha", basename));
						f = FS_LoadFile(name2, tempmempool, fs_quiet_true, &filesize);
						if (f)
						{
							int mymiplevel2 = miplevel ? *miplevel : 0;
