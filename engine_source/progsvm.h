...
// Baker: Feb 18 2025 I assume this is used for VM general where neither 
// client nor server (like either)
...
#if 1
#define PRVM_EDICTFIELDVALUE(ed, fieldoffset)    ((fieldoffset) < 0 ? Con_Printf ("Invalid fieldoffset at %s:%d\n", __FILE__, __LINE__), &prvm_badvalue : (prvm_eval_t *)((ed)->fields.fp + (fieldoffset)))
...
#define PRVM_GLOBALFIELDVALUE(fieldoffset)       ((fieldoffset) < 0 ? Con_Printf ("Invalid fieldoffset at %s:%d\n", __FILE__, __LINE__), &prvm_badvalue : (prvm_eval_t *)(prog->globals.fp + (fieldoffset)))
...
#define PRVM_OP_STATE		1
	...
	#define PRVM_MAX_OPENFILES_256 16
	#define PRVM_MAX_OPENSEARCHES_128 8
#else
	// Baker: This is the norm
	#define	PRVM_MAX_STACK_DEPTH		1024
	#define	PRVM_LOCALSTACK_SIZE		16384
	
	#define PRVM_MAX_OPENFILES_256		256
	#define PRVM_MAX_OPENSEARCHES_128	128
	#define PRVM_MAX_OPEN_DOT_MAPS_8	8
	#define PRVM_MAX_OPEN_FORMS_8		8
	#define PRVM_MAX_INVENTORY_8		8 // LATER
#endif
...
//		cl_globalvars_t *client;
	...
	struct qfile_s		*openfiles[PRVM_MAX_OPENFILES_256];
	const char *         openfiles_origin[PRVM_MAX_OPENFILES_256];
	struct fssearch_s	*opensearches[PRVM_MAX_OPENSEARCHES_128];
	const char *         opensearches_origin[PRVM_MAX_OPENSEARCHES_128];

	entitylist_t		*open_dot_maps[PRVM_MAX_OPEN_DOT_MAPS_8]; // Baker: ZIRCON_DUNGEON_V1
	sys_handle_t			process_handle;
	int						process_index; // Ignore if process_handle is non-zero

	void /*oject_s*/	*forms[PRVM_MAX_OPEN_FORMS_8]; // Baker: ZIRCON_FORMS_V1
	stringlist_t		invent[PRVM_MAX_OPEN_FORMS_8];
	int					invent_num_columns[PRVM_MAX_OPEN_FORMS_8];
	RELATED_ (VM_Zircon_API_Shutdown_Reset FS_Inventory_f)
	
	//int				form_callbacks[PRVM_MAX_OPEN_FORMS_8]; // Baker: ZIRCON_FORMS_V1
...
char *PRVM_Gen_Progs_Commad (void); // Baker: Returns a string like "client,server" indicating running progs
#endif
...
// Baker: prvm_edictget client 23 "absmin"
qbool PRVM_ED_EdictGet_Fly (ccs *s_progname, ccs *s_edict_num, ccs *s_fieldname, char *valuebuf, size_t valuebuf_size);
qbool PRVM_ED_EdictGet_Fly_Lite (prvm_prog_t *prog, int edict_num, ccs *s_fieldname, char *valuebuf, size_t valuebuf_size);

#define PRVM_ED_EdictGet_Fly_LiteX(prog,edict_num,s_fieldname,_valuebuf) \
	PRVM_ED_EdictGet_Fly_Lite (prog, edict_num, s_fieldname, _valuebuf, sizeof(_valuebuf)) // Ender
...
extern cvar_t ent_create_tracepct;
