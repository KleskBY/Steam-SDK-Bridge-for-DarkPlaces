...
// CALLERS_ (CL_ClientMovement_PlayerMove cl.scores[cl.playerentity-1].qw_spectator QW_CL_ProcessUserInfo)

// FTE sends the extensions for Quakeworld in the challenge QW response
//
RELATED_(QW_TREAT_AS_SPECTATOR)


RELATED_ (CL_ClientMovement_Physics_Swim, CL_ClientMovement_PlayerMove cl.scores[cl.playerentity-1].qw_spectator)

RELATED_ (CL_ClientMovement_Move)
static void CL_ClientMovement_Move_PM_SpectatorMove (cl_clientmovement_state_t *s)
{
	int bump;
	double t;
	vec_t f;
	// Dec 22 2024 ... unused ... vec3_t neworigin;
	...
	for (bump = 0, t = s->cmd.clx_frametime; bump < 8 && VectorLength2(s->velocity) > 0; bump++) {
		// Dec 22 2024 ... unused ... VectorMA(s->origin, t, s->velocity, neworigin);
...
void CL_UpdateMoveVars(void)
{
	...
	{
		cl.moveflags = cl.stats_sv[STAT_MOVEFLAGS]; // Baker: MOVEFLAG_NOGRAVITYONGROUND and such
