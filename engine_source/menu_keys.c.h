...
void M_Menu_Keys_f(cmd_state_t *cmd)
{
	...
	} else if (gamemode_is_galaxy) {
		local_count = ARRAY_COUNT(zirconbindnames);
		bindnames = zirconbindnames; // GALAXY #1
	} else  {
...
static void M_Keys_Draw (void)
{
	...
	M_DrawPic ( (320-Draw_GetPicWidth(p0))/2, 4, "gfx/ttl_cstm", HOTSPOTS_NONE_0, NA0, NA0);
		...
		if (keys[0] == -1) {
			...
			for (j = 0;j < NUMKEYS;j++) {
				if (keys[j] != -1) {
					...
					if (gamemode_is_galaxy && s == tinystr) { // GALAXY #2
