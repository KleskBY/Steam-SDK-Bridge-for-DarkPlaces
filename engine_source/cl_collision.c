...
float CL_SelectTraceLine(const vec3_t start, const vec3_t end, vec3_t impact, vec3_t normal, int *hitent, entity_render_t *ignoreent)
{
	...
	for (n = 0;n < cl.num_entities;n++)
	{
		...
		if (!(cl.entities[n].state_current.effects & EF_SELECTABLE) && (ent->ralpha < 1 || (ent->ef_effects & (EF_ADDITIVE_32 | EF_NODEPTHTEST))))
			continue;
		if (ent == ignoreent)
			continue;
	...
	int *hitnetworkentity, qbool hitcsqcentities)
{
	...
	if (hitnetworkbrushmodels) {
		for (i = 0;i < cl.num_brushmodel_entities;i++) {
			...
			Collision_CombineTraces(&cliptrace, &trace, NULL, /*isbrushmodel*/ true);
		}
	}
