...
//			return SUPERCONTENTS_SOLID /*1*/ | SUPERCONTENTS_BODY /*32*/ | SUPERCONTENTS_CORPSE /*64*/ | SUPERCONTENTS_LIQUIDSMASK /*14*/;
		...
		else if (PRVM_serveredictfloat(passedict, solid) == SOLID_CORPSE_5)
...
trace_t SV_TracePoint(const vec3_t start, int type, prvm_edict_t *passedict, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask)
{
	...
	for (i = 0;i < numtouchedicts;i++)
	{
		...
		if (PRVM_serveredictfloat(touch, solid) < SOLID_BBOX_2)
			continue;
		if (type == MOVE_NOMONSTERS_1 && PRVM_serveredictfloat(touch, solid) != SOLID_BSP_4)
			continue;
		...
		if (passedict)
		{
			...
			if (VectorCompare(PRVM_serveredictvector(touch, mins), PRVM_serveredictvector(touch, maxs)) && (type != MOVE_MISSILE_2 || !((int)PRVM_serveredictfloat(touch, flags) & FL_MONSTER_32)))
				continue;
		}

		bodysupercontents = PRVM_serveredictfloat(touch, solid) == SOLID_CORPSE_5 ? SUPERCONTENTS_CORPSE : SUPERCONTENTS_BODY;

		// might interact, so do an exact clip
		model = NULL;
		if ((int) PRVM_serveredictfloat(touch, solid) == SOLID_BSP_4 || type == MOVE_HITMODEL_4)
		{
		...
		if (type == MOVE_MISSILE_2 && (int)PRVM_serveredictfloat(touch, flags) & FL_MONSTER_32)
		...
		Collision_CombineTraces(&cliptrace, &trace, (void *)touch, PRVM_serveredictfloat(touch, solid) == SOLID_BSP_4);
	}
...
trace_t SV_TraceLine(const vec3_t start, const vec3_t end, int type, prvm_edict_t *passedict, int hitsupercontentsmask, 
					 int skipsupercontentsmask, int skipmaterialflagsmask, float extend)
{
	prvm_prog_t *prog = SVVM_prog;
	int i, bodysupercontents;
	int passedictprog;
	...
	for (i = 0;i < numtouchedicts;i++)
	{
		...
		if (passedict)
		{
			...
			if (clipgroup && clipgroup == (int)PRVM_serveredictfloat(touch, clipgroup))
				continue;
			// don't clip points against points (they can't collide)
			if (pointtrace && VectorCompare(PRVM_serveredictvector(touch, mins), PRVM_serveredictvector(touch, maxs)) && (type != MOVE_MISSILE_2 || !((int)PRVM_serveredictfloat(touch, flags) & FL_MONSTER_32)))
				continue;
		}
...
int SV_PointSuperContents(const vec3_t point)
{
	...
	for (i = 0;i < numtouchedicts;i++)
	{
		...
		// we only care about SOLID_BSP_4 for pointcontents
...
void SV_LinkEdict_TouchAreaGrid(prvm_edict_t *ent)
{
	...
	if (PRVM_serveredictfloat(ent, solid) == SOLID_NOT_0)
		return;
	...
	{
		touch = touchedicts[i];
		if (touch != ent && (int)PRVM_serveredictfloat(touch, solid) == SOLID_TRIGGER_1 && PRVM_serveredictfunction(touch, touch))
		{
...
static int SV_TestEntityPosition (prvm_edict_t *ent, vec3_t offset)
{
	...
	VectorCopy(PRVM_serveredictvector(ent, mins), entmins);
	VectorCopy(PRVM_serveredictvector(ent, maxs), entmaxs);
	trace = SV_TraceBox(org, entmins, entmaxs, entorigin, ((PRVM_serveredictfloat(ent, movetype) == MOVETYPE_FLY_WORLDONLY_33) ? MOVE_WORLDONLY_3 : MOVE_NOMONSTERS_1), ent, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask, collision_extendmovelength.value);
...
#define EPSILON (1.0f / 32.0f)
	...
	if (VectorDistance2(trace.endpos, PRVM_serveredictvector(ent, origin)) >= 0.0001)
	{
		...
		trace = SV_TraceBox(org, entmins, entmaxs, org, MOVE_NOMONSTERS_1, ent, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask, collision_extendmovelength.value);
		if (trace.startsolid)
...
static void SV_Impact (prvm_edict_t *e1, trace_t *trace)
{
	...
	if (!e1->free && !e2->free && PRVM_serveredictfunction(e1, touch) && PRVM_serveredictfloat(e1, solid) != SOLID_NOT_0)
	{
		PRVM_serverglobalfloat(time) = sv.time;
	...
	if (!e1->free && !e2->free && PRVM_serveredictfunction(e2, touch) && PRVM_serveredictfloat(e2, solid) != SOLID_NOT_0)
	{
		PRVM_serverglobalfloat(time) = sv.time;
...
static int SV_FlyMove (prvm_edict_t *ent, float time, qbool applygravity, float *stepnormal, int hitsupercontentsmask, int skipsupercontentsmask, int skipmaterialflagsmask, float stepheight)
{
	...
	vec3_t dir, push, planes[MAX_CLIP_PLANES_5];
...
#if 0 // Baker: to debug/watch to catch change in onground
...
//		(int)PRVM_serveredictfloat(ent, flags);
	...
	if (applygravity) {
		...
		// sv_gameplayfix_nogravityonground defaults 0, although objectn uses 1
	...
	for (bumpcount = 0; bumpcount < MAX_CLIP_PLANES_5; bumpcount++) {
		...
		if (!SV_PushEntity(&trace, ent, push, /*dolink?*/ false, /*checkstuck*/ true)) {
...
#endif
		{
			...
			return (BLOCKED_FLOOR_1 | BLOCKED_STEP_2) /*3*/;
		}
		...
		else if (stepheight)
		{
			...
			if (!SV_PushEntity(&steptrace, ent, steppush, /*dolink?*/ false, /*checkstuck*/ true)) {
				blocked |= BLOCKED_TELEPORT_8 /*8*/;
				break;
			}
			...
			if (!SV_PushEntity(&steptrace2, ent, push, /*dolink?*/ false, /*checkstuck*/ true)) {
				blocked |= BLOCKED_TELEPORT_8 /*8*/;
				break;
			}
			//Con_Printf ("%f %f %f : ", PRVM_serveredictvector(ent, origin)[0], PRVM_serveredictvector(ent, origin)[1], PRVM_serveredictvector(ent, origin)[2]);
			VectorSet(steppush, 0, 0, org[2] - PRVM_serveredictvector(ent, origin)[2]);
			if (!SV_PushEntity(&steptrace3, ent, steppush, /*dolink?*/ false, /*checkstuck*/ true)) {
				blocked |= BLOCKED_TELEPORT_8 /*8*/;
				break;
			}
			//Con_Printf ("%f %f %f : ", PRVM_serveredictvector(ent, origin)[0], PRVM_serveredictvector(ent, origin)[1], PRVM_serveredictvector(ent, origin)[2]);
...
//#else
		if (sv_gameplayfix_fiendjumpfix.integer) {
			...
			if (PRVM_serveredictfloat(ent, solid) >= SOLID_TRIGGER_1 && trace.ent)
				SV_Impact(ent, &trace);
			if (ent->free)
...
//#endif
		...
		if (numplanes >= MAX_CLIP_PLANES_5) {
		...
		*/
		...
		else
		{
			...
			{
				VectorClear(PRVM_serveredictvector(ent, velocity));
				blocked = BLOCKED_FLOOR_1 | BLOCKED_STEP_2 | BLOCKED_OTHER_4 /*7*/;
				break;
			}
	...
	//Con_Printf ("entity %d final: blocked %d velocity %f %f %f\n", ent - prog->edicts, blocked, PRVM_serveredictvector(ent, velocity)[0], PRVM_serveredictvector(ent, velocity)[1], PRVM_serveredictvector(ent, velocity)[2]);
	...
	if ((blocked & 1) == 0 && bumpcount > 1)
	{
		...
		trace = SV_TraceBox(PRVM_serveredictvector(ent, origin), PRVM_serveredictvector(ent, mins), PRVM_serveredictvector(ent, maxs), end, MOVE_NORMAL_0, ent, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask);
	}
	*/
...
static qbool SV_NudgeOutOfSolid_PivotIsKnownGood(prvm_edict_t *ent, vec3_t pivot)
{
	...
	for (bump = 0;bump < 6;bump++)
	{
		...
		for(subbump = 0; ; ++subbump)
		{
			...
			stucktrace = SV_TraceBox(stuckorigin, goodmins, goodmaxs, testorigin, MOVE_NOMONSTERS_1, ent, SV_GenericHitSuperContentsMask(ent), 0, 0, collision_extendmovelength.value);
...
qbool SV_NudgeOutOfSolid(prvm_edict_t *ent)
{
	...
	for (pass = 0;pass < 2;pass++)
	{
		...
		{
			stucktrace = SV_TraceBox(stuckorigin, stuckmins, stuckmaxs, stuckorigin, pass ? MOVE_WORLDONLY_3 : MOVE_NOMONSTERS_1, ent, SV_GenericHitSuperContentsMask(ent), 0, 0, collision_extendmovelength.value);
...
============
*/
// Baker: Returns true if move failed
RELATED_ (SV_WalkMove)
static qbool SV_PushEntity (trace_t *trace, prvm_edict_t *ent, vec3_t push, qbool dolink, qbool checkstuck)
{
	prvm_prog_t *prog = SVVM_prog;
	int solid;
	...
	if (sv_gameplayfix_nudgeoutofsolid.integer /*0*/ && sv_gameplayfix_nudgeoutofsolid_separation.value >= 0) {
	...
	if (movetype == MOVETYPE_FLYMISSILE_9)
		...
		type = MOVE_WORLDONLY_3;
	else if (solid == SOLID_TRIGGER_1 || solid == SOLID_NOT_0)
		type = MOVE_NOMONSTERS_1; // only clip against bmodels
	else
		type = MOVE_NORMAL_0;
...
static void SV_PushMove (prvm_edict_t *pusher, float movetime)
{
	...
	case SOLID_CORPSE_5: // LadyHavoc: this would be weird...
		break;
	// LadyHavoc: no collisions
	case SOLID_NOT_0:
	case SOLID_TRIGGER_1:
		...
		return;
	default:
		Con_Printf ("SV_PushMove: entity #%d, unrecognized solid type %f\n", PRVM_NUM_FOR_EDICT(pusher), PRVM_serveredictfloat(pusher, solid));
		return;
	}
...
// see if any solid entities are inside the final position
	...
	for (e = 0;e < numcheckentities;e++) {
		...
		if (!((int)PRVM_serveredictfloat(check, flags) & FL_ONGROUND_512) || PRVM_PROG_TO_EDICT(PRVM_serveredictedict(check, groundentity)) != pusher)
		{
			...
			//trace = SV_TraceBox(PRVM_serveredictvector(check, origin), PRVM_serveredictvector(check, mins), PRVM_serveredictvector(check, maxs), PRVM_serveredictvector(check, origin), MOVE_NOMONSTERS_1, check, checkcontents);
			if (!trace.startsolid) {
		...
		// Baker: edict 1 is player 1 or first player
		if (in_range (1, moved_edicts[num_moved], svs.maxclients)) {
			int client_idnum = moved_edicts[num_moved] - 1;
			client_t *hcl = &svs.clients[client_idnum];
			SV_PhysicsX_Zircon_Warp_Start (hcl, "pushed");
		}
		...
		PRVM_serveredictfloat(pusher, solid) = SOLID_NOT_0;
#if 111 // Baker r0061: Classic DarkPlaces physics
...
#endif // 111
		{
			...
			PRVM_serveredictfloat(pusher, solid) = savesolid; // was SOLID_BSP_4
		...
		if (trace.startsolid)
		{
...
#endif // Just make sure we check stuck
				{
					// entity "check" got teleported
					continue;
				}
			...
			if (PRVM_serveredictvector(check, mins)[0] == PRVM_serveredictvector(check, maxs)[0])
				continue;
			if (isin2(PRVM_serveredictfloat(check, solid), SOLID_NOT_0, SOLID_TRIGGER_1)) {
				// corpse
				if (sv_gameplayfix_nosquashentities.integer == 0) {
					// is crushed by a mover, preventing it from being interacted with again
...
qbool SV_UnstickEntity (prvm_edict_t *ent)
{
	...
	switch(SV_UnstickEntityReturnOffset(ent, offset))
	{
		...
		case UNSTICK_UNSTUCK:
			Con_DPrintf ("Unstuck entity %d (classname " QUOTED_S ") with offset %f %f %f.\n", (int)PRVM_EDICT_TO_PROG(ent), PRVM_GetString(prog, PRVM_serveredictstring(ent, classname)), offset[0], offset[1], offset[2]);
			return true;
		case UNSTICK_STUCK:
			if (developer_extra.integer)
				Con_DPrintf ("Stuck entity %d (classname " QUOTED_S ").\n", (int)PRVM_EDICT_TO_PROG(ent), PRVM_GetString(prog, PRVM_serveredictstring(ent, classname)));
			return false;
		default:
...
static void SV_CheckStuck (prvm_edict_t *ent)
{
	...
	switch(SV_UnstickEntityReturnOffset(ent, offset))
	{
		case UNSTICK_GOOD:
			...
			break;
		case UNSTICK_UNSTUCK:
			Con_DPrintf ("Unstuck player entity %d (classname " QUOTED_S ") with offset %f %f %f.\n", (int)PRVM_EDICT_TO_PROG(ent), PRVM_GetString(prog, PRVM_serveredictstring(ent, classname)), offset[0], offset[1], offset[2]);
			break;
		case UNSTICK_STUCK:
			VectorSubtract(PRVM_serveredictvector(ent, oldorigin), PRVM_serveredictvector(ent, origin), offset);
			if (!SV_TestEntityPosition(ent, offset))
			{
				Con_DPrintf ("Unstuck player entity %d (classname " QUOTED_S ") by restoring oldorigin.\n", (int)PRVM_EDICT_TO_PROG(ent), PRVM_GetString(prog, PRVM_serveredictstring(ent, classname)));
				SV_LinkEdict(ent);
				//SV_LinkEdict_TouchAreaGrid(ent);
			}
			else
				Con_DPrintf ("Stuck player entity %d (classname " QUOTED_S ").\n", (int)PRVM_EDICT_TO_PROG(ent), PRVM_GetString(prog, PRVM_serveredictstring(ent, classname)));
			break;
		default:
			Con_Printf ("SV_UnstickEntityReturnOffset returned a value outside its enum.\n");
	}
...
static void SV_WalkMove (prvm_edict_t *ent)
{
	...
	clip = SV_FlyMove (ent, sv.frametime, applygravity, /*stepnormal*/ NULL, hitsupercontentsmask, skipsupercontentsmask, skipmaterialflagsmask, sv_gameplayfix_stepmultipletimes.integer ? sv_stepheight.value : 0);

	if (sv_gameplayfix_downtracesupportsongroundflag.integer) {
		if (!(clip & BLOCKED_FLOOR_1))
		{
			// only try this if there was no floor in the way in the trace (no,
			// this check seems to be not REALLY necessary, because if clip & BLOCKED_FLOOR_1,
			...
			{
				clip |= BLOCKED_FLOOR_1; // but we HAVE found a floor
	...
	if (Have_Flag(clip, BLOCKED_TELEPORT_8)) // teleport
		return;
	...
	if (Have_Flag (clip, BLOCKED_STEP_2))
	{
...
#if 111 // Baker r0061: Classic DarkPlaces physics
		if (!SV_PushEntity(&trace, ent, upmove, /*dolink?*/ true, /*checkstuck*/ true))
#endif // 111
		{
		...
		if (Have_Flag (clip, BLOCKED_TELEPORT_8)) {
			// we got teleported when upstepping... must abort the move
...
#if 1
	qbool is_csqc_ent = false;
	vec3_t oldorg, oldang;
	prvm_vec_t *porg, *pang;
	
	if (PRVM_serveredictfunction(ent, SendEntity)) {
		is_csqc_ent = true;
		
		// CSQC Entity with MOVETYPE_FOLLOW ... check aiment velocity and avelocity .. if either
		porg = PRVM_serveredictvector(ent, origin);
		pang = PRVM_serveredictvector(ent, angles);
		VectorCopyDestSrc (oldorg, porg);
		VectorCopyDestSrc (oldang, pang);
	} // SendEntity
#endif
...
#if 1
	if (is_csqc_ent && (!VectorCompare(porg, oldorg) || !VectorCompare(pang, oldang))) { // Baker: mind the parens - this crashed without them fool!
		#define SENDFLAGS_ALL_FFF_FFF 0xFFFFFF
		// PRVM_serveredictfloat(ent, SendFlags) = 0xFFFFFF; // Yay!  Magic number 0xFFFFFF instead of a #define
		PRVM_serveredictfloat(ent, SendFlags) = ((int)PRVM_serveredictfloat(ent, SendFlags) | sv_movetype_follow_csqc_sendflags.integer);// SENDFLAGS_ALL_FFF_FFF; // Do it right ... no magic numbers.
	}
#endif
...
// move angles
	...
	movetime = sv.frametime;
	for (bump = 0;bump < MAX_CLIP_PLANES_5 && movetime > 0;bump++)
	{
	// move origin
...
void SV_Physics_ClientMove(void)
{
	...
	if (PRVM_serveredictfloat(ent, fixangle)) {
		...
		SV_PhysicsX_Zircon_Warp_Start (host_client, "fix angles");
...
static void SV_Physics_ClientEntity_PostThink(prvm_edict_t *ent)
{
	...
	if (PRVM_serveredictfloat(ent, fixangle)) {
		...
		VectorCopy(PRVM_serveredictvector(ent, angles), host_client->fixangle_angles);
		host_client->fixangle_angles_set = true;
		SV_PhysicsX_Zircon_Warp_Start (host_client, "fixangles_postthink");
...
//
	...
	if (sv_gameplayfix_consistentplayerprethink.integer /*d: 0*/)
	{
