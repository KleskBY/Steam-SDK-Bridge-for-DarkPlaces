...
typedef struct server_static_s{
	...
	RELATED_ (PEXT_CHUNKEDDOWNLOADS)
	...
	// Dungeon Maker API
//	qbool			dungeon_threaded; // true if server is running on separate thread
//	qbool volatile	dungeon_threadstop;
#ifdef _WIN32 // DTHREAD
	void			*dungeon_thread_mutex_lock;
	void			*dungeon_thread_id;

	void			*gifclip2_thread_mutex_lock;
	void			*gifclip2_thread_id;
#endif // _WIN32 // DTHREAD
} server_static_t;
...
typedef struct server_s
{
	...
	RELATED_ (enable_intermap sv_siv_virtual_files_list) // This means save games use intermap
	...
	int zircon_jumpflag_fieldoffset; // Part of Zircon freemove
} server_t;
...
// SERVER:
	...
	RELATED_ (clc_move clc_zircon_move clc_zircon_warp_ack svc_zircon_warp)
...
#define NUM_CSQCENTITYDB_FRAMES 256
	...
	unsigned char stats_cl_deltabits[(MAX_CL_STATS_256+7)/8];
	int stats_cl[MAX_CL_STATS_256];
...
extern cvar_t sv_interactive_flag;
...
RELATED_ (SV_Changelevel_f, SV_Changelevel2_f, SV_Map_f, SV_Restart_f, SV_Loadgame_f)
...
void SV_ClientPrintLinef(const char *fmt_in, ...) DP_FUNC_PRINTF(1); // Baker: May 9 2025.
...
const char *SV_TimingReport(char *buf, size_t buflen); ///< for output in _Status_f
...
extern cvar_t sv_movetype_follow_csqc_sendflags;
