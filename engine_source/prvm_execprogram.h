...
// Baker: This is the norm - PRVMSLOWINTERPRETER
		{
...
// Baker: This is the norm
			...
			HANDLE_OPCODE(OP_DIV_F):
				...
				else
				{
					PRE_ERROR();
					if (developer.value) {
						VM_WarningLinef (prog, "Attempted division of %f by zero", OPA->_float);
					} else {
		...
		//==================
			...
			HANDLE_OPCODE(OP_STORE_S):
				...
				if (prvm_garbagecollection_enable.integer /*d:1*/)
			...
			HANDLE_OPCODE(OP_STOREP_FNC):		// pointers
				if ((prvm_uint_t)OPB->_int - cached_entityfields >= cached_entityfieldsarea_entityfields)
				{
					...
					{
						PRE_ERROR();
						VM_WarningLinef (prog, "Attempted assignment to NULL entity field .%s (%d) in %s", PRVM_GetString(prog, PRVM_ED_FieldAtOfs(prog, OPB->_int)->s_name), (int)OPB->_int, prog->name);
					}
				}
