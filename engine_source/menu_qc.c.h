...
static void MP_Init (void)
{
	...
	// call the prog init
	RELATED_(MVM_ExecuteProgram)
	prog->ExecuteProgram(prog, PRVM_menufunction(m_init),"m_init() required");

	if (PRVM_ED_FindFunction(MVM_prog, "m_drawloading")) {
		Con_DPrintLinef ("Detected m_drawloading");
	}
...
void (*MR_ToggleMenu) (int mode); RELATED_ (M_ToggleMenu)
