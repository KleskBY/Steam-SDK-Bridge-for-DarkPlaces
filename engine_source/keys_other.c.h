...
static void Key_Message (cmd_state_t *cmd, int key, int ascii)
{
	...
	if (in_range(1, ascii, 31) /*ascii > 0 && ascii < 32*/ && utf8_enable.integer)
		ascii = 0xE000 + ascii;
...
#if 1
	newbinding = Mem_strdup (keysmempool, binding);
#else
	l = strlen (binding);
	newbinding = (char *)Z_Malloc (l + 1); // CMDCVARMEM
...
Key_Bind_f(cmd_state_t *cmd)
{
	...
	if (c != 2 && c != 3) {
		...
		Con_PrintLinef ("Try bindlist to list current keys");
		return;
	}
	b = Key_StringToKeynum (Cmd_Argv(cmd, 1));
	if (b == -1 || b >= MAX_KEYS_44032) {
...
void VID_Alt_Enter_f (void) // Baker: ALT-ENTER
{
	...
	if (developer.value) {
		if (!scr_loading_ex && cls.signon == SIGNONS_4 && cls.world_frames && cls.world_start_realtime) {
...
// Baker r0001 - ALT-ENTER support
	...
	if (key == K_ESCAPE) {
		...
		// 
		if (cl_videoplaying == VIDEO_PLAYING_PLAYVIDEO_1 /*and value is NOT 2 menu qc*/ ) { // K_ESCAPE .. Baker: We want it to stop the video
	...
	if (down &&
		...
		RELATED_ (M_ToggleMenu) // Goes to M_ToggleMenu without CSQC
#ifdef CONFIG_MENU
	...
	if (cl_videoplaying == VIDEO_PLAYING_PLAYVIDEO_1 /*but not 2*/)
	{
