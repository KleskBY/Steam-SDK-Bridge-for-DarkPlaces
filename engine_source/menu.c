...
// Baker: This displays in the video menu as the current working directory ...
static const char *menu_current_working_directory_za;
...
RELATED_ (slist_sort_numplayers)
	...
	for (clnum=0; clnum < QW_MAX_CLIENTS_32; clnum ++) {
		...
		*nl = '\0';
		...
		if (my_entry->protocol != PROTOCOL_QUAKEWORLD) {
			...
			msg = token;
			char com_token2[65536];
			token = COM_Parse_FTE (token, com_token2, sizeof(com_token2)); // COM_Parse(token);  Baker: Is this close enough?
...
static void M_Background(int width, int height, int shall_darken)
{
	...
	menu_x = (vid_conwidth.integer - menu_width) * 0.5; // Baker: Centered
...
{
	DrawQ_String(menu_x + cx, menu_y + cy, str, 0, 8, 8, /*rgb*/ 1.03, 0.5, 0.36, alpha_1_0, 0, NULL, true, FONT_MENU);
}


static void M_PrintRed(float cx, float cy, const char *str)
{ 
	DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, /*rgba*/ 1, 0, 0, alpha_1_0, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_true, FONT_MENU);
}

static void M_ItemPrint(float cx, float cy, const char *str, int unghosted)
{
	if (unghosted)
		DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, q_rgba_solid_white_4_parms, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_true, FONT_MENU);
	else
		DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, /*rgba*/ 0.4, 0.4, 0.4, alpha_1_0, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_true, FONT_MENU);
}

// Ignores color codes.  For map titles.
static void M_ItemPrintBronzey(float cx, float cy, const char *str, int unghosted)
{
	if (unghosted)
		DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, /*rgba*/ 1.03, 0.5, 0.36, alpha_1_0, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_false, FONT_MENU);
	else
		DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, /*rgba*/ 0.4, 0.4, 0.4, alpha_1_0, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_false, FONT_MENU);
}
...
{
	if (unghosted)
		DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, /*rgba*/ q_rgba_solid_white_4_parms, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_false, FONT_MENU);
	else
		DrawQ_String(menu_x + cx, menu_y + cy, str, maxlen_0, /*scalexy*/ 8, 8, /*rgba*/ 0.4, 0.4, 0.4, 1, DRAWFLAG_NORMAL_0, OUTCOLOR_NULL, q_ignore_color_codes_false, FONT_MENU);
}

#define HOTSPOTS_NONE_0		0	// Draw an item that is not interactable like a title.
#define HOTSPOTS_1			1	// Has a single hotspot
...
static void M_DrawPicEx (cachepic_t *pico, float x, float y, float w, float h, float *color3, int count)
{
	if (color3)
		DrawQ_Pic(menu_x + x, menu_y + y, pico, w, h, color3[0], color3[1], color3[2], alpha_1_0, DRAWFLAG_NORMAL_0);
	else
		DrawQ_Pic(menu_x + x, menu_y + y, pico, w, h, q_rgba_solid_white_4_parms, DRAWFLAG_NORMAL_0);

	if (count != HOTSPOTS_NONE_0)
		Hotspots_Add (menu_x + x, menu_y + y, w, h, count_1, hotspottype_button, q_force_scale_0);
...
#if 0
static void M_DrawPicFrac(const char *picname, float x, float y, float w, float h, int count)
{
	float fx = x * vid_conwidth.value;
	float fy = y * vid_conheight.value;
	float fw = w * vid_conwidth.value;
	float fh = h * vid_conheight.value;

	cachepic_t *pico = Draw_CachePic (picname);
	DrawQ_Pic(menu_x + fx, menu_y + fy, pico, fw, fh, q_rgba_solid_white_4_parms, DRAWFLAG_NORMAL_0);
	if (count) {
		Hotspots_Add (menu_x + fx, menu_y + fy, fw, fh, count, hotspottype_button, q_force_scale_0);
	}
...
static void M_DrawTextBox(float x, float y, float width, float height)
{
	...
	cx = x;
	cy = y;
	M_DrawPic (cx, cy, "gfx/box_tl", HOTSPOTS_NONE_0, NA0, NA0);
	for (n = 0; n < height; n++)
	{
		cy += 8;
		M_DrawPic (cx, cy, "gfx/box_ml", HOTSPOTS_NONE_0, NA0, NA0);
	}
	M_DrawPic (cx, cy+8, "gfx/box_bl", HOTSPOTS_NONE_0, NA0, NA0);
	...
	{
		cy = y;
		M_DrawPic (cx, cy, "gfx/box_tm", HOTSPOTS_NONE_0, NA0, NA0);
		for (n = 0; n < height; n++)
		{
			cy += 8;
			if (n >= 1)
				M_DrawPic (cx, cy, "gfx/box_mm2", HOTSPOTS_NONE_0, NA0, NA0);
			else
				M_DrawPic (cx, cy, "gfx/box_mm", HOTSPOTS_NONE_0, NA0, NA0);
		}
		M_DrawPic (cx, cy+8, "gfx/box_bm", HOTSPOTS_NONE_0, NA0, NA0);
		width -= 2;
		cx += 16;
	}
	...
	// draw right side
	cy = y;
	M_DrawPic (cx, cy, "gfx/box_tr", HOTSPOTS_NONE_0, NA0, NA0);
	for (n = 0; n < height; n++)
	{
		cy += 8;
		M_DrawPic (cx, cy, "gfx/box_mr", HOTSPOTS_NONE_0, NA0, NA0);
	}
	M_DrawPic (cx, cy+8, "gfx/box_br", HOTSPOTS_NONE_0, NA0, NA0);
}
...
static void M_ToggleMenu(int mode /*wants on = 1, off = 0*/)
{
	...
	#pragma message ("Baker: Until the key release issue can never-ever happen, make opening the menu do it")
			...
			case m_zdev_29:
				...
				break;
			case m_zform_30:
				M_Menu_DevInfo_f (cmd_local); // Will set menu_state_reenter 0
				break;
#endif
			} // sw
		} // mode
...
static void M_DrawSlider (int x, int y, float num, float rangemin, float rangemax)
{
	char text[16];
	int i;
	float range = bound(0, (num - rangemin) / (rangemax - rangemin), 1);
	extern cvar_t utf8_enable;
	int utfhack = utf8_enable.integer;
	if (utfhack)
...
static void M_Credits_Draw (void)
{
	...
	M_DrawPic (0, 0, "gfx/creditsmiddle", HOTSPOTS_NONE_0, NA0, NA0);
	M_Print (640/2 - 14/2*8, 236, "Coming soon...");
	M_DrawPic (0, 0, "gfx/creditstop", HOTSPOTS_NONE_0, NA0, NA0);
	M_DrawPic (0, 433, "gfx/creditsbottom", HOTSPOTS_NONE_0, NA0, NA0);
}
...
void M_KeyEvent (int key, int ascii, qbool downevent)
{
...
#ifdef DEVINFO
	if (!downevent && isin2(m_state, m_zform_30, m_csqc_form_32) == false ) // Only devinfo wants mousedowns
		return;
#else
	if (!downevent)
		return;
	...
	case m_grid_31:					M_Grid_Key(cmd, key, ascii); 				return;
	...
	case m_csqc_form_32:			M_CSQC_Form_Key(cmd, key, ascii,downevent); return;
#endif
	} // sw

}

RELATED_ (M_Options_Draw M_Options_Nova_Draw)

int m_csqc_form_video_restarted = 0; // If set, cleared on draw, set by m_video_restarted
void M_Draw (void)
{
	...
	if (m_video_restarted) {
		// Baker: We can't rely on draw to do this immediately because
		// the CSQC form does not have to be active.
		// 2 choices ... resize it later (YES)
		// Or resize it now (NO)
		m_csqc_form_video_restarted = true;
		m_video_restarted = false; // Everyone has had a chance to look at it.
	}
	...
	case m_grid_31:					M_Grid_Draw ();						break;
	...
	case m_csqc_form_32:			M_CSQC_Form_Draw ();				break;
#endif
	} // sw
...
qbool IsDoubleClick_Set_Cursor (double *pclicktime, int *pcursor, int start_row, int hotspotx_hover2)
{
	int new_cursor			= hotspotx_hover2 + start_row;
...
#include "menu_main_zirc.c.h" // Baker: July 14 2025 - Idea of alternate menu draw.
	#include "menu_single_player.c.h"
		#include "menu_saveload2.c.h"
		#include "menu_saveload.c.h"
#ifdef DEVINFO
		mempool_t *devinfomempool;
	...
	#include "menu_help.c.h"
		#include "menu_grid.c.h"
#ifdef DEVINFO
		#include "menu_csqc_form.c.h"
#endif
...
CALLERS_ (FS_GameDir_f)
void Menu_Resets(void)
{
    // Baker: #pragma message ("M_Shutdown would be better place or even M_Init?")
	m_main_cursor =
		options_cursor =
		options_cursorx =
...
// Sets menu state
// What are ways this can occur with a form open?
// Server close.
// Demo?
// Clicking in console ... need to eliminate that.
void menu_state_set_nova(int newstate)
{
	if (m_state == m_csqc_form_32) {
		// Is it form to form?
		if (newstate != m_csqc_form_32) {
			// ESC key pressed.
			RELATED_ (VM_CL_Form_Close)
			if (csqc_form) {
				CSQC_Form_Queue_Close (csqc_form, "ESC"); // Wait a frame.
				//Con_PrintLinef ("CSQC form disabled due to menu usage");
				oject_s *f = csqc_form;
				...
				#pragma message ("DO NOT LIKE THIS SETTING CSQC FORM TO NULL")
			}
		}
	}
	
	m_state = (enum m_state_e)newstate;
...
RELATED_ (sv_intermap_siv_list)


static void M_Init(void) // Baker: This is NOT InitOnce (WARNING ***) Gamedir change fires this.
{
	menuplyr_load = true;
	menuplyr_pixels = NULL;

	Mem_FreeNull_ (menu_current_working_directory_za);
	char *cwd_ta = FS_Getcwd_No_Trailing_Slash_Unsafe_TempAlloc ();
		menu_current_working_directory_za = Z_StrDup (cwd_ta);
...
#ifdef DEVINFO
	Cmd_AddCommand(CF_CLIENT, "_devinfo_free", M_Menu_DevInfo_Free_f, "Exit devinfo");
	Cmd_AddCommand(CF_CLIENT, "devinfo", M_Menu_DevInfo_f, "Developer information interface");
	Cmd_AddCommand(CF_CLIENT, "ui_form_load", UI_Form_Load_f, "Load a form");
#endif
	Cmd_AddCommand(CF_CLIENT, "menu_grid", M_Menu_Grid_f, "Developer information interface");
}


// Exit how?
void M_Menu_Form_f (cmd_state_t *cmd)
{
	KeyDest_Set (key_menu); // key_dest = key_menu;
	menu_state_set_nova (m_credits);
	m_entersound = true;
