...
#else
#pragma comment(lib, "C:\\Users\\user\\Documents\\GitHub\\Steam-SDK-Bridge-for-DarkPlaces\\sdk\\redistributable_bin\\win64\\steam_api64.lib")
#endif
static ISteamNetworkingMessages* g_pSteamMsgs = NULL;

void LHNET_SetSteamNetworkingMessages(ISteamNetworkingMessages* p)
{
	g_pSteamMsgs = p;
}

void TestSteamLoopback()
{
	LHNET_SetSteamNetworkingMessages(SteamNetworkingMessages());
	...
	//const char* text = "hello sdr self-test";
	//auto* msgs = SteamNetworkingMessages();

	//EResult r = msgs->SendMessageToUser(
	//	ident,
	//	text,
	...
	//	/*channel=*/0
	//);
	//printf("SendMessageToUser(self) = %d \r\n", (int)r);
}
bool once = false;
bool InitSteam2()
{
...
typedef struct lhnetaddressnative_s
{
	...
	addr;
	SteamNetworkingIdentity steam_identity;   // valid when addresstype == LHNETADDRESSTYPE_STEAM //KLESKBY STEAM RELAY
}
...
int LHNETADDRESS_FromString(lhnetaddress_t *vaddress, const char *string, int defaultport)
{
	...
	//KLESKBY STEAM RELAY
	if (name[0] == 's' && name[1] == 't' && name[2] == 'e') // you may have a helper; otherwise use strncmp  "steam:"
	{
		const char* id_str = name + 6;
		const char* chan_str = strchr(id_str, ':');
		int channel = 0;
		if (chan_str)
		{
			channel = atoi(chan_str + 1);
			// copy only ID part
			char id_only[32];
			size_t len = chan_str - id_str;
			if (len >= sizeof(id_only))
				len = sizeof(id_only) - 1;
			memcpy(id_only, id_str, len);
			id_only[len] = '\0';
			id_str = id_only;
		}

		unsigned long long id64 = _strtoui64(id_str, NULL, 10);
		lhnetaddressnative_t* address = (lhnetaddressnative_t*)vaddress;
		memset(address, 0, sizeof(*address));
		address->addresstype = LHNETADDRESSTYPE_STEAM;
		address->port = channel; // local channel used by ISteamNetworkingMessages

		address->steam_identity.Clear();
		address->steam_identity.SetSteamID64(id64);
		return 1;
	}
...
{
#ifdef IP_TOS // Baker: 3 .. defined at least on Windows
	// Baker: This is the norm for at least Windows
...
lhnetsocket_t *LHNET_OpenSocket_Connectionless(lhnetaddress_t *address)
{
	...
	if (!address)
		return NULL;
	lhnetsocket = (lhnetsocket_t *)Mem_Alloc(netconn_mempool, sizeof(*lhnetsocket)); // CMDCVARMEM: Baker changed to netconn mempool
	if (lhnetsocket)
	{
		memset(lhnetsocket, 0, sizeof(*lhnetsocket));
		lhnetsocket->address = *address;
		lhnetsocket->address.addresstype = LHNETADDRESSTYPE_STEAM;
...
#ifdef IP_TOS // Baker: 3 .. For certain this is defined on Windows
	// Baker: This is the norm on Windows at a minimum
								{
...
#endif
			break;
		case LHNETADDRESSTYPE_STEAM:
			// No real socket; just register in socket list.
			if (!g_pSteamMsgs) {
				Con_Print("LHNET_OpenSocket_Connectionless: Steam networking not available\n");
				Z_Free(lhnetsocket);
				return NULL;
			}

			// If port/channel is 0, pick one
			if (lhnetsocket->address.port == 0)
				lhnetsocket->address.port = 1; // you can manage your own channel allocation
...
#endif
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_STEAM) //KLESKBY STEAM RELAY
	{
		if (!g_pSteamMsgs)
			return 0;

		// We use the socket's port field as our local channel
		int localChannel = lhnetsocket->address.port;

		SteamNetworkingMessage_t* msgs[4];
		int n = g_pSteamMsgs->ReceiveMessagesOnChannel(localChannel, msgs, 4);
		if (n <= 0)
			return 0; // no data

		// Take the first message; you can drain multiple if you like
		SteamNetworkingMessage_t* m = msgs[0];
		int copyLen = (m->m_cbSize < maxcontentlength) ? m->m_cbSize : maxcontentlength;
		memcpy(content, m->m_pData, copyLen);

		// Fill address with the sender identity + channel
		memset(address, 0, sizeof(*address));
		address->addresstype = LHNETADDRESSTYPE_STEAM;
		address->port = localChannel;
		address->steam_identity = m->m_identityPeer;
...
int LHNET_Write(lhnetsocket_t *lhnetsocket, const void *content, int contentlength, const lhnetaddress_t *vaddress)
{
	...
	{
		lhnetpacket_t *p;
		p = (lhnetpacket_t *)Mem_Alloc(netconn_mempool, sizeof(*p) + contentlength); // CMDCVARMEM
		p->data = (void *)(p + 1);
	...
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_STEAM) //KLESKBY STEAM RELAY
	{
		...
		SteamNetworkingIdentity id = address->steam_identity;
		int remoteChannel = address->port; // or choose per-connection

		EResult r = g_pSteamMsgs->SendMessageToUser(
			id,
			content,
			(uint32)contentlength,
			k_nSteamNetworkingSend_Unreliable,   // or Reliable if you want
			remoteChannel
		);
		...
		if (r == k_EResultOK)
			...
			value = 0; // treat as failed to send
...
// Baker: I added all of the following code .. why?
// Is this reverse DNS?  Explain ...
// Or was this required to get MinGW to compile Windows version?
...
struct qsockaddr
{
	...
	#pragma message ("Baker: MinGW headers bad for sockets/IPV6")
