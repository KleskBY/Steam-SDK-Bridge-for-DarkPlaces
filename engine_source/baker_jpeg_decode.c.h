...
RELATED_ (OpenStream dpvsimpledecode_open gd_open_gif_alloc)

// opens a stream

RELATED_ (dpvsimpledecodestream_t)
...
RELATED_ (SCR_gifclip_f dpvsimpledecode_open)


// returns the width of the image data
RELATED_ (dpvsimpledecode_getwidth)
...
RELATED_ (dpvsimpledecode_getheight)
...
RELATED_ (dpvsimpledecode_getframerate)
...
RELATED_ (dpvsimpledecode_getaspectratio)
...
RELATED_ (dpvsimpledecode_video gd_get_frame SCR_gifclip_f video->decodeframe CL_Video_Frame)
...
int bakerjpegdecode_videoframe (void *stream, void *vid_vimagedata, unsigned int Rmask, unsigned int Gmask, unsigned int Bmask, unsigned int bytesperpixel, int imagebytesperrow)
{
	...
	if (developer_loading.integer)
		Con_PrintLinef ("%4d: + %" PRId64 " -> %" PRId64, framenum, (int64_t)jpeg_datasize, (int64_t)s->jpegs_total_disk_size);
...
RELATED_ (gd_close_gif_maybe_returns_null dpvsimpledecode_close)
...
RELATED_ (bakergifdecode_open dpvsimpledecode_open OpenStream gd_open_gif_alloc bakerjpegdecode_videoframe SCR_gifclip_f)
...
alloc_2: ;
	jpegs_folder_stream_t *s	= (jpegs_folder_stream_t *)ZAlloc_VoidP_SizeOf(jpegs_folder_stream_t);

alloc_3: ;
	
	if (cl_video_frame_modulo.integer)
		stringlistappendfssearch_modulo (&s->jpegnames_list, (fssearch_t *)t, cl_video_frame_modulo.integer);
	else
	...
	if (developer_loading.integer) {
		stringlist_condump_with_linenums			(&s->jpegnames_list);
	}
