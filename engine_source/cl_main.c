...
cvar_t csqc_enable = {CF_CLIENT | CF_SERVER, "csqc_enable","1","whether csqc csprogs.dat loads [Zircon]"}; // Baker r0101: csqc_enable
...
cvar_t cl_coronas_without_light = {CF_CLIENT | CF_ARCHIVE, "cl_coronas_without_light", "0", "Enables rendering of standalone coronas for entities with no real-time light [Zircon]"};
...
client_static_t	cls;
client_state_t	cl;
int32list_s cl_list_tosses; // Baker: Ability to store entities thrown with (ent_create).
...
// wipe the entire cl structure
	...
	int32s_freecontents (&cl_list_tosses);

	RELATED_ (V_cshift_f)
#if 1
	// Baker: Cannot clear whole struct, makes crash in CL_MeshEntities_Scene_AddRenderEntity
	r_refdef.scene.worldentity = NULL;
	// Q: Does "restart" come here? A: YES!
	...
	cl.statsf_sv = (float *)cl.stats_sv;

	cl.num_entities = 0;
	cl.num_static_entities = 0;
	cl.num_brushmodel_entities = 0;

	cl.stats_sv[STAT_VIEWZOOM_21] = 255; // Baker r9001: Avoid zoom effect in Quake by correctly reseting cl viewzoom field
...
void CL_DisconnectEx(qbool kicked, const char *fmt, ... )
{
	...
	if (Sys_CheckParm("-profilegameonly")) Sys_AllowProfiling(false);
	...
	FS_UnloadPacks_dlcache(); // prevent dlcache assets from this server from interfering with the next one

	cl.parsingtextexpectingpingforscores = 0; // just in case no reply has come yet
	cl.cl_levelshot_time = 0; // December 13 2024 - No screenshot
	...
	cl.worldmodel = NULL;
	//cls.demonum = -1; // Baker - December 12 2024
	...
	else if (cls.netcon)
	{
		...
		if (cls.demorecording) CL_Stop_f(cmd_local);
		...
		if (!kicked) 
		{
			...
			else
			{
				...
				if (cls.protocol == PROTOCOL_DARKPLACES8) MSG_WriteString(&buf, reason);
			}
		...
		else Con_PrintLinef ("Disconnected");
	}
	...
	// Baker: Can't nuke startdemos here.  This occurs during startdemo playback between demos.
	...
	if (host.hook.SV_Shutdown) host.hook.SV_Shutdown();
}

// Baker: When is this called?  May 26 2025
// On "playdemo"
// When read demo message hits the end
...
CALLERS_ (NetConn_ConnectionEstablished Curl_Curl_f "finish autodownload")
CALLERS_ (Host_Error_Line FS_GameDir_f FS_PurgeAll_f)
CALLERS_ (FS_ChangeGameDirs CL_Record_f) // 

void CL_Disconnect_Real(void) // Baker: May 26 2025
{
	memset (&cls.demos, 0, sizeof(cls.demos)); cls.demonum = STARTDEMO_STOP_LOOP_NEG1;
	CL_Disconnect (); // CL_Disconnect_Real intermediate
}
...
static void CL_Connect_f(cmd_state_t *cmd)
{
	...
	if (rcon_secure.integer <= 0) Cvar_SetQuick(&rcon_password, "");
	CL_SetInfo ("spectator", "0", qnfo_send_true, qnfo_allowstar_false, qnfo_allowmodel_false, qnfo_quiet_false);
...
void CL_Disconnect_f (cmd_state_t *cmd)
{
...
#if 1
	if (argc == 1 && arg1[0] == NULL_CHAR_0) {
		// Baker: This is a disconnect typed at the console generally.  A server could stuffcmd it.
		CL_Disconnect_Real(); // "disconnect" typed at console.
		// Clear the start demos queue.
		//memset (&cls.demos, 0, sizeof(cls.demos)); cls.demonum = STARTDEMO_STOP_LOOP_NEG1;
		//for (int j = 0; j < MAX_DEMOS_8; j ++)
		//	cls.demos[j][0] = 0;
	...
	if (Cmd_Argc(cmd) < 1)
		CL_Disconnect(); // Baker: This happens how?
	else
		CL_DisconnectEx (q_is_kicked_false, Cmd_Argv(cmd, 1));
}
...
void CL_EstablishConnection(const char* address, int firstarg)
{
	...
	// make sure the client ports / backend are ready
	NetConn_UpdateSockets();

	// Parse textual address into cls.connect_address
	if (!LHNETADDRESS_FromString(&cls.connect_address, address, 26000))
	{
		Con_PrintLinef("Invalid address '%s'.", address);
#ifdef CONFIG_MENU
		M_Update_Return_Reason("Invalid address");
#endif
		return;
	}

	// Decide whether to use Steam FakeIP backend or normal UDP
	qbool use_steam = (cls.connect_address.addresstype == LHNETADDRESSTYPE_STEAM_FAKEIP);
	qbool ok = false;

	if (!use_steam)
	{
		// --- Normal UDP path (old CL_EstablishConnection2 behavior) ---
		cls.connect_mysocket = NetConn_ChooseClientSocketForAddress(&cls.connect_address);
		if (!cls.connect_mysocket)
		{
		...
		ok = true; // nothing else to do, UDP is "ready"
	}
	else
	{
		// --- Steam FakeIP path ---
		// We still want a client socket handle for the engine, but the actual
		// transport will be SteamNetworkingSockets.
		cls.connect_mysocket = NetConn_ChooseClientSocketForAddress(&cls.connect_address);
		if (!cls.connect_mysocket)
		{
			Con_PrintLinef("Unable to find a suitable client socket for Steam connection.");
#ifdef CONFIG_MENU
			M_Update_Return_Reason("No network");
#endif
			return;
		...
		// Start the Steam datagram relay + FakeIP connection
		ok = SteamNet_ClientConnectFakeIP(&cls.connect_address) ? true : false;
		if (!ok)
		{
			Con_PrintLinef("Unable to establish connection to server (backend).");
#ifdef CONFIG_MENU
			M_Update_Return_Reason("No network");
#endif
			return;
	...
	// If we get here, either UDP socket is ready or Steam connection init succeeded
	...
	if (firstarg >= 0)
	{
		...
		*cls.connect_userinfo = 0;
		...
		{
			InfoString_SetValue(
				cls.connect_userinfo,
				sizeof(cls.connect_userinfo),
				Cmd_Argv(cmd_local, i),
				Cmd_Argv(cmd_local, i + 1)
			);
		}
	}
	else if (firstarg < -1)
...
static void CL_SoundIndexList_f(cmd_state_t *cmd)
{
	...
	while(cl.sound_precache[i] && i != MAX_SOUNDS_4096) { 
		// Valid Sound
		Con_PrintLinef ("%d : %s", i, cl.sound_precache[i]->name);
		i++;
	}
}
...
static float CL_LerpPoint(void)
{
	...
	return bound(0, f, 1 + cl_lerpexcess.value /*d:0*/ );
}
...
static void CL_DecayLightFlashes(void)
{
	...
	for (i = 0, dl = cl.dlights;i < oldmax;i++, dl++)
	{
		if (dl->rt_radius)
		{
			...
			{
				if (cl_dlights_decayradius.integer /*d:1*/)
					dl->rt_radius = dl->initialradius * dl->intensity;
				else
					dl->rt_radius = dl->initialradius;
				if (cl_dlights_decaybrightness.integer /*d:1*/)
	...
	if (r_dynamic.integer)
	{
		for (i = 0, dl = cl.dlights;i < cl.num_dlights && r_refdef.scene.numlights < MAX_DLIGHTS_256;i++, dl++)
		{
			if (dl->rt_radius) {
				...
				Matrix4x4_Scale(&tempmatrix, dl->rt_radius, 1); // Baker: DEC_PFLAG 24
				// we need the corona fading to be persistent
				R_RTLight_Update(&dl->rtlight, /*isstatic?*/ false, &tempmatrix, dl->color, dl->style, dl->cubemapname, dl->shadow, dl->corona, dl->coronasizescale, dl->ambientscale, dl->diffusescale, dl->specularscale, dl->p_flags);
...
static void CL_UpdateNetworkEntity(entity_t *e, int recursionlimit, qbool interpolate)
{
	...
	e->render.ef_effects = e->state_current.effects;
	...
	else if (e->render.crflags & RENDER_VIEWMODEL)
	{
		...
		if (e->render.ef_effects & EF_NOGUNBOB)
	...
	if (e->render.model)
	{
		...
		if (!(e->render.ef_effects & 0xFF800000))
			e->render.ef_effects |= e->render.model->mf_effects;
		// if model is alias or this is a tenebrae-like dlight, reverse pitch direction
		if (e->render.model->type == mod_alias)
			angles[0] = -angles[0];
		if (Have_Flag(e->render.ef_effects, EF_SELECTABLE) && cl.mcmd.cursor_entitynumber == e->state_current.number)
		{
			VectorScale(e->render.colormod, 2, e->render.colormod);
	...
	if ((e->render.ef_effects & EF_ROTATE) && !(e->render.crflags & RENDER_VIEWMODEL))
	{
	...
	if (!r_fullbright.integer /*0*/ && r_refdef.scene.worldmodel && r_refdef.scene.worldmodel->lit) { // Baker r1002: Proper Quake behavior for Q1BSP maps with no light data -- all entities in map render fullbright.
		if (!(e->render.ef_effects & EF_FULLBRIGHT))
			e->render.crflags |= RENDER_LIGHT;
	}
	// hide player shadow during intermission or nehahra movie
	if (!(e->render.ef_effects & (EF_NOSHADOW | EF_ADDITIVE_32 | EF_NODEPTHTEST))
	 && (e->render.ralpha >= 1)
	...
	if (e->render.ef_effects & EF_NOSELFSHADOW)
		e->render.crflags |= RENDER_NOSELFSHADOW;
	if (e->render.ef_effects & EF_NODEPTHTEST)
		e->render.crflags |= RENDER_NODEPTHTEST;
	if (e->render.ef_effects & EF_ADDITIVE_32)
		e->render.crflags |= RENDER_ADDITIVE;
	if (e->render.ef_effects & EF_DOUBLESIDED)
		e->render.crflags |= RENDER_DOUBLESIDED;
	if (e->render.ef_effects & EF_DYNAMICMODELLIGHT)
...
static void CL_UpdateNetworkEntityTrail(entity_t *e)
{
	...
	if (e->render.ef_effects & (EF_BRIGHTFIELD_1 | EF_FLAME | EF_STARDUST))
	{
		if (e->render.ef_effects & EF_BRIGHTFIELD_1)
		{
			if (IS_NEXUIZ_DERIVED(gamemode))
		...
		if (e->render.ef_effects & EF_FLAME)
		...
		if (e->render.ef_effects & EF_STARDUST)
	...
	// LadyHavoc: if the entity has no effects, don't check each
	if (e->render.ef_effects && !(e->render.crflags & RENDER_VIEWMODEL))
	{
		if (e->render.ef_effects & EF_GIB)
			trailtype = EFFECT_TR_BLOOD;
		else if (e->render.ef_effects & EF_ZOMGIB)
			trailtype = EFFECT_TR_SLIGHTBLOOD;
		else if (e->render.ef_effects & EF_TRACER)
			trailtype = EFFECT_TR_WIZSPIKE;
		else if (e->render.ef_effects & EF_TRACER2)
			trailtype = EFFECT_TR_KNIGHTSPIKE;
		else if (e->render.ef_effects & EF_ROCKET)
			trailtype = EFFECT_TR_ROCKET;
		else if (e->render.ef_effects & EF_GRENADE)
		{
		...
		else if (e->render.ef_effects & EF_TRACER3)
...
static void CL_UpdateViewModel(void)
{
	...
	ent->state_current.active = true;
	ent->state_current.modelindex = cl.stats_sv[STAT_WEAPON];
	ent->state_current.frame = cl.stats_sv[STAT_WEAPONFRAME];
	ent->state_current.sflags = RENDER_VIEWMODEL;
	if ((cl.stats_sv[STAT_HEALTH] <= 0 && cl_deathnoviewmodel.integer) || cl.intermission)
...
static void CL_LinkNetworkEntity(entity_t *e)
{
	...
	dlightcolor[2] = 0;
	// LadyHavoc: if the entity has no effects, don't check each
	if (e->render.ef_effects & (EF_BRIGHTFIELD_1 | EF_DIMLIGHT_8 | EF_BRIGHTLIGHT_4 | EF_RED_128 | EF_BLUE_64 | EF_FLAME | EF_STARDUST))
	{
		if (e->render.ef_effects & EF_BRIGHTFIELD_1)
		{
			if (IS_NEXUIZ_DERIVED(gamemode))
		...
		if (e->render.ef_effects & EF_DIMLIGHT_8)
		{
		...
		if (e->render.ef_effects & EF_BRIGHTLIGHT_4)
		{
		...
		if (e->render.ef_effects & EF_RED_128) // red
		{
			dlightradius = max(dlightradius, 200);
			dlightcolor[0] += 1.50f;
			dlightcolor[1] += 0.15f;
		...
		if (e->render.ef_effects & EF_BLUE_64) // blue
		{
			dlightradius = max(dlightradius, 200);
			dlightcolor[0] += 0.15f;
			dlightcolor[1] += 0.15f;
	...
	if (e->render.model && e->render.ef_effects && !(e->render.crflags & RENDER_VIEWMODEL))
	{
		if (e->render.ef_effects & EF_GIB)
			trailtype = EFFECT_TR_BLOOD;
		else if (e->render.ef_effects & EF_ZOMGIB)
	...
	if (Have_Flag (e->state_current.lightpflags, PFLAGS_FULLDYNAMIC_128) && r_refdef.scene.numlights < MAX_DLIGHTS_256) {
		...
		R_RTLight_Update (
			...
			e->state_current.skin > 0 ? va(vabuf, sizeof(vabuf), "cubemaps/%d", e->state_current.skin) : NULL, 
			!(e->state_current.lightpflags & PFLAGS_NOSHADOW_1),
	...
	else if (cl_coronas_without_light.integer && Have_Flag (e->state_current.lightpflags, PFLAGS_CORONA_2) && r_refdef.scene.numlights < MAX_DLIGHTS_256)
	{
	...
	if (e->render.model && !(e->render.ef_effects & EF_NODRAW_16) && r_refdef.scene.numentities < r_refdef.scene.maxentities)
...
static void CL_RelinkStaticEntities(void)
{
	...
	for (i = 0, e = cl.static_entities;i < cl.num_static_entities && r_refdef.scene.numentities < r_refdef.scene.maxentities;i++, e++)
	{
		...
		// hide player shadow during intermission or nehahra movie
		if (!(e->render.ef_effects & (EF_NOSHADOW | EF_ADDITIVE_32 | EF_NODEPTHTEST)) && (e->render.ralpha >= 1))
			e->render.crflags |= RENDER_SHADOW;
...
void CSQC_RelinkAllEntities (int drawmask)
{
	...
	if (Have_Flag (drawmask, ENTMASK_NO_MESHENTITIES_524288) == false)
		CL_MeshEntities_Scene_AddRenderEntity();
	if (Have_Flag (drawmask, ENTMASK_NO_STATIC_32768) == false)
		CL_RelinkStaticEntities();
	if (Have_Flag (drawmask, ENTMASK_NO_BEAMS_65536) == false)
		CL_RelinkBeams();
	if (Have_Flag (drawmask, ENTMASK_NO_EFFECTS_131072) == false)
		CL_RelinkEffects();
	if (Have_Flag (drawmask, ENTMASK_NO_DLIGHTS_262144) == false)
		CL_RelinkLightFlashes();

	// link stuff
	if (Have_Flag(drawmask, ENTMASK_ENGINE_1))
	{
		CL_RelinkNetworkEntities();
		if (drawmask & ENTMASK_ENGINEVIEWMODELS_2)
...
===============
*/
RELATED_ (CL_Frame CL_Input NetConn_ClientFrame CL_SendMove CL_UpdateWorld)
RELATED_ (CL_ClientMovement_Replay CL_UpdateNetworkEntities interpolated)
void CL_UpdateWorld(void)
{
	...
	if (cls.state == ca_connected && cls.signon == SIGNONS_4) {
		...
		if (!cl.csqc_loaded) {
			...
			CSQC_RelinkAllEntities(ENTMASK_ENGINE_1 | ENTMASK_ENGINEVIEWMODELS_2);
		}
...
static void CL_Fog_f(cmd_state_t *cmd)
{
	...
	if (Cmd_Argc (cmd) == 1) {
		Con_PrintLinef ("\"fog\" is " QUOTED_STR ("%f %f %f %f %f %f %f %f %f"), 
			r_refdef.fog_density0,  // Baker: Can't do fog_density DP changes it!
			r_refdef.fog_red, 
			r_refdef.fog_green, 
			r_refdef.fog_blue, 
			r_refdef.fog_alpha, 
		...
		Con_PrintLineEmpty ();//Con_PrintLinef ("%s","");
		// Baker: March 31 2025 - Due to fog density being final value we print
		// fog_density0.  The fog_density is multiplied by the fog alpha squared.
		//Con_PrintLinef ("density      " "%1.2f", r_refdef.fog_density);
		Con_PrintLinef ("density      " "%1.2f", r_refdef.fog_density0);
...
static void CL_MeshEntities_Start(void)
{
	...
	{
		ent = cl_meshentities + i;
		Mod_Mesh_Create(ent->render.model, cl_meshentitynames[i]); // MESHMEM
	}
}
...
void CL_MeshEntities_Init(void)
{
	...
	for (i = 0; i < NUM_MESHENTITIES_2; i++)
	{
		...
		Mod_Mesh_Create(ent->render.model, cl_meshentitynames[i]); // Baker: June 2 2025 MESHMEM
		ent->render.ralpha = 1;
		ent->render.crflags = RENDER_SHADOW | RENDER_LIGHT;
...
lightme:
	...
	// same direction)
	if (VectorLength2(ent->custommodellight_origin)) { // Baker: Light #1 CSQC
		// CSQC entities always provide this (via CL_GetTagMatrix)
		for (q = 0; q < 3; q++)
			shadingorigin[q] = ent->custommodellight_origin[q];
	}
	else if (ent->entitynumber > 0 && ent->entitynumber < cl.num_entities) { // Baker: Light #2 Network
	...
	else { // Baker: Light #3 - Something else (?) 
		// World hits here, *1 brush models hit here -- Mesh hits here too -- ship uhd hits here (?)
		// Static mesh.
...
#if 1
#define ISPLAYER_ENT(ent)		(ent->model && ent->entitynumber && ent->entitynumber <= cl.maxclients)
#define ISMONSTER_ENT(ent)		Have_Flag (ent->crflags, RENDER_STEP)
#define ISVIEWMODEL_ENT(ent)	Have_Flag (ent->crflags, RENDER_VIEWMODEL)
				if (r_modellight_players_nocolor.value && ISPLAYER_ENT(ent)) {					
					// Baker: CSQC models don't have entitynumber
					float tot = a[0] + a[1] + a[2]; // Baker: is this 0-255 or 0 to 1?  I think 0 to 1.
					float pct = tot / 3.0;
					
					if (pct < r_modellight_players_nocolor_minlight.value)
						pct = r_modellight_players_nocolor_minlight.value;

					a[0] = a[1] = a[2] = pct;
					{
						float tot2 = c[0] + c[1] + c[2]; // Baker: is this 0-255 or 0 to 1?  I think 0 to 1.
						float pct2 = tot2 / 3.0;
						if (pct2 < r_modellight_players_nocolor_minlight.value)
							pct2 = r_modellight_players_nocolor_minlight.value;
						c[0] = c[1] = c[2] = pct2;
					} // bl
				}
				else if (r_modellight_monsters_noccolor.value && ISMONSTER_ENT(ent)) {					
					// Baker: CSQC models don't have entitynumber
					float tot = a[0] + a[1] + a[2]; // Baker: is this 0-255 or 0 to 1?  I think 0 to 1.
					float pct = tot / 3.0;
					
					if (pct < r_modellight_monsters_noccolor_minlight.value)
						pct = r_modellight_monsters_noccolor_minlight.value;

					a[0] = a[1] = a[2] = pct;
					{
						float tot2 = c[0] + c[1] + c[2]; // Baker: is this 0-255 or 0 to 1?  I think 0 to 1.
						float pct2 = tot2 / 3.0;
						if (pct2 < r_modellight_monsters_noccolor_minlight.value)
							pct2 = r_modellight_monsters_noccolor_minlight.value;
						c[0] = c[1] = c[2] = pct2;
					} // bl
				}				
				else if (r_modellight_viewmodels_nocolor.value && ISVIEWMODEL_ENT(ent)) {					
					// Baker: CSQC models don't have entitynumber
					float tot = a[0] + a[1] + a[2]; // Baker: is this 0-255 or 0 to 1?  I think 0 to 1.
					float pct = tot / 3.0;
					
					if (pct < r_modellight_viewmodels_nocolor_minlight.value)
						pct = r_modellight_viewmodels_nocolor_minlight.value;

					a[0] = a[1] = a[2] = pct;
					{
						float tot2 = c[0] + c[1] + c[2]; // Baker: is this 0-255 or 0 to 1?  I think 0 to 1.
						float pct2 = tot2 / 3.0;
						if (pct2 < r_modellight_viewmodels_nocolor_minlight.value)
							pct2 = r_modellight_viewmodels_nocolor_minlight.value;
						c[0] = c[1] = c[2] = pct2;
					} // bl
				}
				else
#endif // 1 - Minlight 4/29/2025
...
CALLERS_ (Mod_ForName /*yes really*/ SV_SpawnServer Host_InitOnce CDAudio_Play_byName)
...
===========
*/
CALLERS_ (CL_Host_Shutdown)
void CL_Host_Shutdown (void)
{
...
void CL_InitOnce (void)
{
	...
	else
	{
		...
		cls.levelmempool = Mem_AllocPool("client (per-level memory)", POOLFLAG_NONE_0, q_parentname_NULL);
		cls.permanentmempool = Mem_AllocPool("client (long term memory)", POOLFLAG_NONE_0, q_parentname_NULL);
	...
	//
		...
		Cmd_AddCommand(CF_CLIENT | CF_CLIENTCLOSECONSOLE, "connect", CL_Connect_f, "connect to a server by IP address or hostname"); // Baker r1003: close console for map/load/etc.
		Cmd_AddCommand(CF_CLIENT | CF_CLIENT_FROM_SERVER | CF_CLIENTCLOSECONSOLE, "reconnect", CL_Reconnect_f, "reconnect to the last server you were on, or resets a quakeworld connection (do not use if currently playing on a netquake server)"); // Baker r1003: close console for map/load/etc.
		...
		Cvar_RegisterVariable(&r_skyfog); // Baker r1201: FitzQuake r_skyfog
		...
		Cvar_RegisterVariable(&cl_coronas_without_light);
