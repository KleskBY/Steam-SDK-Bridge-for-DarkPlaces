...
static void SCR_CaptureVideo_RIFF_Pop(void)
{
	...
	if (x != sizehint)
	{
		...
		{
			int i;
			Con_PrintLinef (CON_WARN "WARNING: invalid size hint %d when writing video data (actual size: %d)", (int) sizehint, x);
...
static void SCR_CaptureVideo_RIFF_IndexEntry(const char *chunkfourcc, int chunksize, int flags)
{
	...
	if (format->riffstacklevel != 2)
		Sys_Error ("SCR_Capturevideo_RIFF_IndexEntry: RIFF stack level is %d (should be 2)", format->riffstacklevel);
...
static void SCR_CaptureVideo_RIFF_Finish(qbool final)
{
	...
	{
		Con_DPrintLinef ("Finishing first chunk (%d frames)", cls.capturevideo.frame);
...
void SCR_CaptureVideo_Avi_BeginVideo(void)
{
	...
	cls.capturevideo.formatspecific = Mem_Alloc(tempmempool, sizeof(capturevideostate_avi_formatspecific_t));
	{
		...
		SCR_CaptureVideo_RIFF_Flush();
		if (format->riffstacklevel != 2)
			Sys_Error ("SCR_CaptureVideo_BeginVideo: broken AVI writing code (stack level is %d (should be 2) at end of headers)", format->riffstacklevel);
