...
RELATED_ (in_movesequence client->movesequence)
qbool EntityFrame5_WriteFrame(sizebuf_t *msg, int maxsize, entityframe5_database_t *d, int numstates, const entity_state_t **states, int viewentnum, unsigned int in_movesequence, qbool need_empty)
{
		...
		for (i = 0;i < MAX_CL_STATS_256 && msg->cursize + 6 + 11 <= maxsize;i++) {
			if (host_client->stats_cl_deltabits[i>>3] & (1<<(i&7))) {
				...
				if (host_client->stats_cl[i] >= 0 && host_client->stats_cl[i] < 256) {
					...
					MSG_WriteByte(msg, i);
					MSG_WriteByte(msg, host_client->stats_cl[i]);
					lengtho = 1;
				}
				else
				{
					...
					MSG_WriteByte(msg, i);
					MSG_WriteLong(msg, host_client->stats_cl[i]);
					lengtho = 1;
				}
			}
		}
	...
	if (sv.protocol != PROTOCOL_QUAKE && sv.protocol != PROTOCOL_QUAKEDP && 
		...
		sv.protocol != PROTOCOL_DARKPLACES6) {
			// Baker: 
			RELATED_ (cls.servermovesequence)
...
void EntityFrame5_LostFrame(entityframe5_database_t *d, int framenum)
{
	...
	for (l = 0;l < (MAX_CL_STATS_256+7)/8;l++)
		host_client->stats_cl_deltabits[l] |= statsdeltabits[l];
