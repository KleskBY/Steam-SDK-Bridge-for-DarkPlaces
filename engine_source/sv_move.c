...
// the midpoint must be within 16 of the bottom
	...
	trace = SV_TraceLine(start, stop, MOVE_NOMONSTERS_1, ent, SV_GenericHitSuperContentsMask(ent), 0, 0, collision_extendmovelength.value);
...
// flying monsters don't step up
	...
	// try one move with vertical motion, then one without
		for (i=0 ; i<2 ; i++)
		{
			...
			trace = SV_TraceBox(entorigin, entmins, entmaxs, neworg, MOVE_NORMAL_0, ent, SV_GenericHitSuperContentsMask(ent), 0, 0, collision_extendmovelength.value);
...
// push down from a step height above the wished position
	...
	trace = SV_TraceBox(neworg, entmins, entmaxs, end, MOVE_NORMAL_0, ent, SV_GenericHitSuperContentsMask(ent), 0, 0, collision_extendmovelength.value);
