...
protocolversioninfo[] =
{
	...
	{   PROTOCOL_VERSION_QW_28, PROTOCOL_QUAKEWORLD  , "QW"},
...
void Protocol_UpdateClientStats(const int *stats)
{
	...
	for (i = 0;i < MAX_CL_STATS_256;i++) {
		if (host_client->stats_cl[i] != stats[i]) {
			host_client->stats_cl_deltabits[i >> 3] |= 1 << (i & 7);
			host_client->stats_cl[i] = stats[i];
		}
	}
}
...
void Protocol_WriteStatsReliable(void)
{
	...
	for (j = 0;sendquakestats[j] >= 0;j++)
	{
		...
		if (host_client->stats_cl_deltabits[i >> 3] & (1 << (i & 7))) {
			host_client->stats_cl_deltabits[i >> 3] -= (1 << (i & 7));
			// send the stat as a byte if possible
			if (sv.protocol == PROTOCOL_QUAKEWORLD) {
				if (host_client->stats_cl[i] >= 0 && host_client->stats_cl[i] < 256) {
					MSG_WriteByte(&host_client->netconnection->message, qw_svc_updatestat);
					MSG_WriteByte(&host_client->netconnection->message, i);
					MSG_WriteByte(&host_client->netconnection->message, host_client->stats_cl[i]);
				}
				else
				{
					MSG_WriteByte(&host_client->netconnection->message, qw_svc_updatestatlong);
					MSG_WriteByte(&host_client->netconnection->message, i);
					MSG_WriteLong(&host_client->netconnection->message, host_client->stats_cl[i]);
				}
			}
			else
			{
