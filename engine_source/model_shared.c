...
RELATED_ (mod->brushq1.lightmapupdateflags)
...
#if 0 // Baker: That is animated lights fool!
			int is_flip_lightmap = shader.numlayers == 2 && 
				shader.layers[0].tcgen.tcgen == Q3TCGEN_LIGHTMAP &&
				shader.layers[0].rgbgen.rgbgen == Q3RGBGEN_IDENTITY;
			if (is_flip_lightmap) {
				Con_PrintLinef ("Warning flipping layers 0 and 1 for %s", shader.name);
				// If shader layer 0 is tcgen lightmap
				// And shader has 2 layers .. flip them.
				//q3shaderinfo_layer_t layers[Q3SHADER_MAXLAYERS_8]
				q3shaderinfo_layer_t flip = {0};
				size_t flipsize = sizeof(shader.layers[0]);
				memcpy (&flip, &shader.layers[0], flipsize);
				memcpy (&shader.layers[0], &shader.layers[1], flipsize);
				memcpy (&shader.layers[1], &flip, flipsize);
			}
#endif
...
*/
		...
		texture->refractive_index = mod_q3shader_default_refractive_index.value /*d:1.33*/; 
		if (shader->dpreflectcube[0])
			texture->rreflectcubetexture = R_GetCubemap(shader->dpreflectcube);
			...
			if (fallback || shall_do_external)
			{
				...
				if (is_q3_shader_video_tex) {
					skinframe = R_SkinFrame_LoadInternalBGRA(texture->name,
						...
						/*compare w/h/crc/srgb*/ 0, 0, 0, q_tx_convertsrgb_false, q_is_sky_load_false); // TODO what sRGB argument to put here?
				} else {
		...
		for(line = 0;;line++)
		{
			...
			if (String_Match(word[0], "replace"))
			{
				if (words == 3)
				{
					...
					c_strlcpy (skinfileitem->skname, word[1]); //, sizeof (skinfileitem->skname));
...
{
	int surfaceindex;
	// I believe these are SLICK, SKY, LADDER, NOIMPACT, NOMARKS, FLESH, NODRAW, HINT, DUST, METALSTEPS
	q3deffect_t *q3_effect_sortsurface; // Baker: I think this is a pointer to shader surface flags.
	texture_t *texture;
...
static int Mod_MakeSortedSurfaces_qsortfunc(const void *a, const void *b)
{
	...
	if (l->q3_effect_sortsurface < r->q3_effect_sortsurface)
		return -1;
	if (l->q3_effect_sortsurface > r->q3_effect_sortsurface)
		return 1;
...
void Mod_MakeSortedSurfaces(model_t *mod)
{
	...
	for (j = 0; j < mod->num_surfaces; j++)
	{
		...
		info[j].q3_effect_sortsurface = mod->data_surfaces[j].q3_effect_msurf;
...
RELATED_ (RSurf_ActiveModelEntity Mod_MDL_AnimateVertices ALIASXPRIME Mod_IDP0_Load)

// Baker: It must be precached.
// Baker: No return in this function.
CALLERS_ (Mod_Decompile_f)
							   ...
							   int submodelnum, ccs *printf_pattern, baker_string_t *bsa)
{
	...
	if (framenumz > 0 && isin1(model->type, mod_alias) ) {
		RELATED_ (Mod_MDLMD2MD3_TraceLine Mod_Alias_MorphMesh_CompileFrames)
	...
	while (0) {
		...
		char filename_tags[MAX_QPATH_128];
		c_strlcpy (filename_tags, rawname);
		//File_URL_Edit_Remove_Extension (filename);
		c_strlcat (filename_tags, ".md3tags");

		//va_super (filename, MAX_QPATH_128, "
		qfile_t *f = FS_OpenRealFile(filename_tags, "wb", fs_quiet_FALSE); // WRITE-EON -- Baker: CleanShader for jack_scripts

		if (!f) {
			Con_PrintLinef ("ERROR: couldn't open " QUOTED_S, filename_tags);
			break;
		}
		...
		for (triangleindex = 0, etri = model->surfmesh.data_element3i + surface->num_firsttriangle * 3;
			...
			RELATED_ (NORMAL Collision_CalcPlanesForTriangleBrushFloat Mod_IDP0_Load)
		...
		vertexindex < model->surfmesh.num_vertices;vertexindex++, v += 3, vn += 3, vt += 2)
	{
		...
		if (mod_obj_orientation.integer /*d: 1*/) {
			...
			"vn %f %f %f"	NEWLINE
			"vt %f %f"		NEWLINE,
			v[0] + vtrans[0], v[2] + vtrans[2], v[1] + vtrans[1], vn[0], vn[2], vn[1], vt[0], shall_invert_y ? 1-vt[1] : vt[1]  );
		}
		else
			jj = dpsnprintf(outbuffer + outbufferpos, outbuffermax - outbufferpos,
			"v %f %f %f"	NEWLINE
	...
	// print some stats
	Con_PrintLinef ("Wrote %s (%d bytes, %d vertices, %d faces, %d surfaces with %d distinct textures)", filename, (int)outbufferpos, countvertices, countfaces, countsurfaces, counttextures);
}
...
obj_export:
	...
	if (mod->surfmesh.num_triangles) {
		...
		// Clipboard_Set_Text (bsa->string); Model decompile, this shouldn't be enabled.
	...
	for (int submodelindex = 0;submodelindex < max(1, model->brush.numsubmodels);submodelindex++) {
		...
		for (int surfaceindex = submodel->submodelsurfaces_start;surfaceindex < submodel->submodelsurfaces_end;surfaceindex++) {
			...
			if (String_Contains_Caseless (texname, swanted)) {
				...
				Clipboard_Set_Text (sorg); // cmd texturefindpos
...
static void Mod_GenerateLightmaps_CreateLightmaps(model_t *model)
{
	...
	lm_basescalepixels = 1.0f / max(0.0001f, mod_generatelightmaps_unitspersample.value /*d:8*/);
	lm_borderpixels = mod_generatelightmaps_borderpixels.integer /*d:2*/;
...
#define LM_DIST_EPSILON (1.0f / 32.0f)
	...
	for (surfaceindex = 0;surfaceindex < model->num_surfaces;surfaceindex++) { // LM_OID 2 May 10 2025
		surface = model->data_surfaces + surfaceindex;
...
texture_t *Mod_Mesh_GetTexture(model_t *mod, const char *name, int defaultdrawflags, int defaulttexflags, int defaultmaterialflags)
{
	...
	RELATED_ (DrawQ_SuperPic_Video cl_video_shutdown SuspendVideo)
...
void Mod_InitOnce (void)
{
	...
	// Baker: Moved these from #58 to here.
