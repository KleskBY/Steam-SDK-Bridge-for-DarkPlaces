...
// Baker: 
CALLERS_ (Host_InitOnce -> CL_InitOnce (blocks dedicated from coming here) -> Key_InitOnce )
static void Key_History_Init(void)
{
	qfile_t *historyfile;
	ConBuffer_Init(&history, HIST_TEXTSIZE_262144, HIST_MAXLINES_4096, historymempool);

// not necessary for mobile
// #ifndef DP_MOBILETOUCH Baker: say yes we want!
	va_super (zircon_history_realpath, MAX_OSPATH_EX_1024, "%s" "zircon_history.txt", *fs_userdir ? fs_userdir : fs_basedir);
	historyfile = FS_SysOpen(zircon_history_realpath, "rb", fs_nonblocking_false); // rb to handle unix line endings on windows too
	if (historyfile) {
...
CALLERS_ (Key_History_Shutdown, FS_ChangeGameDirs)
void Key_History_Write (void)
{
	// TODO write history to a file
	if (con_savehistory.integer == 0)
		return;
// not necessary for mobile
//#ifndef DP_MOBILETOUCH
	//qfile_t *historyfile = FS_OpenRealFile("zircon_history.txt", "w", false);
	va_super (zircon_history_realpath, MAX_OSPATH_EX_1024, "%szircon_history.txt", *fs_userdir ? fs_userdir : fs_basedir);
	qfile_t *historyfile = FS_SysOpen(zircon_history_realpath, "w", fs_nonblocking_false);

	if (historyfile) {
		int i;
		for(i = 0; i < CONBUFFER_LINES_COUNT(&history); i ++)
			FS_Printf (historyfile, "%s" NEWLINE, ConBuffer_GetLine(&history, i));
		FS_Close(historyfile);
	}
//#endif
}
...
#ifdef _DEBUG
	Sys_PrintToTerminal ("Partial Reset" NEWLINE);
#endif
}


CALLERS_ (Key_ClearEditLine)
