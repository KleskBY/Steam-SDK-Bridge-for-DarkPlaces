...
RELATED_ (Host_Main /*Host_CLFrame*/ -> CL_Input -> IN_Move)
			...
			case SDL_WINDOWEVENT:
				...
				//if (event.window.windowID == window) // how to compare?
				{
					...
					case SDL_WINDOWEVENT_RESIZED:
						if (vid_resizable.integer < 2 /*d:1*/) {
...
void Vid_Cursor_Load_To_Slot_Maybe (ccs *imagepath, int slot)
{
	...
	RELATED_(mousepointer_baker_column_20)
...
// COMMANDLINEOPTION: SDL GL: -gl_driver <drivername> selects a GL driver library, default is whatever SDL recommends, useful only for 3dfxogl.dll/3dfxvgl.dll or fxmesa or similar, if you don't know what this is for, you don't need it
	...
	vid_isfullscreen = false;
	{
		if (mode->fullscreen) {
			if (vid_desktopfullscreen.integer)
			{
				...
				// Baker: SDL_WINDOW_FULLSCREEN_DESKTOP is "fake" fullscreen that
				// takes the size of the desktop
	...
	// Baker: This can't hurt.
	SDL_SetHint(/*SDL_HINT_WINDOWS_DPI_AWARENESS*/ "SDL_WINDOWS_DPI_AWARENESS", "unaware");
	...
	// Baker r9511: SDL ensure icon - Baker: October 8 2024 - Not needed and does not fix "slowness"
	// This slowness to present an icon for the window dates back to earliest Zircon Beta and is not new.
#if 0
	#if defined(_WIN32)
		SetWIke (window);
	#endif
...
#if defined(QUALKER) || defined(OBJECTN) // ICON
		const char *s_zircon_icon_jpeg_base64 = "";
#elif defined(INVADERS) // ICON
		const char *s_zircon_icon_jpeg_base64 = "";
#elif defined(DARKNESS) // ICON
		const char *s_zircon_icon_jpeg_base64 = "";
#else // DARKNESS
	...
	Con_DPrintLineEmpty ();//Con_DPrintLinef ("");
#ifdef CONFIG_MENU
...
void Vid_SetWindowTitlef (ccs *fmt, ...)
{
	if (window) {
		//int j = 5;
		// No window!
...
// Baker: The following is endangered as we decided
// to use selfpack instead which is a .zip concat
// to the end of binary, which DarkPlaces accesses
// as a pack file.

// Baker: So we don't use this ...
