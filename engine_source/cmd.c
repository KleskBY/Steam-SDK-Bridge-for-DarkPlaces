...
static void Cmd_Defer_f (cmd_state_t *cmd)
{
	...
	{
		Con_PrintLinef ("usage: defer <seconds> <command>\n"
			   "       defer clear");
		return;
	}
}
...
RELATED_ ()
void Cbuf_InsertTextLine (cmd_state_t *cmd, ccs *text)
{
	va_super (mytext, MAX_INPUTLINE_16384, "%s" NEWLINE, text);

	Cbuf_InsertText (cmd, mytext);
}
	...
	if (gamemode == GAME_NORMAL_0) {
		...
		if (is_quake_rc) {
			if (strstr (text, "default.cfg") == NULL) {
				...
				const char *s_default_cfg2 = "exec default.cfg" "\n";
				size_t extrasize = strlen(s_default_cfg2) + 1; // +1 for null
				...
				strlcpy (s, s_default_cfg2, totalsize);
...
static void Cmd_Cycle_f(cmd_state_t *cmd)
{
	// cycle cvarname 
}
...
static void Cmd_Alias_f (cmd_state_t *cmd)
{
	...
	if (Cmd_Argc(cmd) == 1)
	{
		Con_PrintLinef ("Current alias commands:");
	...
	if (!a)
	{
		...
		a = (cmd_alias_t *)Mem_Alloc (cmdcvarmempool, sizeof(cmd_alias_t)); // CMDCVARMEM
...
// copy the rest of the command line
	...
	a->value = (char *)Mem_Alloc (cmdcvarmempool, alloclen); // CMDCVARMEM
...
static const char *Cmd_GetCvarValue(cmd_state_t *cmd, const char *var, size_t varlen, cmd_alias_t *alias)
{
	...
	if (!varstr)
	{
		...
		{
			if (alias)
				Con_PrintLinef (CON_WARN "Warning: Could not expand $%s in alias %s", varname, alias->name);
			else
...
static void Cmd_List_f (cmd_state_t *cmd)
{
	...
	{
		if (ispattern)
			Con_PrintLinef ("%d Command%s matching " QUOTED_S "\n", count, (count > 1) ? "s" : "", partial);
		else
			Con_PrintLinef ("%d Command%s beginning with " QUOTED_S "\n", count, (count > 1) ? "s" : "", partial);
	}
	else
...
static void Cmd_FindEx_f(cmd_state_t *cmd)
{
	if (Cmd_Argc(cmd) < 2) {
		Con_PrintLinef ("usage: %s <string> [string2 ...]", Cmd_Argv(cmd, 0));
		return;
	}

	if (Cmd_Argc(cmd) == 2) {
		Cmd_Apropos_f (cmd); // 1 arg = old style
		return;
	}

	char vabuf[1024];
	char vabuf2[1024];
	//ccs *partial = Cmd_Args(cmd);
	int argc = Cmd_Argc (cmd);
	int num_matches = 0;

	for (int idx = 1; idx == 1; idx ++) {
		ccs *partialx = Cmd_Argv(cmd, idx);
		qbool ispattern = partialx && (strchr(partialx, '*') || strchr(partialx, '?'));
		if (!ispattern)
			partialx = va(vabuf, sizeof(vabuf), "*%s*", partialx);

		#define COMMON_OPTS fs_caseless_true, fs_noseparators_zls, fs_atleastone_false

		for (cvar_t *cvar = cmd->cvars->vars; cvar; cvar = cvar->next) {
			if (!matchpattern_with_separator(cvar->name, partialx, COMMON_OPTS) && !matchpattern_with_separator(cvar->description, partialx, COMMON_OPTS))
				goto part2; // FAIL

			int shall_add = true;
			for (int n = 2; n < argc; n ++) {
				ccs *partial = Cmd_Argv(cmd, n);
				if (!ispattern)
					partial = va(vabuf2, sizeof(vabuf2), "*%s*", partial);

				//int find_in_name = matchpattern_with_separator(cvar->name, partial, COMMON_OPTS);
				//int find_in_desc = matchpattern_with_separator(cvar->description, partial, COMMON_OPTS);
				if (!matchpattern_with_separator(cvar->name, partial, COMMON_OPTS) && !matchpattern_with_separator(cvar->description, partial, COMMON_OPTS) ) {
					shall_add = false;
					break; // disqualified
				}
			} // for beyonders
			...
			if (shall_add) {
				// ADD
				Con_Printf ("cvar ");
				Cvar_PrintHelp(cvar, cvar->name, fs_description_true);
				num_matches ++;
			}
...
part2:
			// Part 2 ...
			for (char **cvar_alias = cvar->aliases; cvar_alias && *cvar_alias; cvar_alias++) {
				if (!matchpattern_with_separator(*cvar_alias, partialx, COMMON_OPTS))
					continue;
				...
				for (int n = 2; n < argc; n ++) {
					...
					if (!matchpattern_with_separator(*cvar_alias, partial, COMMON_OPTS) ) {
						shall_add2 = false;
						break; // disqualified because it does not contain this word
					}
				} // for beyonders
				...
				if (shall_add2) {
					// ADD
					Con_Printf ("cvar ");
					Cvar_PrintHelp(cvar, *cvar_alias, fs_description_true);
					num_matches ++;
				}
			}
		} // for

		// Baker: userdefined->qc_functions ?
		for (cmd_function_t *func = cmd->userdefined->qc_functions; func; func = func->next) {
			if (!matchpattern_with_separator(func->name, partialx, COMMON_OPTS) && !matchpattern_with_separator(func->description, partialx, COMMON_OPTS))
				continue;
			...
			for (int n = 2; n < argc; n ++) {
				...
				if (!ispattern)
					partial = va(vabuf2, sizeof(vabuf2), "*%s*", partial);
				if (!matchpattern_with_separator(func->name, partial, COMMON_OPTS) && !matchpattern_with_separator(func->description, partial, COMMON_OPTS)) {
					shall_add = false;
					break; // disqualified because it does not contain this word
				}
			} // for beyonders
		...
		for (cmd_alias_t *alias = cmd->userdefined->alias; alias; alias = alias->next) {
			// proceed here a bit differently as an alias value always got a final \n
			if (!matchpattern_with_separator(alias->name, partialx, COMMON_OPTS) && !matchpattern_with_separator(alias->value, partialx, fs_caseless_true, "\n", fs_atleastone_false)) // when \n is as separator wildcards don't match it
					continue;
			...
			for (int n = 2; n < argc; n ++) {
				...
				if (!ispattern)
					partial = va(vabuf2, sizeof(vabuf2), "*%s*", partial);
				if (!matchpattern_with_separator(alias->name, partial, COMMON_OPTS) && !matchpattern_with_separator(alias->value, partial, fs_caseless_true, "\n", fs_atleastone_false)) {
					shall_add = false;
					break; // disqualified because it does not contain this word
				}
			} // for beyonders
	...
	Con_PrintLinef ("%d result%s" NEWLINE, num_matches, (num_matches > 1) ? "s" : "");
}
...
{
	if (arg >= cmd->argc )
		return cmd->null_string; // Baker: Zero length string
	return cmd->argv[arg];
}
...
============
*/
// Baker: Cmd_Args is the entire string after the command.
...
// check functions
	...
	RELATED_ (examples: wait cprint sv_downloads )
	for (cmd_function_t *func = cmd->engine_functions; func; func = func->next) {
		if (String_Match_Caseless (func->name, s_command_wanted))
			return true;
...
void Argv_Cumulate (cmd_state_t *cmd, int start_arg, char *sbuf, size_t sizeofsbuf)
{
	...
	for (int j = start_arg; j < Cmd_Argc(cmd); j ++) {
		ccs *sxy = Cmd_Argv (cmd, j);
		if (j > start_arg)
			BakerString_CatC (bsa, " ");
		BakerString_CatC (bsa, sxy);
	}
	dp_strlcpy (sbuf, bsa->string, sizeofsbuf);
	BakerString_Destroy_And_Null_It (&bsa);
	Con_DPrintLinef ("Accumulated value string = " QUOTED_S, sbuf);
}
...
//
	...
	Cmd_AddCommand (CF_SHARED, "cvar_resettodefaults_autocvar", Cvar_ResetToDefaults_Autocvar_f, "sets all autocvars their default on first registration (registercvar)");
	...
	Cmd_AddCommand (CF_SHARED, "cmdlist", Cmd_List_f,	"lists console commands beginning with specified prefix or matching the specified wildcard pattern");
	Cmd_AddCommand (CF_SHARED, "cvarlist", Cvar_List_f,	"lists console variables beginning with specified prefix or matching the specified wildcard pattern, 'cvarlist changed' will print only changed cvars");
	Cmd_AddCommand (CF_SHARED, "apropos", Cmd_FindEx_f,	"lists console commands/variables/aliases containing all specified string(s) in name or description [Zircon enhanced]");
	Cmd_AddCommand (CF_SHARED, "find", Cmd_FindEx_f,		"lists console commands/variables/aliases containing all specified string(s) in name or description [Zircon enhanced]");
	...
	Cmd_AddCommand (CF_SHARED | CF_CLIENT_FROM_SERVER, "cycle", Cmd_Cycle_f, "Cycles a cvar through values [Zircon]");
