...
// Baker: Is this a DarkPlaces named function or mine?
// A: Math_atov is in standard DarkPlaces
	//char *t = s;
...
// Baker: Don't have the parse caller this, does not advance the cursor
int Math_atov_vm (ccs *s, prvm_vec3_t out)
{
	int i;
	VectorClear(out);
	if (*s == '\'')
