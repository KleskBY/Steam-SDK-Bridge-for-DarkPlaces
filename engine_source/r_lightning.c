...
static void R_DrawLightningBeam_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	float myvertex3f[12*3];
	float texcoord2f[12*2];

	RSurf_ActiveCustomEntity(&identitymatrix, &identitymatrix, 0, 0, r_lightningbeam_color_red.value, r_lightningbeam_color_green.value, r_lightningbeam_color_blue.value, 1, 12, myvertex3f, texcoord2f, NULL, NULL, NULL, NULL, 6, r_lightningbeamelement3i, r_lightningbeamelement3s, false, false);
	...
	for (surfacelistindex = 0;surfacelistindex < numsurfaces;surfacelistindex++)
	{
		...
		R_CalcLightningBeamPolygonVertex3f(myvertex3f + 0, start, end, offset);
		// polygon 2, verts 4-7
		VectorAdd(right, up, offset);
		VectorScale(offset, r_lightningbeam_thickness.value * 0.70710681f, offset);
		R_CalcLightningBeamPolygonVertex3f(myvertex3f + 12, start, end, offset);
		// polygon 3, verts 8-11
		VectorSubtract(right, up, offset);
		VectorScale(offset, r_lightningbeam_thickness.value * 0.70710681f, offset);
		R_CalcLightningBeamPolygonVertex3f(myvertex3f + 24, start, end, offset);
		...
		if (0 && r_lightningbeam_qmbtexture.integer) {
			R_DrawCustomSurface(
				r_lightningbeamqmbtexture, 
				&identitymatrix, 
				MATERIALFLAG_ALPHA | MATERIALFLAG_BLENDED | 
