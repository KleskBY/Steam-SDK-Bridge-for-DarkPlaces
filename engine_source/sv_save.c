...
CALLERS_ (SV_Loadgame_from PRVM_Breakpoint SV_Savegame_f VM_changelevel q_is_intermap_true)

qfile_t			*f_save;
static baker_string_t	*k_save; // temppool <------------

RELATED_ (SV_Savegame_to)
...
// Baker: We get a siv_string or a save name (name_or_null) - never both
void SV_Savegame_to (prvm_prog_t *prog, char **p_siv_string, ccs *name_or_null, int is_intermap_siv_write, float totaltimeatlastexit_to_write)
{
	int		i;
	...
	qbool is_autosave = name_or_null && String_Match (AUTOSAVE_NAME_AUTOSAVE0_SAV, name_or_null);
	...
	if (p_siv_string) {
		...
		if (is_autosave == false)
			Con_PrintLinef ("Saving game to %s...", name_or_null);
		f_save = FS_OpenRealFile(name_or_null, "wb", fs_quiet_FALSE); //  WRITE-EON savegameto ... DONE
		if (!f_save) {
...
//			PRVM_ED_Write (prog, f, PRVM_EDICT_NUM(i));
	...
	for (i = 0 ; i < sv_intermap_siv_list.numstrings; i += 2) {
		...
		RELATED_ (FS_Base64ClipboardCompressed_f FS_Base64ClipboardDeCompressed_f)

		size_t data_compressed_size = 0;
		unsigned char *data_compressed = string_zlib_compress_malloc (sxy_siv, &data_compressed_size, SIV_DECOMPRESS_BUFSIZE_16_MB);
	...
	if (cl.islocalgame && sv_save_screenshots.integer && Have_JPEG() ) {
		// free (s_base64_alloc);
...
intermap_no_siv_write_no_stringbuffers:
	...
	if (p_siv_string) {
		(*p_siv_string) = Mem_strdup (tempmempool, k_save->string);
...
RELATED_ (SV_Savegame_to stringlist_t)
void SV_Siv_f (cmd_state_t *cmd)
{
	...
	if (Cmd_Argc(cmd) == 1) {
		...
		Con_PrintLineEmpty ();//Con_PrintLinef ("");
...
void SV_Savegame_f (cmd_state_t *cmd)
{
	...
	// Dec 22 2024 unused ... int have_intermap = false;
	// Dec 22 2024 int v_enable_intermap_offset = PRVM_ED_FindGlobalOffset(prog, "enable_intermap");
	// Dec 22 2024 if (v_enable_intermap_offset >= 0) {
		//float val = PRVM_GLOBALFIELDFLOAT(v_enable_intermap_offset);
		// Dec 22 2024 unused ... if (val != 0)
		// Dec 22 2024 unused ... 	have_intermap = true;
	// Dec 22 2024 } // if
	...
	SV_Savegame_to (prog, q_siv_savestring_NULL, name_with_dot_sav, q_is_siv_write_false, /*totaltimeatlastexit*/ 0);
}
...
RELATED_ (VM_changelevel, SV_Changelevel2_f SV_SpawnServer)
...
// load the edicts out of the savegame file
			...
			while (COM_ParseToken_Simple(&t, false, false, true)) {
				...
				// VFS_PRINTF(f, "sv.startspot %s\n", InfoBuf_ValueForKey(&svs.info, "*startspot")); //startspot, for restarts.
					...
					if (is_odd == false) {
						...
						unsigned char *data_unbase64_malloc = base64_decode_calloc (k_siv->string, &unbase_datasize); // malloc

						// Baker: s_data_uncompressed_alloc is an entity string like a save game
						char *s_data_uncompressed_malloc = string_zlib_decompress_malloc (data_unbase64_malloc, unbase_datasize, SIV_DECOMPRESS_BUFSIZE_16_MB);

						// Baker: s_data_uncompressed_alloc is a save game blob of text
						stringlistappend (&sv_intermap_siv_list, (char *) s_data_uncompressed_malloc);

						freenull_ (s_data_uncompressed_malloc);
						freenull_ (data_unbase64_malloc);
	...
	RELATED_ (CL_EstablishConnection_Local)
...
void SV_Loadgame_f(cmd_state_t *cmd)
{
	...
	memset (&cls.demos, 0, sizeof(cls.demos)); cls.demonum = STARTDEMO_STOP_LOOP_NEG1; // stop demo loop in case this fails

	SV_Loadgame_from (cmd, filename, q_is_siv_write_false);
}


RELATED_ (VM_changelevel SV_Changelevel2_f)
...
// this silliness is so we can load 1.06 save files, which have float skill values
	...
	RELATED_ (SV_Changelevel2_f SV_Savegame_to SV_Loadgame_f)
