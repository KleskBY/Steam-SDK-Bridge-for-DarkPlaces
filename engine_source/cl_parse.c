...
static void CL_ParseStartSoundPacket(int is_largesoundindex)
{
	...
	if (cls.protocol == PROTOCOL_QUAKEWORLD) {
		...
		if (channel & (1<<15))
			...
			nvolume = DEFAULT_SOUND_PACKET_VOLUME_255;
...
CALLERS_ (All over the place even model load plus texture load)
...
CALLERS_ (CL_BeginDownloads_DP QW_CL_RequestNextDownload)

// Baker: It is already loaded?  YES.  The model is referenced below
static void CL_SetupWorldModel(void)
{
	prvm_prog_t *prog = CLVM_prog;

#if 1
	RELATED_ (CL_RESETNEWMAP_0)
	...
	cl.tool_marker_on = false; // Baker: cl should clear this now .. so not needed?
	...
	int have_levelshot = has_levelshot(cl.worldbasename, cl.levelshotsname, sizeof(cl.levelshotsname));

	extern cvar_t scr_levelshot_auto;
	qbool Have_JPEG (void);
	if (!have_levelshot && scr_levelshot_auto.integer && Have_JPEG() )
		cl.cl_levelshot_time = 2.0; // Screenshot at 2 seconds.
#endif // 1 LEVELSHOTS
...
CALLERS_ (QW_CL_RequestNextDownload ezQuake equiv CL_CheckOrDownloadFile)
static qbool QW_CL_CheckOrDownloadFile(const char *filename)
{
	qfile_t *file;
...
RELATED_ (QW_CL_FindModelNumbers EntityFrameQW_AllocDatabase right above)
...
RELATED_ (QW_Is_Step_ModelIndex, QW_Is_Step_Dead qw_monsters)
...
void CL_Models_Query (feed_fn_t myfeed_shall_stop)
{
	...
	for (int j = 1; j < MAX_MODELS_8192 && cl.model_name[j][0]; j ++) {
		RELATED_ (EZDev_Models_Feed_Shall_Stop_Fn EZDev_Sounds_Feed_Shall_Stop_Fn)
		int num_tri = 0;
...
RELATED_ (qw_svc_download oob is false)
...
RELATED_ (qw_svc_setinfo			QW_CL_SetInfo)
RELATED_ (qw_svc_updateuserinfo	QW_CL_UpdateUserInfo)
RELATED_ (QW_CL_RequestNextDownload)
...
RELATED_ (QW_CL_ServerInfo qw_svc_serverinfo cl.serverinfo )
...
static void CL_SignonReply (void)
{
	...
	case SIGNON_1:
		if (cls.netcon) {
			...
			RELATED_ (CL_BeginDownloads_DP_f)
...
==================
*/
RELATED_ (svc_serverinfo ) // Baker: svc_serverinfo and qw_svc_serverdata are both 11
RELATED_ (qw_svc_serverdata)
...
==================
*/
RELATED_ (svcfitz_spawnbaseline2)
...
static void CL_ParseClientdata (void)
{
	...
	if (isin13 (cls.protocol,	PROTOCOL_FITZQUAKE666,	PROTOCOL_FITZQUAKE999,	PROTOCOL_QUAKE,
		...
		cl.stats_sv[STAT_VIEWZOOM_21] = 255;
	}
	cl.idealpitch = 0;
	...
	if (cls.protocol == PROTOCOL_DARKPLACES5) {
		...
		if (isin2 (cls.protocol, PROTOCOL_FITZQUAKE666,	PROTOCOL_FITZQUAKE999)) {
			...
			if (bits & SU_FITZ_WEAPON2_S16)
				cl.stats_sv[STAT_WEAPON] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_ARMOR2_S17)
				cl.stats_sv[STAT_ARMOR] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_AMMO2_S18)
				cl.stats_sv[STAT_AMMO] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_SHELLS2_S19)
				cl.stats_sv[STAT_SHELLS] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_NAILS2_S20)
				cl.stats_sv[STAT_NAILS] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_ROCKETS2_S21)
				cl.stats_sv[STAT_ROCKETS] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_CELLS2_S22)
				cl.stats_sv[STAT_CELLS] |= (MSG_ReadByte(&cl_message) << 8);
			if (bits & SU_FITZ_WEAPONFRAME2_24)
				cl.stats_sv[STAT_WEAPONFRAME] |= (MSG_ReadByte(&cl_message) << 8);
	...
	if (Have_Flag (bits, SU_VIEWZOOM_S19) &&
		...
		if (isin3 (cls.protocol, PROTOCOL_DARKPLACES2, PROTOCOL_DARKPLACES3, PROTOCOL_DARKPLACES4) )
			cl.stats_sv[STAT_VIEWZOOM_21] = MSG_ReadByte(&cl_message);
		else {
			if (developer_qw.integer)
				Con_PrintLinef ("STAT_VIEWZOOM_21");
			cl.stats_sv[STAT_VIEWZOOM_21] = (unsigned short) MSG_ReadShort(&cl_message);
	}
	}

	// viewzoom interpolation
	cl.mviewzoom[0] = (float) max(cl.stats_sv[STAT_VIEWZOOM_21], 2) * (1.0f / 255.0f);
}
...
// copy it to the current state
	...
	ent->render.ef_effects = ent->state_baseline.effects;
...
// with velocity "svc_pointparticles"
static void CL_ParsePointParticles(void)
{
	...
	if (developer_pointparticles.value) {
		ccs *s_name = CL_Particles_EffectIndex_To_Name (effectindex);

		Con_PrintLinef ("pointparticles: %s (index = %d) origin " VECTOR3_5d1F " velocity " VECTOR3_5d1F, 
			s_name, effectindex, VECTOR3_SEND(origin), VECTOR3_SEND(velocity) );
	}
	CL_ParticleEffect(effectindex, count, origin, origin, velocity, velocity, NULL, 0);
}

// Baker: no velocity "svc_pointparticles1"
static void CL_ParsePointParticles1(void)
{
	...
	if (developer_pointparticles.value) {
		...
		Con_PrintLinef ("pointparticles1: %s (index = %d) origin" VECTOR3_5d1F, 
			s_name, effectindex, VECTOR3_SEND(origin));
	}
...
static void CL_IPLog_List_f(cmd_state_t *cmd)
{
	...
	if (Cmd_Argc(cmd) > 2)
	{
		Con_PrintLinef ("usage: %s 123.456.789.", Cmd_Argv(cmd, 0));
		return;
	}
	addressprefix = "";
	if (Cmd_Argc(cmd) >= 2)
...
RELATED_ (CL_ParseServerMessage svc_time also QW CL_ParseServerMessage)
...
void CL_ParseHint (const char *str)
{
	...
	if (scursor[0] == SPACE_CHAR_32 && scursor[1] > SPACE_CHAR_32) {
		...
		const char	*scmd_arg1 = &scursor[1];
			...
			RELATED_ (CLIENT_SUPPORTED_ZIRCON_EXT, SV_Zircon_Extensions_Send,  CL_BeginDownloads_Prespawn_Zircon_Extensions_Send)
...
#else
	MSG_ReadByte(&cl_message); // den
	MSG_ReadByte(&cl_message); // red
	MSG_ReadByte(&cl_message); // gr
	MSG_ReadByte(&cl_message); // blue
	MSG_ReadShort(&cl_message); // time
...
//			R_TimeReport("END OF MESSAGE");
		...
		if (cmd & U_SIGNAL_128) { // just differentiates from other updates
			...
			// Baker: The Quake start demos come here a lot
		...
		case svc_disconnect:
			if (cls.demonum != STARTDEMO_STOP_LOOP_NEG1)
				CL_NextDemo();
			else {
