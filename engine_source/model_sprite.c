			...
			if (cls.state != ca_dedicated)
			{
				...
				if (width > 0 && height > 0)
				{
					...
					if (!(skinframe = R_SkinFrame_LoadExternal(name, texflags | TEXF_COMPRESS, q_tx_complain_false, q_tx_fallback_notexture_false)))
					{
						...
						skinframe = R_SkinFrame_LoadInternalBGRA(name, texflags, pixels, width, height, 0, 0, 0, q_tx_convertsrgb_false, q_is_sky_load_false);
...
int Mod_IDSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (version == SPRITE_VERSION || version == SPRITE32_VERSION)
	{
		...
		loadmodel->synctype = (int)/*(synctype_t)*/LittleLong (pinqsprite->synctype);
	...
	else if (version == SPRITEHL_VERSION)
	{
		...
		loadmodel->synctype = (int)/*(synctype_t)*/LittleLong (pinhlsprite->synctype);
