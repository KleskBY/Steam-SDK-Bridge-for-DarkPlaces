...
int LHNETADDRESS_FromPort(lhnetaddress_t *vaddress, lhnetaddresstype_t addresstype, int port)
{
	...
	switch(addresstype)
	{
		...
		case LHNETADDRESSTYPE_LOOP:
			...
			address->port = port;
			return 1;
		case LHNETADDRESSTYPE_STEAM_FAKEIP:
...
#define sscanf sscanf_s
	...
	if (sscanf(name, "%d.%d.%d.%d", &d1, &d2, &d3, &d4) >= 1 && (unsigned int)d1 < 256 && (unsigned int)d2 < 256 && (unsigned int)d3 < 256 && (unsigned int)d4 < 256)
	{
		...
		{
			if (SteamNet_IsFakeIPAddr(address))
			{
				address->addresstype = LHNETADDRESSTYPE_STEAM_FAKEIP;
			}
		}
#endif
		return 1;
...
#endif
	{
		*address = namecache[i].address;
		address->port = port;
		if (address->addresstype == LHNETADDRESSTYPE_INET4 || address->addresstype == LHNETADDRESSTYPE_INET6)
		{
			address->addr.in.sin_port = htons((unsigned short)port);
#ifdef STEAM_NETWORKING
			if (g_steamNetInitialized)
	...
	if (hostentry)
	{
		if (hostentry->h_addrtype == AF_INET)
		{
			...
			for (i = 0;i < (int)sizeof(namecache[namecacheposition].name)-1 && name[i];i++) namecache[namecacheposition].name[i] = name[i];
...
int LHNET_DefaultDSCP(int dscp)
{
	...
	if (dscp >= 0) lhnet_default_dscp = dscp;
	return prev;
#else
	return -1;
#endif
...
void LHNET_Shutdown(void)
{
	...
	if (!lhnet_active) return;
	List_For_Each_Entry_Safe(s, snext, &lhnet_socketlist.list, lhnetsocket_t, list) LHNET_CloseSocket(s);
...
void LHNET_CloseSocket(lhnetsocket_t *lhnetsocket)
{
	if (lhnetsocket)
	{
		...
		if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_INET4 || lhnetsocket->address.addresstype == LHNETADDRESSTYPE_INET6 || lhnetsocket->address.addresstype == LHNETADDRESSTYPE_STEAM_FAKEIP)
		{
			closesocket(lhnetsocket->inetsocket);
		}
		Z_Free(lhnetsocket);
...
int LHNET_Read(lhnetsocket_t *lhnetsocket, void *content, int maxcontentlength, lhnetaddress_t *vaddress)
{
	lhnetaddressnative_t *address = (lhnetaddressnative_t *)vaddress;
	int value = 0;
	if (!lhnetsocket || !address || !content || maxcontentlength < 1) return -1;
...
#ifdef STEAM_NETWORKING
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_STEAM_FAKEIP)
	{
		value = SteamNet_Read(lhnetsocket, content, maxcontentlength, vaddress);
	}
#endif
	return value;
}
...
int LHNET_Write(lhnetsocket_t *lhnetsocket, const void *content, int contentlength, const lhnetaddress_t *vaddress)
{
	...
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_INET4)
	{
		...
		if (value == -1)
		{
			if (SOCKETERRNO == EWOULDBLOCK) return 0;
	...
	{
		value = SteamNet_Write(lhnetsocket, content, contentlength, vaddress);
	}
#endif
	return value;
}
