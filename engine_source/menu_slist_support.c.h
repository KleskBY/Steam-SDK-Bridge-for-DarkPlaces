...
void SList_Tiebreaker_Changed_c (cvar_t *var)
{
	...
	for (int idx = 0; idx < comma_items_count && idx < MAX_TIEBREAKERS_10; idx ++) {
		char *s_this_za =  String_Instance_Base1_ZAlloc (var->string, ',' , idx + 1, q_reply_len_NULL);
		// gcc unused ... int sz = sizeof(tiebreakers[idx]);
		c_strlcpy (tiebreakers[idx], s_this_za);
		Mem_FreeNull_ (s_this_za);
		tiebreakers_count ++;
	} // idx
}
