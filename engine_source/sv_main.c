...
cvar_t sv_entpatch_start = {CF_SERVER, "sv_entpatch_start", "", "appends info_player_start with this as origin [Zircon]"};
...
cvar_t sv_gameplayfix_grenadebouncedownslopes = {CF_SERVER, "sv_gameplayfix_grenadebouncedownslopes", "1", "prevents MOVETYPE_BOUNCE_10 (grenades) from getting stuck when fired down a downward sloping surface"};
...
cvar_t sv_movetype_follow_csqc_sendflags = {CF_SERVER, "sv_movetype_follow_csqc_sendflags", "16777215", "Flags to set when a MOVETYPE_FOLLOW CSQC entity moves. [Zircon]"}; // Default is 0x00FFFFFF
...
cvar_t sv_interactive_flag = {CF_SERVER, "sv_interactive_flag", "256", "Entities with this .flags bit set are treated as interactable for protocol, 256 is FL_ITEM [Zircon]"};
...
cvar_t sv_gameplayfix_slidemoveprojectiles = {CF_SERVER, "sv_gameplayfix_slidemoveprojectiles", "1", "allows MOVETYPE_FLY_5/FLYMISSILE/TOSS/BOUNCE/BOUNCEMISSILE entities to finish their move in a frame even if they hit something, fixes 'gravity accumulation' bug for grenades on steep slopes"};
...
cvar_t sv_gameplayfix_nostepmoveonsteepslopes = {CF_SERVER, "sv_gameplayfix_nostepmoveonsteepslopes", "0", "crude fix which prevents MOVETYPE_STEP_4 (not swimming or flying) to move on slopes whose angle is bigger than 45 degree"};
...
#if 111 // MX
cvar_t sv_gameplayfix_unstickplayers = {CF_SERVER, "sv_gameplayfix_unstickplayers", "1", "big hack to try and fix the rare case of MOVETYPE_WALK_3 entities getting stuck in the world clipping hull."};
...
cvar_t sv_nostep = {CF_SERVER | CF_NOTIFY, "sv_nostep","0", "prevents MOVETYPE_STEP_4 entities (monsters) from moving"};
...
cvar_t sys_ticrate = {CF_SERVER | CF_ARCHIVE, "sys_ticrate","0.01388889", "how long a server frame is in seconds, 0.05 is 20fps server rate, 0.1 is 10fps (can not be set higher than 0.1), 0 runs as many server frames as possible (makes games against bots a little smoother, overwhelms network players), 0.01388889 matches QuakeWorld physics 1/72"};
...
cvar_t _scr_devinfo = {CF_SERVER, "_scr_devinfo","engine/form1.txt", "Dev Info Form [Zircon]" }; // AURA
...
#ifdef CONFIG_MENU
// CLIENT
cvar_t sv_autosave = {CF_SERVER | CF_ARCHIVE, "sv_autosave", "1", "Automatic save game every 90 seconds to _auto_save_0.sav.  2 = Silent. [Zircon]"};
#else
// DEDICATED SERVER - DEFAULTS 0
cvar_t sv_autosave = {CF_SERVER | CF_ARCHIVE, "sv_autosave", "0", "Automatic save game every 90 seconds to _auto_save_0.sav.  2 = Silent. [Zircon]"};
#endif


server_t sv;
server_static_t svs;
RELATED_ (sv.intermap_startspot)
...
static void SV_SaveEntFile_f(cmd_state_t *cmd)
{
	...
	va(vabuf, sizeof(vabuf), "%s.ent", sv.worldnamenoextension);  // DOT_ENT (5)
...
void SV_SendServerinfo (client_t *client)
{
	...
	memset(client->stats_cl, 0, sizeof(client->stats_cl));
	memset(client->stats_cl_deltabits, 0, sizeof(client->stats_cl_deltabits));
...
downloadx_sv_start_download_2:
	...
	if (host_client->download_chunked) {
		RELATED_ (QW_CL_ParseChunkedDownload)
		// Reliable
		//DPCHUNKS
...
static void Download_CheckExtensions(cmd_state_t *cmd)
{
	...
	for(i = 2; i < argc; ++i)
	{
		if (String_Match(Cmd_Argv(cmd, i), "deflate"))
		{
	...
	if ((whichpack && whichpack2 && String_Match_Caseless(whichpack, whichpack2))
...
int SV_ModelIndex(const char *s, int precachemode)
{
	...
	for (i = 2;i < limit;i++)
	{
		if (!sv.model_precache[i][0]) {
			if (precachemode > PRECACHE_MODE_0) {
			//if (precachemode) {
				...
				{
					Con_PrintLinef ("SV_ModelIndex(" QUOTED_S "): precache_model can only be done in spawn functions", filename);
					return 0;
				}
				//if (precachemode == 1)
				...
				if (sv.state == ss_loading)
				{
					...
					sv.models[i] = Mod_ForName (sv.model_precache[i], /*crash?*/ true, /*checkdisk?*/ false, s[0] == '*' ? sv.worldname : NULL);
				}
				else
				{
					if (svs.threaded)
					...
					else
					{
						...
						sv.models[i] = Mod_ForName (sv.model_precache[i], /*crash?*/ true, /*checkdisc?*/ false, s[0] == '*' ? sv.worldname : NULL);
					}
			...
			Con_PrintLinef ("SV_ModelIndex(" QUOTED_S "): not precached", filename);
			return 0;
		}
		if (String_Match(sv.model_precache[i], filename))
			return i;
	}
	Con_PrintLinef ("SV_ModelIndex(" QUOTED_S "): i (%d) == MAX_MODELS_8192 (%d)", filename, i, MAX_MODELS_8192);
	return 0;
}

 // Baker r9067: loadgame precaches "precache at any time models and sounds"
// Baker: This is for our precache fix that seems to not apply to DarkPlaces Beta anyway
// See: sv_main_precache.c.h
int SV_ModelIndex_Count(void)
{
	int i, limit = isin6 (sv.protocol, PROTOCOL_QUAKE,       PROTOCOL_QUAKEDP,     PROTOCOL_NEHAHRAMOVIE, 
									   PROTOCOL_NEHAHRABJP,  PROTOCOL_NEHAHRABJP2, PROTOCOL_NEHAHRABJP3) ? 256 : MAX_MODELS_8192;
...
{
	int i, limit = isin6(sv.protocol, PROTOCOL_QUAKE,		PROTOCOL_QUAKEDP,	  PROTOCOL_NEHAHRAMOVIE,
						              PROTOCOL_NEHAHRABJP,  PROTOCOL_NEHAHRABJP2, PROTOCOL_NEHAHRABJP3) ? 256 : MAX_SOUNDS_4096;
...
#define is_small_limit isin4 (sv.protocol, PROTOCOL_QUAKE, PROTOCOL_QUAKEDP, PROTOCOL_NEHAHRAMOVIE, PROTOCOL_NEHAHRABJP)
	...
	for (i = 1;i < limit;i++) {
		...
		if (String_Match(sv.sound_precache[i], filename))
			return i;
	}
...
int SV_ParticleEffectIndex (const char *name)
{
			...
			for (linenumber = 1; ; linenumber ++) {
				...
				for (;;) {
					if (!COM_ParseToken_Simple(&text, true, false, true) || 
						String_Match(com_token, NEWLINE))
						break;
					...
					if (argc == 2) {
						for (effectnameindex = 1;effectnameindex < MAX_PARTICLEEFFECTNAME_4096;effectnameindex++) {
							if (sv.particleeffectname[effectnameindex][0]) {
								if (String_Match(sv.particleeffectname[effectnameindex], argv[1]))
									break;
							} else {
	...
	for (effectnameindex = 1; effectnameindex < MAX_PARTICLEEFFECTNAME_4096 && sv.particleeffectname[effectnameindex][0]; effectnameindex ++)
		if (String_Match(sv.particleeffectname[effectnameindex], name))
...
static void SV_CreateBaseline (void)
{
	...
	for (entnum = 0; entnum < prog->max_edicts; entnum++) {
		...
		if (entnum > 0 && entnum <= svs.maxclients) {
			...
			svent->priv.server->baseline.modelindex = SV_ModelIndex("progs/player.mdl", PRECACHE_MODE_1);
...
================
*/
RELATED_ (SV_Changelevel_f SV_Restart_f VM_changelevel SV_SpawnServer SV_SaveSpawnparms) // Q2X
...
RELATED_ (SV_Restart_f SV_Changelevel2_f)
...
CALLERS_ (SV_Loadgame_f SV_Map_f SV_Changelevel_f SV_Restart_f SV_Savegame_f)

// Baker: intermap .siv read never comes here
// Baker: What is client equivalent ...
RELATED_ (CL_SetupWorldModel) // Baker: CL_ equivalent
void SV_SpawnServer (const char *mapshortname, const char *s_loadgame, const char *s_startspot, const vec3_t startorigin, float totaltimeatstart) // Q2X
{
	...
	RELATED_ (CL_SetupWorldModel)
	Cvar_SetValueQuick	(&sv_freezenonclients, 0);	// Baker r0090: freezeall
	
// Baker: Moved to CL_SetupWorldModel since it is client
//#ifdef CONFIG_MENU
//
...
//	SV_LockThreadMutex();
	...
	if (sv.active) {
		...
		if (PRVM_serverfunction(SV_Shutdown)) {
			...
			prog->loaded = false; // Baker SV_Shutdown
		}
	}
...
//
	...
	if (*sv_random_seed.string /*d:empty string*/) {
	...
	if (sv_entpatch.integer &&
		(s_entities = (char *)FS_LoadFile(va(vabuf, sizeof(vabuf), "%s.ent", sv.worldnamenoextension), tempmempool, fs_quiet_true, fs_size_ptr_null))) {  // DOT_ENT (6)
...
// serverflags are for cross level information (sigils)
	...
	if (sv_entpatch.integer && (entities = (char *)FS_LoadFile(va(vabuf, sizeof(vabuf), "%s.ent", sv.worldnamenoextension), tempmempool, fs_quiet_true, fs_size_ptr_null))) {  // DOT_ENT (7)
		Con_PrintLinef ("Loaded %s.ent", sv.worldnamenoextension);
		PRVM_ED_LoadFromFile(prog, entities);
		Mem_Free(entities);
	}
	else if (cls.s_find_za) {
		char *s_malloc = String_Replace_Malloc (sv.worldmodel->brush.entities, cls.s_find_za, cls.s_replace_za);
		PRVM_ED_LoadFromFile(prog, s_malloc);
		freenull_ (s_malloc);
	} else if (sv_entpatch_start.string[0]) {
		ccs *s1 = "{" NEWLINE 
					"\"classname\" \"info_player_start\"" NEWLINE
					"\"origin\" ";
		ccs *s2 =	NEWLINE "\"angle\" \"90\"" NEWLINE
					"}" NEWLINE;
		// String with origin in quotes.
		va_super (s12, 1024, "%s\"%s\"%s", s1, sv_entpatch_start.string, s2);
		char *sza = Z_StrDupf ("%s%s", s_entities, s12); // Add new entity to end.
		PRVM_ED_LoadFromFile(prog, sza);
		Mem_FreeNull_ (sza);
	}
	else {
...
#ifdef CONFIG_MENU
		if (sv.frametime && sv_autosave.integer /*d:1 client d:0 server*/) {
			if (!sv.sv_autosave_time) {
				...
				if (did_save) {
					// ((int)(sv.time / AUTOSAVE_INTERVAL_90)) == 1 means first time
					if (sv_autosave.integer < 2 && ((int)(sv.time / AUTOSAVE_INTERVAL_90)) == 1) {
						prvm_prog_t *menu_prog = MVM_prog;
						if (menu_prog->loaded)
							Con_PrintLinef ("Auto-saved.");
						else 
							Con_PrintLinef ("Auto-saved.  Menu->Single Player->Load Game.");
					}
					else {
						prvm_prog_t *menu_prog = MVM_prog;
						if (menu_prog->loaded)
							Con_DPrintLinef ("Auto-saved.");
						else 
							Con_DPrintLinef ("Auto-saved.  Menu->Single Player->Load Game.");
					}
				} // did save
			} // time to autosave
		}
...
void SV_InitOnce (void)
{
	...
	Cvar_RegisterCallback (&csqc_progcrc, CSQC_Progcrc_c);
	...
	Cvar_RegisterVariable (&sv_entpatch_start);
...
#if 0
	Cvar_RegisterVariable (&sv_gameplayfix_hemebond_pushmove);
#endif
	...
	Cvar_RegisterVariable (&sv_interactive_flag);
	...
	Cvar_RegisterVariable (&_scr_devinfo); // AURA
	...
	Cvar_RegisterVariable (&sv_movetype_follow_csqc_sendflags);
