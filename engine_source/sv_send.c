...
// #define SV_ClientPrintLinef(FORMAT_STRING, ...) SV_ClientPrintf (FORMAT_STRING "\n", __VA_ARGS__)
void SV_ClientPrintLinef(const char *fmt_in, ...)
{
	va_list argptr;
	va_super (fmt_out, MAX_INPUTLINE_16384, "%s" NEWLINE, fmt_in); // Append newline to format.
	
	char msg[MAX_INPUTLINE_16384];

	va_start(argptr,fmt_in);
	dpvsnprintf(msg,sizeof(msg),fmt_out,argptr);
	va_end(argptr);
...
// find precache number for sound
	...
	if (isin2 (sv.protocol, PROTOCOL_FITZQUAKE666, PROTOCOL_FITZQUAKE999)) {
		...
		field_mask = 0;
		if (nvolume != DEFAULT_SOUND_PACKET_VOLUME_255)
...
static qbool SV_PrepareEntityForSending (prvm_edict_t *ent, entity_state_t *cs, int enumber)
{
	...
	if (sv_onlycsqcnetworking.integer /*d: 0*/ && !sendentity && enumber > svs.maxclients)
		return false; // Baker: This is NOT normal.
	...
	*cs = defaultstate;
	...
	int is_interactive = Have_Flag ((int)PRVM_serveredictfloat(ent, flags), 
		sv_interactive_flag.integer /*d: 256 FL_ITEM*/ );
	...
	RELATED_ (E5_ALPHA, E5_NON_SOLID_S27)
	...
	if (is_interactive) {
		cs->sflags |= RENDER_FL_INTERACTIVE_512;
	}
	...
	if ((model = SV_GetModelByIndex(modelindex)) && (model->type != mod_null))
	{
		...
		else if (cs->angles[1] || ((effects | model->mf_effects) & EF_ROTATE))
		{
...
void SV_MarkWriteEntityStateToClient(entity_state_t *s, client_t *client)
{
	...
	if (s->number != sv.writeentitiestoclient_cliententitynumber)
	{
			...
			)))
		{
			...
			{
				// Baker: NOT THE NORM BECAUSE is 0 by default sv_cullentities_trace
				int samples =
