...
cvar_t cl_video_frame_modulo = {CF_CLIENT | CF_ARCHIVE, "cl_video_frame_modulo", "0", "run 50% or less of frames by setting to 2, 25% by setting 4 [Zircon]"};
...
RELATED_ (bakergifdecode_open dpvsimpledecode_open)
...
CALLERS_ (OpenVideo DrawQ_SuperPic_Video)
...
void CL_RestartVideo (clvideo_t *video)
{
	...
	// reopen stream
	RELATED_ (dpvsimpledecode_close)
	if (video->stream)
		video->close(video->stream);
	video->stream = NULL;
	if (!OpenStream(video))
...
// update all videos
// Baker: Oct 24 2025 - Where does this get rendered? DrawQ_SuperPic_Video
RELATED_ (DrawQ_SuperPic_Video)
		...
		if (video->state != CLVIDEO_UNUSED_0 && !video->suspended) {
			...
			if (video->is_gif_stream) {
				...
				if (video->framegif_endtime == 0 || time_into_vid >= video->framegif_endtime) {
					...
					RELATED_ (bakergifdecode_videoframe)
			...
			if (video->framenum_vid < destframe) {
				...
				do {
					video->framenum_vid ++;
					RELATED_ (dpvsimpledecode_video)
...
void CL_DrawVideo_SCR_DrawScreen(void)
{
	...
	if (cl_videoplaying == VIDEO_NOT_PLAYING_0) // cl_videoplaying
		return;

	if (cl_videoplaying == VIDEO_PLAYING_PLAYVIDEO_1)
		video = CL_GetVideoBySlot(0);
	else /*if (cl_videoplaying == VIDEO_PLAYING_MENU_2)*/
		video = CL_GetVideoBySlot(1);
...
// Baker: There is no way to calculate time left as we have no information on how long the video is
	...
	else
	{
		...
		if (v_glslgamma_video.value /*d: 1*/ > 0.0)
...
RELATED_ (VM_cin_open uses CL_Cin_OpenVideo)
...
void CL_PlayVideo_Start (char *filename, const char *subtitlesfile, const char *soundname)
{
	...
	cl_videoplaying = VIDEO_PLAYING_PLAYVIDEO_1;
...
void CL_Video_Init( void )
{
	...
	Cvar_RegisterVariable(&cl_video_frame_modulo);
