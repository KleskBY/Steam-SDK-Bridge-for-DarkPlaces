...
RELATED_ (QW_CL_FindModelNumbers qw_monsters)
...
void EntityStateQW_ReadPlayerUpdate (void)
{
	...
	*s = defaultstate;
	...
	if (enumber == cl.playerentity) {
		...
		cl.stats_sv[STAT_WEAPONFRAME] = weaponframe;
		if (playerflags & QW_PF_GIB)
			cl.stats_sv[STAT_VIEWHEIGHT] = 8;
		else if (playerflags & QW_PF_DEAD)
			cl.stats_sv[STAT_VIEWHEIGHT] = -16;
		else
			cl.stats_sv[STAT_VIEWHEIGHT] = 22;
	}
...
RELATED_ (qw_svc_packetentities qw_svc_deltapacketentities)
