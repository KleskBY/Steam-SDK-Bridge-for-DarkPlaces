...
//#include "../steamlauncher/wrath_launcher.h"
...
void Steam_SetName(void)
{
	...
	Cvar_Set(&cvars_all, "steam_name", name);
...
void Steam_SetSteamID(void)
{
	...
	Cvar_Set(&cvars_all, "steam_id", id);
...
void Steam_SetLanguage(void)
{
	...
	Cvar_Set(&cvars_all, "steam_language", lang_code);
}
...
void Steam_OpenOnScreenKeyboard(cmd_state_t *cmd)
{
	if (!steamConnected)
		return;
	...
	type = atoi(cmd_argv_command_0);	// Cmd_Argv(0));	// Baker: Concerned this needs to be parm1
	xpos = atoi(cmd_argv_parm1);		// Cmd_Argv(1));
	ypos = atoi(cmd_argv_parm2);		// Cmd_Argv(2));
	xsize = atoi(cmd_argv_parm3);		// Cmd_Argv(3));
	ysize = atoi(cmd_argv_parm4);		// Cmd_Argv(4));
...
// Baker: I'm guess about the CF_SHARED
cvar_t cv_steamid = { CF_SHARED |CF_USERINFO | CF_NOTIFY | CF_READONLY, "steam_id", "", "steam id of currently logged in client" };
cvar_t cv_steamlang = { CF_SHARED | CF_USERINFO | CF_NOTIFY | CF_READONLY, "steam_language", "", "steam language that was last used" };
cvar_t cv_steaminit = { CF_SHARED |CF_ARCHIVE, "steam_firsttime", "", "steam first time setup" };
cvar_t cv_steamname = { CF_SHARED | CF_USERINFO | CF_NOTIFY | CF_READONLY, "steam_name", "", "steam persona name of currently logged in client" };

CALLERS_ (CL_InitOnce)
void Steam_Startup(void)
{
	...
	// Baker: If these are read-only does this work?
	Cvar_Set(&cvars_all, "steam_id", ""); // start on empty
	Cvar_Set(&cvars_all, "steam_language", ""); // start on empty
	Cvar_Set(&cvars_all, "steam_name", ""); // start on empty
	...
	Cmd_AddCommand(CF_CLIENT, "steam_openkeyboard", Steam_OpenOnScreenKeyboard, "[steam_openkeyboard type xpos ypos xsize ysize] polls steam to open the on-screen keyboard");
