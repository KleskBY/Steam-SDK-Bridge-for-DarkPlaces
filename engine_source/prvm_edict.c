...
#if 0 // Retired April 21 2025 - Thank you for your service
...
cvar_t ent_create_tracepct = {CF_CLIENT, "ent_create_trace_fraction", "0.80", "ent_create traceline percent - allows backing off from impact point. Set to 1 for full distance. [Zircon]"};  // Baker: July 30 2025

cvar_t _val = {CF_CLIENT, "_val", "", "datavar: last value of \"eq\" (or another data command) [Zircon]"};  /// Baker: August 8 2024 .. Baker: Changed to _last Oct 13 2024
cvar_t _pos = {CF_CLIENT, "_pos", "", "datavar: origin set by pos, traceline or other [Zircon]"};  /// Baker: August 8 2024 .. Baker: Changed to _last Oct 13 2024
cvar_t _ang = {CF_CLIENT, "_ang", "", "datavar: angles set by pos or other [Zircon]"};  /// Baker: August 8 2024 .. Baker: Changed to _last Oct 13 2024
cvar_t _ent = {CF_CLIENT, "_ent", "", "datavar: entity num set by traceline, showmodel, etc, [Zircon]"};  /// Baker: August 8 2024 .. Baker: Changed to _last Oct 13 2024
cvar_t _tex = {CF_CLIENT, "_tex", "", "datavar: texture name set by traceline [Zircon]"};  /// Baker: August 8 2024 .. Baker: Changed to _last Oct 13 2024
...
#ifdef CONFIG_MENU
char *PRVM_Gen_Progs_Commad (void)
{
	if (CLVM_prog->loaded && MVM_prog->loaded && SVVM_prog->loaded)
		return "client,menu,server";
	if (CLVM_prog->loaded && MVM_prog->loaded && !SVVM_prog->loaded)
		return "client,menu";
	if (CLVM_prog->loaded && !MVM_prog->loaded && SVVM_prog->loaded)
	...
	if (!CLVM_prog->loaded && !MVM_prog->loaded && SVVM_prog->loaded)
		return "server";
	//if (!CLVM_prog && !MVM_prog && SVVM_prog)
	return "(Not Running!)";
}
#endif
...
prvm_prog_t *PRVM_FriendlyProgFromString(const char *str)
{
	...
	if (!prog)
	{
		Con_PrintLinef ("%s: unknown program name", str);
		return NULL;
	}
	if (!prog->loaded)
	{
		Con_PrintLinef ("%s: program is not loaded", str);
		return NULL;
	}
	return prog;
}
...
void stringlist_append_functions_unique (stringlist_t *plist, prvm_prog_t *prog, ccs *prefix_or_null)
{
	for (int j = 0; j < prog->numfunctions; j ++) {
		mfunction_t	*func = &prog->functions[j];

		int firststatement = func->first_statement;
		if (firststatement < 0) {
			//Con_PrintLinef ("%s progs: function %s is builtin #%d", prog->name, name, -firststatement);
			continue;
		}


		ccs *fname = PRVM_GetString(prog, func->s_name);

		if (prefix_or_null && !String_Starts_With_Caseless (fname, prefix_or_null))
			continue; // DO NOT ADD
		stringlistappend (plist, fname);
	}
	stringlistsort_unique (plist);
}

void stringlist_append_builtins_unique (stringlist_t *plist, prvm_prog_t *prog, ccs *prefix_or_null)
{
	for (int j = 0; j < prog->numfunctions; j ++) {
		mfunction_t	*func = &prog->functions[j];
		...
		if (prefix_or_null && prefix_or_null[0] && !String_Starts_With_Caseless (fname, prefix_or_null))
			continue; // DO NOT ADD
		stringlistappend (plist, fname);
	}
	stringlistsort_unique (plist);
...
static char *PRVM_ValueString (prvm_prog_t *prog, etype_t type, prvm_eval_t *val, char *line, size_t linelength)
{
	...
	case ev_entity:
		...
		if (n < 0 || n >= prog->max_edicts)
			dpsnprintf (line, linelength, "entity %d (invalid!)", n);
		else
			dpsnprintf (line, linelength, "entity %d", n);
		break;
	case ev_function:
	...
	case ev_pointer:
		...
		break;
	default:
		dpsnprintf (line, linelength, "bad type %d", (int) type);
		break;
	}
...
char *PRVM_UglyValueString (prvm_prog_t *prog, etype_t type, prvm_eval_t *val, char *line, size_t linelength)
{
	...
	case ev_entity:
		i = val->edict;
		dpsnprintf (line, linelength, "%d", i);
		break;
	case ev_function:
		if ((unsigned int)val->function < (unsigned int)prog->progs_numfunctions)
		{
	...
	case ev_vector:
		...
		break;
	default:
		dpsnprintf (line, linelength, "bad type %d", type);
		break;
	}
...
char *PRVM_GlobalString (prvm_prog_t *prog, int ofs, char *line, size_t linelength)
{
	...
	if (!def)
		dpsnprintf (line, linelength, "GLOBAL%d", ofs);
	else
	{
...
RELATED_ (PRVM_ED_PrintEdict_SV_f PRVM_ED_PrintNum)
				   ...
				   const char *s_fieldname_partial, const char *s_fieldvalue_partial, stringlist_t *plist)
{
	...
	// if the value is still all 0, skip the field
		...
		if (plist) {
#define valuebuf NO NO
				char valuebuf2[MAX_INPUTLINE_16384];
				c_strlcpy (valuebuf2, name);

				char *sold = stringlist_lastp(plist);
				/*unused qbool did_replace = */ Edict_Value_Fixup_Did_Change (/*key*/ sold, /*value*/ valuebuf2, sizeof(valuebuf2));
				char *sa = Z_StrDupf ("%-14.14s %s", sold, valuebuf2);
...
RELATED_ (PRVM_ED_WriteGlobals)
CALLERS_ (SV_Savegame_to)
...
// "prvm_edicts"
void PRVM_ED_PrintEdicts_Text_f (cmd_state_t *cmd)
{
	CMD_USAGE_NUM_ARGS_DESCRIBE_ (2, "<client|server|menu> <first edict #> [second #] [..] ", "Accumulates text and copies to clipboard");

	ccs *s_progname = cmd_argv_parm1;

	prvm_prog_t *prog = PRVM_FriendlyProgFromString(s_progname);
	CMD_RETURN_IF_FMT_ (!prog, "Program %s not active", s_progname);

	char valuebuf[MAX_INPUTLINE_16384];
	Argv_Cumulate (cmd, LOW_ARG_NUM_2, valuebuf, sizeof(valuebuf) );

	int32list_s ilist = {0};
	int32s_append_split_dequote (&ilist, valuebuf, SPACER);
	...
	for (int idx = 0; idx < ilist.count; idx ++) {
		...
		PRVM_Ent_Text_Add_Edict_Is_Ok (SVVM_prog, bs_malloc, d, /*keep fields list*/ NULL);
...
void PRVM_ED_PrintEdicts_f(cmd_state_t *cmd)
{
	...
	{
		Con_PrintLinef ("prvm_edicts <program name> <optional field name wildcard>");
		return;
	}
...
RELATED_ (PRVM_ED_PrintEdict_f "prvm_printedict")
void PRVM_ED_PrintEdicts_SV_f(cmd_state_t *cmd) // "edicts"
{
	prvm_prog_t *prog = SVVM_prog;

	if (in_range (2, cmd_argc, 3) && String_Match (cmd_argv_parm1, "nearby")) {
		PRVM_ED_anyset_f (cmd, "server"); // Re-route nearby to eset
		return;
	}
...
void PRVM_ED_PrintEdicts_CL_f(cmd_state_t *cmd) // "csedicts"
{
	prvm_prog_t *prog = CLVM_prog;

	//if (Cmd_Argc(cmd) < 2) {
	//	Con_PrintLinef ("%s <edict number>", cmd_argv_command_0);
	//	Con_PrintLinef ("%s nearby - print edicts 300 distance or less", cmd_argv_command_0);
	//	Con_PrintLinef ("%s nearby 200 - print edicts 200 distance or less", cmd_argv_command_0);
	//	return;
	//}

	if (in_range (2, cmd_argc, 3) && String_Match (cmd_argv_parm1, "nearby")) {
		PRVM_ED_anyset_f (cmd, "client"); // Re-route nearby to eset
		return;
	}
...
#ifdef CONFIG_MENU
// "medicts"
void PRVM_ED_PrintEdicts_Menu_f(cmd_state_t *cmd)
{
...
qbool PRVM_Ent_Text_Add_Edict_Is_Ok (prvm_prog_t *prog, baker_string_t *bsa, int edict_num, stringlist_t *plist_keeps)
{
//{
//"classname" "light"
//"spawnflags" "0"
...
//"origin" "-1872 -112 176"
//}
	if (in_range_beyond (1, edict_num, prog->num_edicts) == false) {
		Con_PrintLinef ("Bad edict number (1 to %d valid)", prog->num_edicts - 1);
		return false; // Bad edict number
	}

//	stringlist_t list_pairs = {0};

	RELATED_ (PRVM_ED_EdictGet_Fly_Lite PRVM_Entities_Query_EdictNum_ZAlloc)

	prvm_edict_t	*ed = PRVM_EDICT_NUM(edict_num);

	stringlist_t list_paired = {0};


	for (int fieldnum = 1; fieldnum < prog->numfielddefs; fieldnum ++) {
		mdef_t	*d = &prog->fielddefs[fieldnum];
		ccs	*s_key = PRVM_GetString(prog, d->s_name);

		if (plist_keeps)
			if (stringlist_has_string(plist_keeps, s_key) == false)
				continue; // Not one we want

		prvm_eval_t	*val = (prvm_eval_t *)(ed->fields.fp + d->ofs);

		// if the value is still all 0, skip the field
		int	vartype = Flag_Remove(d->type, DEF_SAVEGLOBAL);

		// Any of the following suffix we ignore, covered by vector.
		if (vartype == ev_float_2 && String_Ends_With_Caseless_3 (s_key, "_x", "_y", "_z"))
			continue;

		qbool was_found = false;
		for (int vector_idx = 0; vector_idx < prvm_type_size[vartype]; vector_idx ++) {
		...
		if (!was_found)
			continue; // Baker: July 30 2025 - Does this happen?

		va_super (crash_location, 1024, "PRVM_ED_Write, ent=%d, name=%s", edict_num, s_key);
		prog->statestring = crash_location;

		char valuebuf[MAX_INPUTLINE_16384];
		char *s_value = PRVM_UglyValueString(prog, (etype_t)d->type, val, valuebuf, sizeof(valuebuf));

		// Baker: Should be a non-zero value here ...			//"classname" "light"
		//stringlistappendf (&list_pairs, QUOTED_S, s_key);		// "classname"
		//stringlistappendf (&list_pairs, QUOTED_S, s_value);		// "light"
		//BakerString_CatCFmt (bsa, QUOTED_S " " QUOTED_S NEWLINE, s_key, s_value);

		stringlistappendf (&list_paired, "%s" TABBER "%s", s_key, s_value);

		prog->statestring = NULL;
	} // each fieldnum

	stringlistsort_no_unique (&list_paired);

	BakerString_CatCFmt (bsa, "{ // edict %d" NEWLINE, edict_num);

	for (int psidx = 0; psidx < list_paired.numstrings; psidx ++) {
		ccs *sxy =  list_paired.strings[psidx];
		char *key_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 1, /*len reply*/ NULL);
		char *val_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 2, /*len reply*/ NULL);

		BakerString_CatCFmt (bsa, QUOTED_S " " QUOTED_S NEWLINE, key_za, val_za);

		Mem_FreeNull_ (key_za);
		Mem_FreeNull_ (val_za);
	}

	BakerString_CatC (bsa, "}" NEWLINE);

	stringlistfreecontents (&list_paired);
...
// "prvm_edict" not "edict"
RELATED_ (PRVM_ED_PrintEdicts_SV_f "edict")
...
RELATED_ (PRVM_ED_Print)


//#define CON_COLOR_SOMETHING "^x88e"

static qbool BitAtoms_Did_Change (char *valuebuf, size_t valuebuf_size, ccs **pkeys, size_t keys_count)
{

	RELATED_ (FS_BitAtomize_f)
	...
	for (uint64_t j = 0; j < 32; j ++) {
		...
		// Is it in the table?
		if (j < keys_count) {
			ccs *s_enumeration_name = pkeys[j];

			// check thisbit here ...
			c_strlcat (s_work, s_enumeration_name);
			continue;
		}
	...
	strlcat (valuebuf, " " CON_BRONZE, valuebuf_size);
...
static qbool Colorizer_Did_Change (char *valuebuf, size_t valuebuf_size)
{
	...
	vec3_t vcolor = {0,0,0};
	COM_Parse_Vector_Solo_Safe_ (data, vcolor);
...
qbool Edict_Value_Fixup_Did_Change (ccs *key, char *valuebuf, size_t valuebuf_size)
{
	...
	if (String_Match(key, "movetype")) { // EASY
		ccs *movetypes[] = {
			...
			"movetype_follow", // 12
			"movetype_fakepush", // 13
			// MOVETYPE_PHYSICS_32
			// MOVETYPE_FLY_WORLDONLY_33
		};
	...
	if (String_Isin1(key, "effects")) {
		ccs *effectsa[] = {
			...
			"EF_NODEPTHTEST_8192",
			"EF_SELECTABLE_16384",
			"EF_DOUBLESIDED_32768",
			"EF_NOSELFSHADOW_65536",
			"EF_DYNAMICMODELLIGHT_131072",
			"EF_UNUSED18_262144",		// 18
			"EF_UNUSED19_524288",		// 19
			"EF_RESTARTANIM_BIT_1048576",
			"EF_TELEPORT_BIT_2097152",
			"EF_LOWPRECISION_4194304",
			"EF_NOMODELFLAGS_8388608",
			"EF_ROCKET_16777216",
			"EF_GRENADE_33554432",
			"EF_GIB_67108864",
			"EF_ROTATE_134217728",
			"EF_TRACER_268435456",	// 28	green split trail
			"EF_ZOMGIB_536870912",	// 29	small blood trail
			"EF_TRACER2_1073741824",	// 30	orange split trail + rotate
			"EF_TRACER3_2147483648",	// 31	purple trail 2147483648
		}; // ccs **pkeys, size_t keys_count Replacer_Did_Change
		return BitAtoms_Did_Change (valuebuf, valuebuf_size, effectsa, ARRAY_COUNT(effectsa));
	}

	if (String_Isin4(key, "flags", "spawnflags", "items", "dphitcontentsmask")) {
		return BitAtoms_Did_Change (valuebuf, valuebuf_size, NULL, 0);
	}
...
// PRVM_ED_PrintNum with &list is the magic
RELATED_ (PRVM_ED_PrintNum PRVM_ED_Print)
RELATED_ (Edict_Value_Fixup_Did_Change Colorizer_Did_Change)
RELATED_ (PRVM_ED_Eset_f "eset")

// "edict", "csedict", "medict"
static void PRVM_ED_PrintEdict_Common_f (cmd_state_t *cmd, prvm_prog_t *prog)
{
	if (Cmd_Argc(cmd) < 2) {
		Con_PrintLinef ("%s <edict number>", cmd_argv_command_0);
		return;
	...
	if (!prog) {
		Con_PrintLinef ("program not running");
		return; // Not running
	}
...
{
	PRVM_ED_PrintEdict_Common_f (cmd, SVVM_prog);
}

#ifdef CONFIG_MENU
// "medict"
static void PRVM_ED_PrintEdict_Menu_f(cmd_state_t *cmd)
{
	PRVM_ED_PrintEdict_Common_f (cmd, MVM_prog);
}
#endif // CONFIG_MENU
...
{
	PRVM_ED_PrintEdict_Common_f (cmd, CLVM_prog);
}
...
RELATED_ (SV_Savegame_to PRVM_ED_Write)
...
void PRVM_ED_ParseGlobals (prvm_prog_t *prog, const char *data)
{
	...
	while (1)
	{
		...
		if (!PRVM_ED_ParseEpair(prog, NULL, key, com_token, qp_parse_backslash_true))
...
qbool PRVM_ED_ParseEpair(prvm_prog_t *prog, prvm_edict_t *ent, mdef_t *key, const char *s, qbool parsebackslash)
{
	...
	case ev_entity:
		...
		i = atoi(s);
		if (i >= prog->limit_edicts)
			Con_PrintLinef ("PRVM_ED_ParseEpair: ev_entity reference too large (edict %u >= MAX_EDICTS_32768 %u) on %s", (unsigned int)i, prog->limit_edicts, prog->name);
	...
	case ev_field:
		if (*s != '.')
		{
			Con_DPrintLinef ("PRVM_ED_ParseEpair: Bogus field name %s in %s", s, prog->name);
			return false;
		}
		def = PRVM_ED_FindField(prog, s + 1);
		if (!def)
		{
			Con_DPrintLinef ("PRVM_ED_ParseEpair: Can't find field %s in %s", s, prog->name);
			return false;
		}
		val->_int = def->ofs;
		break;
	...
	case ev_function:
		...
		if (!func)
		{
			Con_PrintLinef ("PRVM_ED_ParseEpair: Can't find function %s in %s", s, prog->name);
			return false;
		}
	...
	default:
		Con_PrintLinef ("PRVM_ED_ParseEpair: Unknown key->type %d for key " QUOTED_S " on %s", key->type, PRVM_GetString(prog, key->s_name), prog->name);
		return false;
	}
	return true;
}
...
=============
*/
static void PRVM_GameCommand(cmd_state_t *cmd, ccs *whichprogs, ccs *whichcmd)
{
	prvm_prog_t *prog;
	if (Cmd_Argc(cmd) < 1)
	{
...
=============
*/
// s_progname -> client, server or "menu"
...
static void PRVM_ED_GlobalGet_f(cmd_state_t *cmd)
{
	...
	ccs *s_progname		= Cmd_Argv(cmd, 1);
	ccs *s_fieldname	= Cmd_Argv(cmd, 2);
	ccs *s_cvarname		= Cmd_Argv(cmd, 3); // If it exists.

	prvm_prog_t *prog = PRVM_FriendlyProgFromString(s_progname);
	if (!prog)
		return;

	mdef_t *key = PRVM_ED_FindGlobal(prog, Cmd_Argv(cmd, 2));
	if (!key) {
		Con_PrintLinef ( "No global \"%s\" in %s!", s_fieldname, s_progname);
		goto fail;
	}

	prvm_eval_t *v = (prvm_eval_t *) &prog->globals.fp[key->ofs];

	char valuebuf[MAX_INPUTLINE_16384];
	ccs *s = PRVM_UglyValueString(prog, (etype_t)key->type, v, valuebuf, sizeof(valuebuf));
	if (Cmd_Argc(cmd) == 4) {
		cvar_t *cvar = Cvar_FindVar(cmd->cvars, s_cvarname, cmd->cvars_flagsmask);
		if (cvar)
			if (Cvar_Readonly(cvar, "prvm_globalget"))
				goto fail;
		Cvar_Get(cmd->cvars, s_cvarname, s, cmd->cvars_flagsmask, NULL);
	}
	else {
		Con_PrintLinef ("%s", s);

		if (prvm_edictget_autostore.integer /*d:1*/) {
			_Cvar_Set_IsNotify_IsClipboard_String (&_val, q_notifyinfobar_true, prvm_edictget_autostore.integer >= 2 /*d:2*/, q_shall_print_true,
				"%s",
				s
			);
...
RELATED_ (PRVM_ED_PrintEdict_SV_f "edict")

RELATED_ (PRVM_ED_EdictGet_Fly)

#include "prvm_edict_ex.c.h"
...
static void PRVM_ED_EdictSet_f(cmd_state_t *cmd)
{
	...
	ccs *s_progname		= Cmd_Argv(cmd, 1);
	ccs *s_edict_num	= Cmd_Argv(cmd, 2);
	ccs *s_fieldname	= Cmd_Argv(cmd, 3);
	ccs *s_value		= Cmd_Argv(cmd, 4);

	if (cmd_argc < 3) {
		// No params - print information
		// ex: prvm_edictset
		// ex: prvm_edictset server
	...
	if (cmd_argc == 4) {
		// Become "get"
		// ex: prvm_edictset server 25 targetname
	...
	if (cmd_argc == 5) {
		...
		Argv_Cumulate (cmd, /*low arg num*/ 4, valuebuf, sizeof(valuebuf) );
...
// go through all the dictionary pairs
	...
	// parse key
		...
		if (!COM_ParseToken_Simple(&data, false, false, true)) // WRATH: has parsebackslash true here ==> false, true, true
...
void PRVM_ED_LoadFromFile (prvm_prog_t *prog, const char *data)
{
	...
	Con_DPrintf ("%s: %d new entities parsed, %d new inhibited, %d (%d new) spawned (whereas %d removed self, %d stayed)\n", prog->name, parsed, inhibited, prog->num_edicts, spawned, died, spawned - died);
...
// Baker: Wrath has PRVM_PO_Initialize and PRVM_PO_AddFile instead of PRVM_PO_Load
...
CALLERS_ (SV_SpawnServer_VM_Setup -> PRVM_Prog_Init MP_Shutdown CL_VM_ShutDown PRVM_Prog_Init)
void PRVM_Prog_Reset(prvm_prog_t *prog)
{
	if (prog->loaded)
	{
#if 1 // Baker: Wrath 1.12 does not do this ...
...
// byte swap the header
	...
	if (prog->progs_version == PROG_VERSION_FTE_7) {
		...
		if (structtype == PROG_SECONDARYVERSION16 ||
			...
			Con_PrintLinef (CON_WARN "WARNING: %s: %s targets FTEQW, for which support is incomplete. Proceed at your own risk.", prog->name, filename);
		else
	...
	else if (prog->progs_version != PROG_VERSION_6)
		prog->error_cmd("%s: %s has wrong version number (%d should be %d)", prog->name, filename, prog->progs_version, PROG_VERSION_6);
...
//		case OP_GADDRESS:
	...
	if (*prvm_language.string) // Baker: WRATH has || (*prvm_language_fallback.string)) and some other changes.
	...
	for (i=0 ; i<prog->numglobaldefs ; i++)
	{
		...
		)
		{
				...
				switch(prog->globaldefs[i].type & ~DEF_SAVEGLOBAL)
				{
					case ev_float:
						if ((float)((int)(val->_float)) == val->_float)
							dpsnprintf(buf, sizeof(buf), "%d", (int)(val->_float));
						else
						{
							// ftos_slow
							f = val->_float;
			...
			else if (Have_Flag(cvar->flags, CF_PRIVATE) == 0)
			{
...
fail:
	...
	Con_Printf ("%s: program loaded (crc %d, size %dK)\n", prog->name, prog->filecrc, (int)(filesize/1024));
...
// is_type_column = tabbed
#define COLUMNS_NONE_0					0
#define COLUMNS_DATATYPE_1				1
#define COLUMNS_DATATYPE_AND_VALUE_2	2

void stringlistappendfields (stringlist_t *plist, prvm_prog_t *prog, ccs *prefix, int columns_wanted)
{
	for (int idx = 0; idx < prog->numfielddefs; idx ++) {
		mdef_t		*def	= &prog->fielddefs[idx];
		int			type	= def->type;
		int			vartype	= Flag_Remove(type, DEF_SAVEGLOBAL);
		ccs			*s_key	= PRVM_GetString(prog, def->s_name);

		if (strlen(s_key) == 0)
			continue; // Ignore blank

		if (prefix[0] && String_Starts_With (s_key, prefix) == false)
			continue; //

		if (vartype == ev_float_2 && String_Ends_With_Caseless_3 (s_key, "_x", "_y", "_z"))
			continue; // Ignore blank

		ccs *s_datatype = VarType_For_EV(vartype);

		if (columns_wanted == COLUMNS_DATATYPE_1)
			stringlistappendf (plist, "%s" TABBER "%s", s_key, s_datatype);
		else
			stringlistappendf (plist, s_key);
	} // for
}


// is_type_column == 2 means value also.
void stringlistappendglobals (stringlist_t *plist, prvm_prog_t *prog, ccs *prefix, int columns_wanted)
{
	for (int idx = 0; idx < prog->numglobaldefs; idx ++) {
		...
		ccs *s_key = PRVM_GetString(prog, global->s_name);
		...
		ccs *s_datatype = VarType_For_EV(vartype);
		switch (columns_wanted) {
		case COLUMNS_NONE_0:		stringlistappend	(plist, s_key); break;
		case COLUMNS_DATATYPE_1:	stringlistappendf	(plist, "%s" TABBER "%s", s_key, s_datatype); break;

		case COLUMNS_DATATYPE_AND_VALUE_2:
			{
				char valuebuf[MAX_INPUTLINE_16384];
				PRVM_UglyValueString (prog, (etype_t)vartype, (prvm_eval_t *)&prog->globals.fp[global->ofs], valuebuf, sizeof(valuebuf));
				stringlistappendf (plist, "%s" TABBER "%s" TABBER "%s", s_key, s_datatype, valuebuf); break;
			}
...
static void PRVM_Fields_f(cmd_state_t *cmd)
{
	...
	*/
	...
	if (Cmd_Argc(cmd) < 2) {
		Con_PrintLinef ("prvm_fields <program name>");
		Con_PrintLinef ("prvm_fields <program name> sorted");
		Con_PrintLinef ("prvm_fields <program name> sorted <prefix>");
		return;
	}
	ccs *s_progname = cmd_argv_parm1;


	if (!(prog = PRVM_FriendlyProgFromString(s_progname)))
		return;

	qbool is_sorted = String_Match (cmd_argv_parm2, "sorted");
	ccs *prefix = cmd_argv_parm3;
	if (is_sorted) {
		stringlist_t list = {0};
		stringlistappendfields (&list, prog, prefix, COLUMNS_DATATYPE_1);
		stringlistsort_unique (&list);
		for (int sidx = 0; sidx < list.numstrings; sidx ++) {
			ccs *sxy = list.strings[sidx];
			char *s1_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 1, NULL);
			char *s2_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 2, NULL);
			Con_PrintLinef ("%5d " S_FMT_LEFT_PAD_40 " %s", sidx, s1_za, s2_za);

			Mem_FreeNull_ (s1_za); Mem_FreeNull_ (s2_za);
		} // for
		stringlistfreecontents (&list);
		return;
	}
	...
	for (i = 0;i < prog->numfielddefs;i++)
	{
		...
		case ev_pointer:
			...
			break;
		default:
			dpsnprintf (tempstring2, sizeof(tempstring2), "bad type %d ", d->type & ~DEF_SAVEGLOBAL);
...
static void PRVM_Builtins_f(cmd_state_t *cmd)
{
	void Client_Builtins_Dump (cmd_state_t *cmd);
	void Menu_Builtins_Dump (cmd_state_t *cmd);
	void Server_Builtins_Dump (cmd_state_t *cmd);

	ccs *s_progname = cmd_argv_parm1;
	ccs *s_prefix	= cmd_argv_parm2;

	if (String_Match (s_progname, "server"))
		Server_Builtins_Dump (cmd);
	else if (String_Match (s_progname, "client"))
		Client_Builtins_Dump (cmd);
#ifdef CONFIG_MENU
	else if (String_Match (s_progname, "menu"))
		Menu_Builtins_Dump (cmd);
#endif // CONFIG_MENU
	else
		Con_PrintLinef ("Not implemented");
}

static void PRVM_Prog_Builtins_f(cmd_state_t *cmd)
{
	ccs *s_progname = cmd_argv_parm1;
	ccs *s_prefix	= cmd_argv_parm2;

	if (cmd_argc < 2) {
		Con_PrintLinef ("%s <program name> [prefix]", cmd_argv_command_0);
		return;
	}
	...
	Con_PrintLinef ("Note: Optimizations used during prog compile may have stripped these ..");

	stringlist_t list = {0};
	stringlist_append_builtins_unique (&list, prog, s_prefix);
	stringlist_condump_with_linenums (&list);
	stringlistfreecontents (&list);
}

static void PRVM_Functions_f(cmd_state_t *cmd)
{
	ccs *s_progname = cmd_argv_parm1;
	ccs *s_prefix	= cmd_argv_parm2;
	...
	stringlist_t list = {0};
	stringlist_append_functions_unique (&list, prog, s_prefix);
	stringlist_condump_with_linenums (&list);
	stringlistfreecontents (&list);
}
...
static void PRVM_Globals_f(cmd_state_t *cmd)
{
	...
	if (Cmd_Argc (cmd) < 2 || Cmd_Argc(cmd) > 4) {
		Con_PrintLinef ("prvm_globals <program name> <optional name wildcard>");
		Con_PrintLinef ("prvm_globals <program name> sorted");
		Con_PrintLinef ("prvm_globals <program name> sorted <prefix>");
		return;
	}
	...
	if (is_sorted) {
		ccs *prefix = cmd_argv_parm3;
		stringlist_t list = {0};
		stringlistappendglobals (&list, prog, prefix, COLUMNS_DATATYPE_AND_VALUE_2);
		stringlistsort_unique (&list);
		for (int sidx = 0; sidx < list.numstrings; sidx ++) {
			ccs *sxy = list.strings[sidx];
			char *s1_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 1, NULL);
			char *s2_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 2, NULL);
			char *s3_za = String_Instance_Base1_ZAlloc (sxy, TAB_CHAR_9, 3, NULL);
			Con_PrintLinef ("%5d " S_FMT_LEFT_PAD_40 " " S_FMT_LEFT_PAD_8 " %s", sidx, s1_za, s2_za, s3_za);

			Mem_FreeNull_ (s1_za); Mem_FreeNull_ (s2_za); Mem_FreeNull_ (s3_za);
		} // for
		stringlistfreecontents (&list);
		return;
	}
	...
	for (i = 0;i < prog->numglobaldefs;i++)
	{
		...
		Con_PrintLinef ("%s", PRVM_GetString(prog, prog->globaldefs[i].s_name));
	}
	Con_Printf ("%d global variables, %d culled, totalling %d bytes\n", prog->numglobals, numculled, prog->numglobals * 4);
}
...
===============
*/
RELATED_ (PRVM_ED_Eset_f PRVM_ED_GlobalGet_f)
// Baker: Gets the value and prints it.  Versus prvm_globalget ?
// PRVM_ED_GlobalGet_f sets a cvar?
static void PRVM_Global_f(cmd_state_t *cmd)
{
	char valuebuf[MAX_INPUTLINE_16384];

	if (cmd_argc != 3) {
		Con_PrintLinef ( "prvm_global <program name> <global name>" );
		return;
	}
	...
	Con_PrintLinef ( "%s: %s", s_fieldname, valuebuf);
}
...
===============
*/
RELATED_ (PRVM_ED_EdictSet_f PRVM_ED_Eset_f PRVM_ED_GlobalGet_f)

RELATED_ (PRVM_ED_GlobalGet_f)
static void PRVM_GlobalSet_f(cmd_state_t *cmd)
{
	...
	ccs *s_progname		= Cmd_Argv(cmd, 1);
	ccs *s_fieldname	= Cmd_Argv(cmd, 2);
	ccs *s_value		= Cmd_Argv(cmd, 3);

	if (cmd_argc < 2 ) {
		// No params - print information
		// ex: prvm_globalset
		Con_PrintLinef ( "prvm_globalset <program name> <global name> <value>" );
		return;
	}

	if (cmd_argc == 2) {
		// Become "edict"
		// ex: prvm_globalset server
		/*int isok =*/ Print_Globals (s_progname);
		return;
	}

	if (cmd_argc == 3) {
		// Become "get"
		// ex: prvm_globalset server message
		PRVM_ED_Global_Fly (s_progname, s_fieldname, valuebuf, sizeof(valuebuf));
		Con_PrintLinef ("%s", valuebuf);
		if (prvm_edictget_autostore.integer) {
			_Cvar_Set_IsNotify_IsClipboard_String (&_val, q_notifyinfobar_true, prvm_edictget_autostore.integer >= 2 /*d:2*/, q_shall_print_true,
				"%s",
	...
	if (cmd_argc == 4) {
		// 0    1  2     3
		// ex: prvm_globalset server My_Evil_World    (argc 4)
		c_strlcpy (valuebuf, s_value);
	} else {
	...
	/*int isok =*/ Set_A_Global_Print_Errors (s_progname, s_fieldname, valuebuf);
...
void PRVM_Breakpoint(prvm_prog_t *prog, int stack_index, const char *text)
{
	...
	if (prvm_breakpointdump.integer)
		SV_Savegame_to(prog, q_siv_savestring_NULL, va(vabuf, sizeof(vabuf), "breakpoint-%s.dmp", prog->name), q_is_siv_write_false, /*totaltimeatlastexit*/ 0);
}
...
{
	prog->error_cmd("PRVM_EDICT_NUM: %s: bad number %d (called at %s:%d)", prog->name, n, filename, fileline);
	return 0;
}
...
const char *PRVM_GetString(prvm_prog_t *prog, int num)
{
	...
	{
		// invalid
		if (prvm_stringdebug.integer /*d:0*/)
			VM_WarningLinef (prog, "PRVM_GetString: Invalid string offset (%d < 0)", num);
		return "";
	}
	...
	else if (num <= prog->stringssize + prog->tempstringsbuf.maxsize)
	{
		...
		else
		{
			if (prvm_stringdebug.integer)
				VM_WarningLinef (prog, "PRVM_GetString: Invalid temp-string offset (%d >= %d prog->tempstringsbuf.cursize)", num, prog->tempstringsbuf.cursize);
			return "";
		}
	}
	else if (num & PRVM_KNOWNSTRINGBASE)
	{
		...
		if (num >= 0 && num < prog->numknownstrings)
		{
			...
			{
				if (prvm_stringdebug.integer)
					VM_WarningLinef (prog, "PRVM_GetString: Invalid zone-string offset (%d has been freed)", num);
				return "";
			}
		...
		{
			if (prvm_stringdebug.integer)
				VM_WarningLinef (prog, "PRVM_GetString: Invalid zone-string offset (%d >= %d)", num, prog->numknownstrings);
			return "";
		}
	}
	else
	{
		...
		if (prvm_stringdebug.integer)
			VM_WarningLinef (prog, "PRVM_GetString: Invalid constant-string offset (%d >= %d prog->stringssize)", num, prog->stringssize);
		return "";
	}
}
...
const char *PRVM_ChangeEngineString(prvm_prog_t *prog, int i, const char *s)
{
	...
	if (i < 0 || i >= prog->numknownstrings)
		prog->error_cmd("PRVM_ChangeEngineString: string index %d is out of bounds", i);
	else if ((prog->knownstrings_flags[i] & KNOWNSTRINGFLAG_ENGINE) == 0)
		prog->error_cmd("PRVM_ChangeEngineString: string index %d is not an engine string", i);
...
int PRVM_SetTempString(prvm_prog_t *prog, const char *s)
{
	...
	size = (int)strlen(s) + 1;
	if (developer_insane.integer)
		Con_DPrintf ("PRVM_SetTempString: cursize %d, size %d\n", prog->tempstringsbuf.cursize, size);
	if (prog->tempstringsbuf.maxsize < prog->tempstringsbuf.cursize + size)
	{
		...
		if (prog->tempstringsbuf.cursize + size >= 1<<28)
			prog->error_cmd("PRVM_SetTempString: ran out of tempstring memory!  (refusing to grow tempstring buffer over 256MB, cursize %d, size %d)\n", prog->tempstringsbuf.cursize, size);
		...
		{
			Con_DPrintf ("PRVM_SetTempString: enlarging tempstrings buffer (%dKB -> %dKB)\n", old.maxsize/1024, prog->tempstringsbuf.maxsize/1024);
...
void PRVM_FreeString(prvm_prog_t *prog, int num)
{
	...
	else if (num >= PRVM_KNOWNSTRINGBASE && num < PRVM_KNOWNSTRINGBASE + prog->numknownstrings)
	{
		...
		prog->error_cmd("PRVM_FreeString: invalid string offset %d", num);
}
...
static void PRVM_MarkReferencedEdicts(prvm_prog_t *prog)
{
	...
	while(found_new);
	Con_DPrintLinef ("leak check used %d stages to find all references", stage);
}

CALLERS_ (PRVM_Prog_Reset)
void PRVM_LeakTest(prvm_prog_t *prog)
{
	qbool leaked = false;
	...
	// 1. Strings
	for (int i = 0; i < prog->numknownstrings; i ++) {
		if (prog->knownstrings[i])
		if (prog->knownstrings_flags[i])
		if (prog->knownstrings_origin[i])
		if (!PRVM_IsStringReferenced(prog, PRVM_KNOWNSTRINGBASE + i)) {
			Con_PrintLinef ("Unreferenced string found!" NEWLINE "  Value: %s" NEWLINE "  Origin: %s", prog->knownstrings[i], prog->knownstrings_origin[i]);
			leaked = true;
		}
	}
	...
	for (int j = 0; j < prog->num_edicts; j ++) {
		prvm_edict_t *ed = PRVM_EDICT_NUM(j);
		if (ed->free)
			continue;
		if (!ed->priv.required->mark)
	...
	for (int i = 0; i < (int)Mem_ExpandableArray_IndexRange(&prog->stringbuffersarray); i ++) {
		...
		if (stringbuffer->origin) {
			Con_PrintLinef ("Open string buffer handle found!" NEWLINE "  Allocated at: %s", stringbuffer->origin);
			leaked = true;
		}
	}

	for (int i = 0; i < PRVM_MAX_OPENFILES_256; i++) {
		if (prog->openfiles[i])
		if (prog->openfiles_origin[i]) {
			Con_PrintLinef ("Open file handle found!" NEWLINE "  Allocated at: %s", prog->openfiles_origin[i]);
			leaked = true;
		}
	}

	for (int i = 0; i < PRVM_MAX_OPENSEARCHES_128; i ++) {
		if (prog->opensearches[i])
		if (prog->opensearches_origin[i]) {
			Con_PrintLinef ("Open search handle found!" NEWLINE "  Allocated at: %s", prog->opensearches_origin[i]);
			leaked = true;
		}
	}

	// Baker:
	for (int j = 0; j < PRVM_MAX_OPEN_DOT_MAPS_8; j ++) {
		if (prog->open_dot_maps[j]) {
			Con_PrintLinef ("Open dot map handle found!");
		}
	} // for

	if (!leaked)
		Con_PrintLinef ("Congratulations. No leaks found.");
}
			...
			if (prog->knownstrings[num] && (prog->knownstrings_flags[num] & (KNOWNSTRINGFLAG_GCMARK | KNOWNSTRINGFLAG_ENGINE)) == 0)
			{
				if (prog->knownstrings_flags[num] & KNOWNSTRINGFLAG_GCPRUNE)
				{
					...
					if (prvm_garbagecollection_notify.integer) {
						if (prvm_garbagecollection_notify.integer > 1)
							Con_PrintLinef (CON_RED "prvm_garbagecollection_notify: %s: freeing unreferenced string %d: " QUOTED_S, prog->name, num, prog->knownstrings[num]);
						else
...
void PRVM_InitOnce (void)
{
	...
	Cmd_AddCommand(CF_SHARED, "prvm_edicts_text_to_clipboard", PRVM_ED_PrintEdicts_Text_f, "copies edict text for space delimited entity numbers to clipboard (server, client, menu)");
...
#ifdef CONFIG_MENU
	Cmd_AddCommand (CF_SHARED, "medict", PRVM_ED_PrintEdict_Menu_f, "print all data about an entity number in the menu VM [Zircon]");  // Baker r1083: "edict" and "edicts"
#endif

	// Baker: Notice this is plural!
	Cmd_AddCommand (CF_SHARED, "edicts", PRVM_ED_PrintEdicts_SV_f, "prints all data about all entities in the server VM.  edicts <classname> or edicts <fieldnamepartial> <fieldvaluepartial> [Zircon]");  // Baker r1083: "edict" and "edicts"
	Cmd_AddCommand (CF_SHARED, "csedicts", PRVM_ED_PrintEdicts_CL_f, "prints all data about all entities in the client VM [Zircon]");  // Baker r1083: "edict" and "edicts"

#ifdef CONFIG_MENU
	Cmd_AddCommand (CF_SHARED, "medicts", PRVM_ED_PrintEdicts_Menu_f, "prints all data about all entities in the menu VM [Zircon]");  // Baker r1083: "edict" and "edicts"
#endif
	...
	Cmd_AddCommand(CF_SHARED, "prvm_functions", PRVM_Functions_f, "prints functions in the selected VM (server, client, menu) [Zircon]");
	Cmd_AddCommand(CF_SHARED, "prvm_builtins", PRVM_Builtins_f, "prints list of engine builtins in the selected VM (server, client, menu) [Zircon]");
	Cmd_AddCommand(CF_SHARED, "prvm_builtins_in_prog", PRVM_Prog_Builtins_f, "prints builtins used in the prog for the selected VM (server, client, menu) [Zircon]");
	...
	Cmd_AddCommand(CF_SHARED | CF_CHEAT, "cset", PRVM_ED_Cset_f, "prvm_edictset for client [Zircon]");
#ifdef CONFIG_MENU
	Cmd_AddCommand(CF_SHARED | CF_CHEAT, "mset", PRVM_ED_Mset_f, "prvm_edictset for menu [Zircon]");
#endif
	...
	Cmd_AddCommand(CF_SHARED, "prvm_edict_create_csqc", PRVM_ED_Edict_Create_CSQC_f, "spawn() a CSQC entity [Zircon]");
	Cmd_AddCommand(CF_SHARED, "prvm_edict_remove_csqc", PRVM_ED_Edict_Remove_CSQC_f, "remove() a CSQC entity [Zircon]");
...
//	Cvar_RegisterVariable (&prvm_sv_progfields); // Baker r7103 gamecommand autocomplete
	...
	Cvar_RegisterVariable (&_ang);
	Cvar_RegisterVariable (&_ent);
	Cvar_RegisterVariable (&_pos);
	Cvar_RegisterVariable (&_tex);
	Cvar_RegisterVariable (&_val);

	Cvar_RegisterVariable (&ent_create_tracepct);
