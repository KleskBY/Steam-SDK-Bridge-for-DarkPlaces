...
CALLERS_ (CL_ClientMovement_Physics_Swim, CL_ClientMovement_Physics_Walk x 2 )

CALLERS_ (CL_ClientMovement_Move plus 3 others)
...
RELATED_ (Calls CL_ClientMovement_UpdateStatus)
...
RELATED_ (CL_Frame -> CL_UpdateWorld -> )
RELATED_ (CL_UpdateMoveVars CL_ClientMovement_UpdateStatus)
...
void CL_ClientMovement_Replay (int collide_type)
{
	...
	cl.movement_predicted = /*MOVED this BELOW --> totalmovemsec >= cl_movement_minping.value d: 0 && */
		...
		cls.signon == SIGNONS_4 &&
			(cl.stats_sv[STAT_HEALTH] > 0 || QW_TREAT_AS_SPECTATOR) &&
			!cl.intermission
		);
