...
levtime:
	...
	Sbar_DrawString (8, 12, va(vabuf, sizeof(vabuf), "Kills: %d/%d", cl.stats_sv[STAT_MONSTERS], cl.stats_sv[STAT_TOTALMONSTERS]));
	Sbar_DrawString (8 + 26 * 8, 12, va(vabuf, sizeof(vabuf), "Secrets: %d/%d", cl.stats_sv[STAT_SECRETS], cl.stats_sv[STAT_TOTALSECRETS]));
...
static void Sbar_DrawInventory (void)
{
	...
	if (gamemode == GAME_ROGUE) {
		if ( cl.stats_sv[STAT_ACTIVEWEAPON] >= RIT_LAVA_NAILGUN ) 
	...
	// weapons
	for (i = 0 ; i < 7 ; i ++) {
		if (cl.stats_sv[STAT_ITEMS] & (IT_SHOTGUN<<i) ) {
			time = cl.item_gettime[i];
			flashon = (int)(max(0, cl.time - time)*10);
			if (flashon >= 10) {
				if ( cl.stats_sv[STAT_ACTIVEWEAPON] == (IT_SHOTGUN<<i)  ) flashon = 1;
				else flashon = 0;
	...
	if (gamemode == GAME_HIPNOTIC || gamemode == GAME_QUOTH) {
		...
		for (i = 0 ; i < 4 ; i ++) {
			if (cl.stats_sv[STAT_ITEMS] & (1<<hipweapons[i]) ) {
				time = max(0, cl.item_gettime[hipweapons[i]]);
				flashon = (int)((cl.time - time)*10);
				if (flashon >= 10) {
					if ( cl.stats_sv[STAT_ACTIVEWEAPON] == (1<<hipweapons[i])  ) flashon = 1;
					else flashon = 0;
				}
				else flashon = (flashon%5) + 2;
				...
				if (i==2) {
					if (cl.stats_sv[STAT_ITEMS] & HIT_PROXIMITY_GUN) {
						if (flashon) {
							grenadeflashing = 1;
					...
					if (cl.stats_sv[STAT_ITEMS] & (IT_SHOTGUN<<4)) {
						if (!grenadeflashing)
	...
	if (gamemode == GAME_ROGUE) {
		...
		if ( cl.stats_sv[STAT_ACTIVEWEAPON] >= RIT_LAVA_NAILGUN )
			for (i=0;i<5;i++)
				if (cl.stats_sv[STAT_ACTIVEWEAPON] == (RIT_LAVA_NAILGUN << i))
					Sbar_DrawPic ((i+2)*24, -16, rsb_weapons[i]);
	}

	// ammo counts
	for (i=0 ; i<4 ; i++) {
		dpsnprintf (num, sizeof(num), "%4i",cl.stats_sv[STAT_SHELLS+i] );
	...
	// items
	for (i=0 ; i<6 ; i++) {
		if (cl.stats_sv[STAT_ITEMS] & (1<<(17+i))) {
	...
	if (gamemode == GAME_HIPNOTIC || gamemode == GAME_QUOTH) {
		for (i=0 ; i<2 ; i++)
			if (cl.stats_sv[STAT_ITEMS] & (1<<(24+i)))
	...
	if (gamemode == GAME_ROGUE) {
		// new rogue items
		for (i=0 ; i<2 ; i++)
			if (cl.stats_sv[STAT_ITEMS] & (1<<(29+i)))
				Sbar_DrawPic (288 + i*16, -16, rsb_items[i]);
	} else {
		// sigils
		for (i=0 ; i<4 ; i++)
			if (cl.stats_sv[STAT_ITEMS] & (1<<(28+i)))
...
// PGM 01/19/97 - team color drawing
	...
	if ( (cl.stats_sv[STAT_ITEMS] & (IT_INVISIBILITY | IT_INVULNERABILITY) ) == (IT_INVISIBILITY | IT_INVULNERABILITY) )
		Sbar_DrawPic (112, 0, sb_face_invis_invuln);
	else if (cl.stats_sv[STAT_ITEMS] & IT_QUAD)	Sbar_DrawPic (112, 0, sb_face_quad );
	else if (cl.stats_sv[STAT_ITEMS] & IT_INVISIBILITY)	Sbar_DrawPic (112, 0, sb_face_invis );
	else if (cl.stats_sv[STAT_ITEMS] & IT_INVULNERABILITY)	Sbar_DrawPic (112, 0, sb_face_invuln);
	else {
		f = cl.stats_sv[STAT_HEALTH] / 20;
		f = bound(0, f, 4);
...
void Sbar_ShowFPS(void)
{
	...
	if (showangles.integer) {
		if (cl.entities) {
			if (showangles.integer > 1) {
				// v_forward makevectors
				RELATED_ (VM_makevectors)
				vec3_t forward;//, right, up;
				AngleVectors(cl.entities[cl.playerentity].state_current.angles, forward, NULL, NULL);
				dpsnprintf(angstring, sizeof(angstring), VECTOR3_5d1F, VECTOR3_SEND(forward));					
				fps_strings++;
			} else {
	...
	if (showtex.integer) {
		...
		if (showtex.integer < 0) {
			vec3_t vdelta;
			VectorSubtract (cltrace.endpos, org, vdelta);
			float d = VectorLength (vdelta);
			c_dpsnprintf1(entstring, "Distance: %3.1f", d);
			fps_strings++;
			VectorCopyDestSrc (cl.tool_marker_origin, cltrace.endpos);
			RELATED_ (SCR_ToolMarker_f)
		} 
		else
		if (showtex.integer >= 2) {
			...
			if (showtex.integer < 5 && sv.active) {
				...
				svtrace = SV_TraceLine(org, dest, MOVE_HITMODEL_4, plyr, SUPERCONTENTS_SOLID, 0, 
...
//
		...
		else if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_QUAD) )				iface = sb_face_quad;
		else if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_INVISIBILITY) )		iface = sb_face_invis;
		else if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_INVULNERABILITY) )	iface = sb_face_invuln;
		else {
			fi = cl.stats_sv[STAT_HEALTH] / 20;
			fi = bound(0, fi, 4);
		...
		Sbar_DrawNum2 (
			...
			cl.stats_sv[STAT_HEALTH],					// num
			3,											// numdigits
			/*coloridx*/ cl.stats_sv[STAT_HEALTH] <= 25	// 0 or 1 is red
		);
		...
		iface = NULL;
		if (gamemode == GAME_ROGUE) {
			     if (cl.stats_sv[STAT_ITEMS] & RIT_SHELLS)				iface = sb_ammo[0];
			else if (cl.stats_sv[STAT_ITEMS] & RIT_NAILS)				iface = sb_ammo[1];
			else if (cl.stats_sv[STAT_ITEMS] & RIT_ROCKETS)				iface = sb_ammo[2];
			else if (cl.stats_sv[STAT_ITEMS] & RIT_CELLS)				iface = sb_ammo[3];
			else if (cl.stats_sv[STAT_ITEMS] & RIT_LAVA_NAILS)			iface = rsb_ammo[0];
			else if (cl.stats_sv[STAT_ITEMS] & RIT_PLASMA_AMMO)			iface = rsb_ammo[1];
			else if (cl.stats_sv[STAT_ITEMS] & RIT_MULTI_ROCKETS)		iface = rsb_ammo[2];
		} else {
			if (cl.stats_sv[STAT_ITEMS] & IT_SHELLS)					iface = sb_ammo[0];
			else if (cl.stats_sv[STAT_ITEMS] & IT_NAILS)				iface = sb_ammo[1];
			else if (cl.stats_sv[STAT_ITEMS] & IT_ROCKETS)				iface = sb_ammo[2];
...
no_ammo:
		...
		if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_INVULNERABILITY)) {
			...
			if (gamemode == GAME_ROGUE) {
				...
				//Sbar_DrawNum (24, 0, cl.stats_sv[STAT_ARMOR], 3, /*numscoloridx*/ cl.stats_sv[STAT_ARMOR] <= 25);
				if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_ARMOR3))				iface = sb_armor[2];
				else if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_ARMOR2))		iface = sb_armor[1];
				else if (Have_Flag (cl.stats_sv[STAT_ITEMS], IT_ARMOR1))		iface = sb_armor[0];
			}
		}
		...
		Sbar_DrawNum2 (
			...
			frow0,										// y
			sbar_pelz,									// lettersize
			iface == sb_disc ? 666 : cl.stats_sv[STAT_ARMOR],					// num
			3,											// numdigits
			/*coloridx*/ cl.stats_sv[STAT_ARMOR] <= 25	// 0 or 1 is red
		);
...
no_armor:
		...
		if (sb_showscores) {
			...
			Sbar_DrawScoreboard ();
		} else if (sb_showscores || (cl.stats_sv[STAT_HEALTH] <= 0 && /*defaults 1*/ cl_deathscoreboard.integer)) {
			// DEAD
...
cl_deathscoreboard.integer && cl.scores[cl.playerentity-1].qw_spectator == false)) {
		...
		Sbar_DrawScoreboard ();
	} else if (sb_showscores || (cl.stats_sv[STAT_HEALTH] <= 0 && cl_deathscoreboard.integer && cl.scores[cl.playerentity-1].qw_spectator == false)) {
		// DEAD
		Sbar_DrawAlphaPic (0, 0, sb_scorebar, sbar_alpha_bg.value);
				Sbar_DrawScoreboard ();
			} else if (sb_lines) {
				...
				if (gamemode == GAME_HIPNOTIC || gamemode == GAME_QUOTH) {
					if (cl.stats_sv[STAT_ITEMS] & IT_KEY1)
						Sbar_DrawPic (209, 3, sb_items[0]);
					if (cl.stats_sv[STAT_ITEMS] & IT_KEY2)
						Sbar_DrawPic (209, 12, sb_items[1]);
				}

				// armor
				if (cl.stats_sv[STAT_ITEMS] & IT_INVULNERABILITY) {
					...
					if (gamemode == GAME_ROGUE) {
						...
						else if (cl.stats_sv[STAT_ITEMS] & RIT_ARMOR1) Sbar_DrawPic (0, 0, sb_armor[0]);
					} else { // NOT GAME_ROGUE
						Sbar_DrawNum (24, 0, cl.stats_sv[STAT_ARMOR], 3, cl.stats_sv[STAT_ARMOR] <= 25);
						if (cl.stats_sv[STAT_ITEMS] & IT_ARMOR3) Sbar_DrawPic (0, 0, sb_armor[2]);
						else if (cl.stats_sv[STAT_ITEMS] & IT_ARMOR2) Sbar_DrawPic (0, 0, sb_armor[1]);
				...
				// health
				Sbar_DrawNum (136, 0, cl.stats_sv[STAT_HEALTH], 3, cl.stats_sv[STAT_HEALTH] <= 25);

				// ammo icon
				if (gamemode == GAME_ROGUE) {
					if (cl.stats_sv[STAT_ITEMS] & RIT_SHELLS) Sbar_DrawPic (224, 0, sb_ammo[0]);
					else if (cl.stats_sv[STAT_ITEMS] & RIT_NAILS) Sbar_DrawPic (224, 0, sb_ammo[1]);
					else if (cl.stats_sv[STAT_ITEMS] & RIT_ROCKETS) Sbar_DrawPic (224, 0, sb_ammo[2]);
					else if (cl.stats_sv[STAT_ITEMS] & RIT_CELLS) Sbar_DrawPic (224, 0, sb_ammo[3]);
					else if (cl.stats_sv[STAT_ITEMS] & RIT_LAVA_NAILS) Sbar_DrawPic (224, 0, rsb_ammo[0]);
					else if (cl.stats_sv[STAT_ITEMS] & RIT_PLASMA_AMMO) Sbar_DrawPic (224, 0, rsb_ammo[1]);
					else if (cl.stats_sv[STAT_ITEMS] & RIT_MULTI_ROCKETS) Sbar_DrawPic (224, 0, rsb_ammo[2]);
				} else {
					if (cl.stats_sv[STAT_ITEMS] & IT_SHELLS) Sbar_DrawPic (224, 0, sb_ammo[0]);
					else if (cl.stats_sv[STAT_ITEMS] & IT_NAILS) Sbar_DrawPic (224, 0, sb_ammo[1]);
					else if (cl.stats_sv[STAT_ITEMS] & IT_ROCKETS) Sbar_DrawPic (224, 0, sb_ammo[2]);
					else if (cl.stats_sv[STAT_ITEMS] & IT_CELLS) Sbar_DrawPic (224, 0, sb_ammo[3]);
				}

				Sbar_DrawNum (248, 0, cl.stats_sv[STAT_AMMO], 3, cl.stats_sv[STAT_AMMO] <= 10);
...
// LA: Display as "a" instead of "a/b" if b is 0
	if (1 || cl.stats_sv[STAT_TOTALSECRETS]) // Baker r1082: display total monsters/secrets at all times like Quake
	{
		Sbar_DrawNum (160, 104, cl.stats_sv[STAT_SECRETS], 3, 0);
		if (!IS_OLDNEXUIZ_DERIVED(gamemode))
			Sbar_DrawPic (232, 104, sb_slash);
		Sbar_DrawNum (240, 104, cl.stats_sv[STAT_TOTALSECRETS], 3, 0);
	}
	else
	{
		Sbar_DrawNum (240, 104, cl.stats_sv[STAT_SECRETS], 3, 0);
	}

	if (1 || cl.stats_sv[STAT_TOTALMONSTERS]) // Baker r1082: display total monsters/secrets at all times like Quake
	{
		Sbar_DrawNum (160, 144, cl.stats_sv[STAT_MONSTERS], 3, 0);
		if (!IS_OLDNEXUIZ_DERIVED(gamemode))
			Sbar_DrawPic (232, 144, sb_slash);
		Sbar_DrawNum (240, 144, cl.stats_sv[STAT_TOTALMONSTERS], 3, 0);
	}
	else
	{
		Sbar_DrawNum (240, 144, cl.stats_sv[STAT_MONSTERS], 3, 0);
	}
}
