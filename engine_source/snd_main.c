...
static void S_Play_Common (cmd_state_t *cmd, float fvol, float attenuation, qbool is_forceloop)
{
	...
	while (i < Cmd_Argc (cmd)) {
		...
		sfx = S_PrecacheSound (name, q_tx_complain_true, /*levelsound?*/ true);
		if (sfx) {
...
static void S_PlayVol_f (cmd_state_t *cmd)
{
	...
	if (Cmd_Argc(cmd) == 4) {
		...
		ccs *sext	= File_URL_GetExtension_With_Dot(name);
		if (!sext)
...
static void S_SoundList_f(cmd_state_t *cmd)
{
	...
	Con_PrintLinef ("Total resident: %d", total);
}
...
void S_StaticSound (sfx_t *sfx, vec3_t origin, float fvol, float attenuation)
{
	...
	if (!sfx->fetcher)
	{
		Con_PrintLinef ("S_StaticSound: " QUOTED_S " hasn't been precached", sfx->name);
		return;
	}
...
static void S_UpdateAmbientSounds (void)
{
	...
	RELATED_ (Mod_Q1BSP_AmbientSoundLevelsForPoint)
